...
var NetstarBlockList = (function () {
		...
		if (expression) {
				...
				for (var i = 0; i < strArr.length; i++) {
					if (strArr[i].indexOf('.') > -1) {
						...
						if (strArr[i].match(rex2)[1] == 'iconClass') {
							...
							} else if (typeof (data[strArr[i].match(rex2)[1]]) == 'boolean') {
								str = data[strArr[i].match(rex2)[1]];
							}
							if (strArr[i].match(rex2)[1] == 'workItemState') {
								...
								if (data.rollbackType == 3) {
									str = '回退';
								} else if (data.rollbackType == 0) {
									str = '待办';
								}
								...
								if (chargeIcon[chargeIconByField]) {
									if(str == '回退'){
										listHtml = listHtml.replace('ns-icon="workItemState"', 'ns-icon="rollbackstate-icon"');
									}else{
										listHtml = listHtml.replace('ns-icon="workItemState"', 'ns-icon="workitemstate-icon"');
									}
							...
							} else if (strArr[i].match(rex2)[1] == 'hasOvertime') {
								if (typeof (str) == 'boolean') {
									if (str === true) {
										str = '超期';
										listHtml = listHtml.replace('ns-icon="hasOvertime"', 'ns-icon="overduestate"');
									} else {
										str = '';
										listHtml = listHtml.replace('ns-icon="hasOvertime"', 'ns-icon=""');
									}
								}
							} else if (strArr[i].match(rex2)[1] == 'longPeriod') {
								str = '';
								if (Number(originalRow.longPeriod) == 1) {
									str = '长周期';
								} else {
									listHtml = listHtml.replace('ns-icon="long-period"', 'ns-icon=""');
								}
							}
							var columnJson = typeof (column[strArr[i].match(rex2)[1]]) == 'object' ? column[strArr[i].match(rex2)[1]] : {};
							if (columnJson.columnType == 'buildPdf') {
								var colValue = '';
								if (str) {
									var valArr = typeof (str) == "string" ? str.split(',') : [];
									colValue = valArr.length == 1 ? '附件' : valArr.length + '个附件';
								} else {
									colValue = '生成';
								}
								var html = '<div class="pt-btn-group">' +
									'<button class="pt-btn pt-btn-default" onclick="NetStarGrid.methodsManager.buildPdfFuncByBlockList(this,' + rowIndex + ',\'' + _vueData.$options.id + '\',\'' + strArr[i].match(rex2)[1] + '\')">' + colValue + '</button>' +
									'</div>';
								listHtml = listHtml.replace(strArr[i], html);
							} else {
			...
			if (data.propertyValue && data.propertyValue.indexOf('[') == 0) {
				...
				for (var p = 0; p < propertyValueArr.length; p++) {
					var propertyData = propertyValueArr[p];
					//if(propertyData.propName && propertyData.customValue){
					propertyValueHtml += '<div class="pt-block-content-item">' +
						'<span class="tag">' + propertyData.propName + '：</span>' +
						'<span class="code">' + propertyData.customValue + '</span>' +
						'</div>';
					//}
				}
				listHtml += propertyValueHtml;
			}
			value = listHtml;
		} else {
	...
	methodsManager.body.NetstarBlockState = function (data) {
		var classStr = '';
		if (typeof (data.nsStateLabel) == 'number') {
			switch (Number(data.nsStateLabel)) {
				case 0:
					//正常
					classStr = ' pt-state-normal';
					break;
				case 1:
					//警告
					classStr = ' pt-state-warning';
					break;
				case 2:
					//错误
					classStr = ' pt-state-error';
					break;
				case 3:
					//成功
					classStr = ' pt-state-success';
					break;
				case 4:
					//禁用
					classStr = ' pt-state-disabled';
					break;
			}
		} else {
						...
						handler: function (newRowsData, oldRowsData) {
							...
							for (var i = 0; i < this.originalRows.length; i++) {
								if (this.rows[i]) {
									...
									this.originalRows[i].netstarSelectedFlag = typeof(this.originalRows[i].netstarSelectedFlag) == "boolean" ? this.originalRows[i].netstarSelectedFlag : false;
								}
							}
	...
	function deleteAjaxRow(data) {
		...
		*/
		...
		var ajaxConfig = nsVals.getAjaxConfig(deleteAjax, data.rowData, {
			idField: configs.gridConfig.data.idField
		});
		ajaxConfig.plusData = {
			data: data
		};
