 ...
 * @LastEditTime: 2019-12-19 10:32:37
 */
...
NetstarUI.resultTable.init = function(config, reIntoData){
	...
	//初始化设备接口
    NetstarUI.resultTable.cloudInstrument = new NetstarUI.resultTable.cloudInstrumentObject();
}
//重新加载时需删除非必要属性
				...
				switch(tdData.originalType){
					...
					case 'datestring':
						...
						if(typeof(tdData.value.real)=='string'){
							...
							if($.isEmptyObject(realStr)){realStr = '';}
							if(realStr == ''){
								visibleStr = '';
...
NetstarUI.resultTable.stringInit = function(resizeData){
	...
	$errorLabel.remove();
	//清除辅助输入面板
	var $assBtns = $currentInput.closest('.input-string').find('.pt-text-assistant-btns');
	$assBtns.remove();
	...
	//输入辅助 根据字典 INPUTASSIST  cy 191106 start------
	$currentInput.off('keyup');
	$currentInput.on('keyup', function(ev){
		console.log(ev);
	});
	function assistantInput(){
		var btnHtml = '';
		var dictDatas = nsVals.dictData.INPUTASSIST.subdata;
		for(var i = 0; i<dictDatas.length; i++){
			var dictValue = dictDatas[i].value;
			if(dictValue && dictValue!= ''){
				btnHtml += '<button class="pt-btn pt-btn-default">'+dictValue+'</button>';
			}
		}
		var assistantHtml = '<div class="pt-text-assistant-btns"><div class="pt-btn-group">'+btnHtml+'</div></div>';
		var $assistantPanel = $(assistantHtml);

		$currentInput.parent().append($assistantPanel);
		...
		$assistantPanel.css(cssObj);
		//判断辅助输入面板是否需要向上显示 start------------
		var wHeight = $(window).height();
		var iHeight = $currentInput.parent().height();
		var oHeight = $currentInput.offset().top;
		var cHeight = $assistantPanel.height();
		if(oHeight + cHeight + iHeight > wHeight){
			if(oHeight > cHeight){
				cssObj.bottom = cssObj.top;
				cssObj.top = 'auto';
				$assistantPanel.css(cssObj);
			}
		}
		...
		debugger;
		//判断辅助输入面板是否需要向上显示 end  ------------

		$assistantPanel.find('button').on('click', function(ev){
			ev.stopPropagation();
			ev.preventDefault(); 
			var inputEv = $currentInput[0];
			var name = $(this).text();
	...
	function initAssistantInput(){
		if($currentInput.length == 1){
			...
			if(nsVals){
				if(nsVals.dictData){
					if(nsVals.dictData.INPUTASSIST){
						if(nsVals.dictData.INPUTASSIST.subdata){
							//当按下 ctrl+alt+r
							$currentInput.on('keyup', function(ev){
								if(ev.ctrlKey && ev.altKey && ev.key == 'r'){
									assistantInput();
								}
	...
	//输入辅助 根据字典 INPUTASSIST  cy 191106 end  ------
				...
				var fieldArray = [
					{
						...
						type: 		'upload',
						rules: 		'required',
						btns:['upload','edit','delete','download'],
						ajax:{
							url : getRootPath() + '/files/uploadList',
						},
						editAjax:{
							url : getRootPath() + '/files/rename',
						},
						downloadAjax:{
							url : getRootPath() + '/files/download/',
						},
						getFileAjax:{
						...
						valueField: 'id',
						fileTypeField:'photoType',
						previewAjax:{
							type:'GET',
							url:getRootPath()+'/files/pdf'
						},
						previewImagesAjax:{
							type:'GET',
							url:getRootPath()+'/files/images'
						},
						isShowThum : true,
                    	isMultiple : true,
					}
					...
					btns:[
						{
							...
							handler:function(data){
								var valueJson = NetstarComponent.getValues('dialog-upload-resulttable-dialog-body');
								if(valueJson){
									var postData = {
										title:valueJson.sqid,
										recordId:tdData.value.data.recordId,
										testDataId:tdData.nsId,
										bigThumb:valueJson.imgId,
									};
									var ajaxConfig = {
										url:getRootPath()+'/sample/detection/testData/saveListThumb',
										type:'post',
										data:postData
									};
									NetStarUtils.ajax(ajaxConfig,function(data){
										if(data.success){
											resizeData.tdData.subdata = data.rows;
											nsalert('保存成功');
											NetstarComponent.dialog['upload-resulttable-dialog'].vueConfig.close();
											NetstarUI.resultTable.getUploadResult(resizeData);
										}else{
											nsalert(data.result);
										}
...
NetstarUI.resultTable.uploadtitleInit = function(resizeData){
	...
	function clickHandler(){
		var dialogConfig = {
			...
			shownHandler : function(_showConfig){
				var valueFileStr = '';
				if($.isArray(tdData.subdata)){
					if(tdData.subdata[0]){
						valueFileStr = tdData.subdata[0].filePath
					}
				}
				var fieldArray = [
					{
					...
					btns:[
						{
							...
							handler:function(data){
								if(valueFileStr){
									NetStarUtils.ajax({url:getRootPath()+'/files/delById',contentType:'application/x-www-form-urlencoded',type:'post',data:{id:valueFileStr}},function(res){
										if(res.success){
								...
								if(valueJson){
									var postData = {
										...
										recordId:tdData.value.data.recordId,
										testDataId:tdData.nsId,
										bigThumbFileId:valueJson.imgId,
									};
									var ajaxConfig = {
										url:getRootPath()+'/sample/detection/testData/saveThumb',
										type:'post',
										data:postData
									};
									NetStarUtils.ajax(ajaxConfig,function(data){
										if(data.success){
											tdData.subdata = [data.data];
											nsalert('保存成功');
											NetstarComponent.dialog['upload-resulttable-dialog'].vueConfig.close();
											NetstarUI.resultTable.getUploadResult(resizeData);
										}else{
...
NetstarUI.resultTable.getUploadVisible = function(tdData){
    ...
    if(tdData.subdata.length > 0){
		for(var imageI=0; imageI<tdData.subdata.length; imageI++){
			...
			var filePath = tdData.subdata[imageI].filePath;
			var srcStr = getRootPath()+'/files/images/'+filePath;
			var authorization = NetStarUtils.OAuthCode.get();
			if(authorization){
				srcStr += '?Authorization=' + authorization;
			}
			/*if(typeof(imageId)=='undefined'){
				imageId = tdData.subdata[imageI].bigThumbFileId;
				srcStr = getRootPath()+'/files/images/'+imageId;
				var authorization = NetStarUtils.OAuthCode.get();
				if(authorization){
					srcStr += '?Authorization=' + authorization;
				}
...
NetstarUI.resultTable.initShortkey = function(){
	$(document).off('keydown', NetstarUI.resultTable.keydownHandler);
	$(document).on('keydown', NetstarUI.resultTable.keydownHandler);
}
NetstarUI.resultTable.enterSave = function(callBackFunc){
...
//快捷键 上下左右回车  ctrl+alt+f 全屏/退出全屏
NetstarUI.resultTable.keydownHandler = function(ev){
	var tdData = NetstarUI.resultTable.data.tdData[NetstarUI.resultTable.currentTdID];
	...
	switch(ev.keyCode){
		...
		case 9:
			...
			switch(NetstarUI.resultTable.shortkeyType){
				...
				default:
					// moveInput('enter');
					NetstarUI.resultTable.enterSave(function(){
						moveInput('enter');
					})
					break;
			}
			...
			if(tdData.value.real != resultData.value){
				switch(resultDataType){
					...
					case 'none':
						...
						break;
					//补充了对保存的返回刷新 cy 191216
					case 'scientificInput':
						tdData.value.real = resultData.value;
						tdData.value.visible = NetstarUI.resultTable.getScientificValue(tdData.subdata, resultData.value);
						break;
					case 'date':
						tdData.value.real = resultData.value;
...
//获取当前选中的数据，批注新建调用方法
NetstarUI.resultTable.getCurrentData = function(containerId){
	var $container = $('#resultTableContainer-inputcomponent');
	if(containerId){
		$container = $("#"+containerId+'-inputcomponent');
	}
	if($container.length == 0){
		return false;
	}else{
		var isEditState = $container.css('display')=='block';
...
//获取仪器数据
NetstarUI.resultTable.cloudInstrumentObject = function() {
    var _scanInstrumentId = "";

    var eventHandleObject = function () {
        var _handles = new Array();

        function findHandle(handle) {
            for (var i = 0, len = _handles.length; i < len; i += 1) {
        ...
        this.registerHandle = function (handle, func) {
            if (typeof handle === "function") {
                var idx = findHandle(handle);
                if (typeof idx === "undefined") {
                    if (typeof func === "function") {
                        func(handle);
                    }
                    if (typeof _handles === "undefined") {
                        _handles = [handle];
                    } else {
        ...
        this.unRegisterHandle = function (handle) {
            if (_handles instanceof Array) {
                if (typeof handle === "function") {
                    var idx = findHandle(handle);
                    if (idx) {
                        _handles.splice(idx, 1);
                    }
                }
            }
            return this;
        ...
        this.fireHandle = function (events, filter) {
            for (var len = _handles.length, start = 0; start < len; start += 1) {
                if (typeof filter === "function") {
                    var e = filter(_handles[start], events);
                    if (typeof e != "undefined") {
                        _handles[start].call(this, e);
                    }
                } else {
                    _handles[start].call(this, events);
                }
            }
        }
    ...
    var webSocketManagerObject = function () {
        ...
        var _eventHandle = new eventHandleObject();

        this.registerHandle = function (handle, type, obj, filter) {

            _eventHandle.registerHandle(handle, function (handle) {
                if (type) {
                    handle.type = type;
                }
                if (filter) {
            ...
            var msg = {action: type + "Reg", obj: obj};
        ...
        this.unRegisterHandle = function (handle) {
            if (typeof handle === "function") {
                if (typeof handle.type != "undefined") {
                    var msg = {action: handle.type + "UnReg"};
                    _webSocket.send(msg);
                }
                _eventHandle.unRegisterHandle(handle);
            }
            return this;
        }

        var onmessage = function (data) {
            _eventHandle.fireHandle(data, function (handle, data) {
                if (handle.type) {
                    if (handle.type == data.type) {
                        if (handle.filter) {
                            return handle.filter(handle, data);
                        } else {
                            return data;
                        }
                    } else {
    ...
    var webSocketManager = new webSocketManagerObject();

    var _registerHandle = function (handle, instrumentId) {
        // 注册仪器数据接收
        handle.instrumentId = instrumentId;
        webSocketManager
            .registerHandle(
                handle,
                'instrumentData',
                ...
                function (handle, data) {
                    var instrumentDatas = JSON.parse(data.body);
                    var datas = new Array();
                    var j = 0;
                    for (var i = 0; i < instrumentDatas.length; i++) {
                        if (handle.instrumentId == instrumentDatas[i].instrumentId) {
                            datas[j] = instrumentDatas[i];
                            j++;
                        }
                    }
                    return datas;
    ...
    var _registerSyncFileHandle = function (handle, instrumentId) {
        // 注册仪器文件接收
        handle.instrumentId = instrumentId;
        webSocketManager
            .registerHandle(
                ...
                instrumentId,
                function (handle, data) {
                    var instrumentFile = JSON.parse(data.body);
                    var datas = undefined;
                    if (handle.instrumentId == instrumentFile.instrumentId) {
                        datas = JSON.parse(instrumentFile.json);
                    }
                    return datas;
                });
        return this;
    ...
    var _registerInstrument = function (instrumentId) {
        // 注册试验数据接收事件
        _registerHandle(onInstrumentData, instrumentId);
        //注册仪器文件接收事件
        _registerSyncFileHandle(onInstrumentFile, instrumentId);
    }

    var _doScanCode = function (barcode) {
        if (barcode.indexOf('YP') == 0) {
            var sampleId = Number(barcode.substr(2));
            // 设置试验序号
            NetstarUI.resultTable.setSampleOrderInterface({
                sampleId: sampleId
            });
            // 载入试验数据
        }
        if (barcode.indexOf('YQ') == 0) {
            _scanInstrumentId = Number(barcode.substr(2));
            _registerInstrument(_scanInstrumentId);
        }
    }

    this.registerHandle = _registerHandle;

    this.registerSyncFileHandle = _registerSyncFileHandle;

    this.unRegisterHandle = function (handle) {
        webSocketManager.unRegisterHandle(handle);
        return this;
    }

    this.unRegisterSyncFileHandle = function (handle) {
        webSocketManager.unRegisterHandle(handle);
        return this;
    }

    this.registerInstrument = _registerInstrument;

    this.scanCode = function () {
        //this.selectInstrument();
        //scanCodeManager.scanCode(_doScanCode);
        var code = window.prompt("请输入条码");
        if (code) {
            _doScanCode(code);
        }
    }

    this.selectInstrument = function () {
        var config = {
            id: "cloudInstrument-selectInstrument-view",
            title: "选择仪器",
            size: "s",
            form: [{
                id: 'selectInstrument',
                ...
                valueField: 'id',
                isServiceMode: true,
                url: getRootPath() + '/netStarRights/equipmentController-getEquipmentList',
                method: 'post',
                data: {page: 1},
                dataSrc: 'rows',
                requestParams: 'eNameOreCode',
                placeholder: '请输入仪器编号或名称'
            }],
            btns: [{
                text: language.messager.determine,
                handler: function () {
                    var json = formPlane.getFormJSON('cloudInstrument-selectInstrument-view');
                    if (json) {
                        nsdialog.hide();
                        var instrumentId = json.selectInstrument;
                        _registerInstrument(instrumentId);
                    }
                }
            }]
        };
    ...
    this.selectInstrumentData = function (useCurrentInstrument) {
        var tableId = "table-frame-resultinput-layout-resultinputTable";
        var obj = baseDataTable.getSingleRowSelectedData(tableId);
        if (useCurrentInstrument) {
            $.ajax({
                url: getRootPath() + '/useRecordController/getRecordListByRecordId',
                type: "POST",
                dataType: 'json',
                data: {
                    recordId: obj.testGroupId,
                    type: 1
                },
                success: function (data) {
                    if (data.success) {
                        if (data.rows.length > 0) {
                            ...
                            for (var i = 0; i < data.rows.length; i++) {
                                instrumentIds += data.rows[i].articlesId + ',';
                            }
                            instrumentIds = instrumentIds.substr(0, instrumentIds.length - 1);
                            console.log(instrumentIds);
                            var config = {
                                url: getRootPath() + '/cdevInstrumentData/dialog?instrumentIds=' + instrumentIds,
                                width: '70%',
            ...
            var config = {
                url: getRootPath()
                + '/cdevInstrumentData/dialog?instrumentId='
                + _scanInstrumentId + '&taskId=' + obj.taskId,
                width: '70%',
                // loadedHandler: loadedHandler,
                // closeHandler: ,
                title: '选择仪器数据'
    ...
    function onInstrumentData(data) {
        try {
            var indata = [];
            for (var i = 0; i < data.length; i++) {
                data[i].itemValues = JSON.parse(data[i].itemValue);
                data[i].sampleId = undefined;
                data[i].testNo = undefined;

                //将仪器数据对应到样品
                var sampleNo = data[i].sampleNo;
                if (sampleNo) {
                    if (typeof(NetstarUI.resultTable.data.tdDataBySampleCode[sampleNo]) != 'undefined') {

                        var datas = NetstarUI.resultTable.data.tdDataBySampleCode[sampleNo];
                        var c = 0;
                        var sampleId = undefined;
                        for (sampleIdIndex in datas) {
                            if (c > 0) {
                                //当样品编号在当前表格中是唯一的，才按样品编号赋值
                                sampleId = undefined;
                                break;
                            }
                            sampleId = sampleIdIndex;
                            c++;
                        }
                        if (sampleId) {
                            data[i].sampleId = sampleId;
                            //找到样品编号的才算做有效数据
                            indata.push(data[i]);
                        }
                    }
            ...
            // 调用结果录入表格填充数据方法
            NetstarUI.resultTable.setSampleDataInterface(indata);
        } catch (err) {
            console.debug(err);
        }
    }

    function onInstrumentFile(data) {
        //console.debug(data);
        var _row = baseDataTable.getSingleRowSelectedData("table-frame-resultinput-layout-resultinputTable");
        if (!_row) {
            return;
        }
        var _recordId = _row.testGroupId;
        var _fileName = data[0].realName;
        $.ajax({
            url: getRootPath() + '/Attachment/save',
            type: "POST",
            dataType: 'json',
            data: {
                fileName: _fileName,
                filePath: data[0].realpath,
                fileSize: data[0].size,
                fileType: data[0].suffix,
                fileContentType: data[0].contentType,
                isUse: 1
            },
            success: function (data) {
                var _fileId = data.id;

                var _url = getRootPath() + '/ptTestGroupFileController/savePtTestGroupFile';
                $.ajax({
                    async: true,
                    url: _url,
                    data: {recordId: _recordId, fileId: _fileId},
                    type: "post",
                    dataType: "json",
                    success: function (data) {
                        if (data.success && data) {
                            nsalert('上传成功');
                            baseDataTable.reloadTableAJAX("form-plane-viewState-sqfj-upload")
                        } else {
                            nsalert(data.msg);
                        }
                    }
    ...
    function doOverrideResultTableVaildData() {
        // override
        // 覆盖结果录入表格的方法,实现数据的格式转换
        // NetstarUI.resultTable.vaildData(NetstarUI.resultTable.data);
        // 保存原方法,
        //debugger;
        window._NetstarUI_resultTable_vaildData = window._NetstarUI_resultTable_vaildData
            || NetstarUI.resultTable.vaildData;
        // 覆盖验证方法
        NetstarUI.resultTable.vaildData = function (data) {
            // 先调用原来的验证方法
            window._NetstarUI_resultTable_vaildData(data);

            data.tdDataBySampleCode = {}; //根据样品编号组织的单元格数据

            // 数据格式转化
            for (tableID in data.tableData) {
                for (tr in data.tableData[tableID].rows) {
                    for (td in data.tableData[tableID].rows[tr]) {
                        if (data.tableData[tableID].rows[tr][td].value) {
                            if (data.tableData[tableID].rows[tr][td].value.data) {
                                var tddata = data.tableData[tableID].rows[tr][td].value.data;
                                var nodeName1 = tddata.nodeName;
                                if (nodeName1 && nodeName1 != '') {
                                    var nodePath = tddata.nodePath;
                                    if (nodePath && nodePath != '') {
                                        nodeName1 = nodePath;
                                    }
                                    var itemName = tddata.itemName;
                                    if (itemName && itemName != '') {
                                        //去掉平行样
                                        var pIndex = nodeName1.indexOf('平行样');
                                        if (pIndex == 0) {
                                            var dotIndex = nodeName1.indexOf('.');
                                            nodeName1 = nodeName1.substring(dotIndex + 1);
                                        }
                                        nodeName1 = itemName + "."
                                            + nodeName1;
                                    }
                                    data.tableData[tableID].rows[tr][td].value.data.nodeName = nodeName1;
                                }

                                //根据样品编号组织单元格
                                var sampleId = data.tableData[tableID].rows[tr][td].value.data.sampleId;
                                var sampleCode = data.tableData[tableID].rows[tr][td].value.data.sampleCode;
                                if (sampleCode) {
                                    if (typeof(data.tdDataBySampleCode[sampleCode]) == 'undefined') {
                                        data.tdDataBySampleCode[sampleCode] = {};
                                    }
                                    if (typeof(data.tdDataBySampleCode[sampleCode][sampleId]) == 'undefined') {
                                        data.tdDataBySampleCode[sampleCode][sampleId] = [];
                                    }
                                    data.tdDataBySampleCode[sampleCode][sampleId].push(tddata);
                                }
                            }
                        }
                    }
    ...
    doOverrideResultTableVaildData();
}
