...
NetstarComponent.common = {
    ...
    //     + '</div>',
    form:  
        '<div class="panel panel-default panel-form" :class="[formStyle,formLayout,plusClass]" :style="formInitStyle">'
            + '<div class="pt-form-header"></div>'
            + '<div class="pt-form-body" :id="id">'
                + '{{nscontainer}}'
            + '</div>'
...
NetstarComponent.formComponent = {
    ...
    getHtml: function(config){
        ...
        componentsContent = '<form class="clearfix  standard compactmode ' + config.formSource.toLowerCase() + '" novalidate="true" autocomplete="off" onkeydown="if(event.keyCode==13){return false;}" onsubmit="return false">' + componentsContent + '</form>';
...
NetstarComponent.number = {
    ...
    TEMPLATE:{
        ...
        MOBILE: '<div class="mobile-form-control" :class="inputClass" :id="id" @click="calculator">{{inputText}}</div>'
    ...
    getHtml: function(config){
        ...
        var tempalte = this.TEMPLATE[templateName];
        var contentHtml = '';
        var containerType = 'formComponent';
        var containerHtml = NetstarComponent.common[containerType]; // 容器模板
        switch(config.formSource){
            case 'staticData':
                contentHtml = NetstarComponent.getStaticTemplate(config);
                break;
            default:
                contentHtml = tempalte;
                break;
        }
        containerHtml = containerHtml.replace('{{nscontainer}}',contentHtml);
        return containerHtml;
        ...
        var component = {
            ...
            methods: {
                ...
                calculatorShow: function(){
                    ...
                    $calculator.children('.calculator-keyentry').attr('disabled', true);
                },
                // 计算器销毁
            ...
            mounted : function(){
                ...
                if(config.formSource!="staticData" && !config.disabled && config.isAlwaysShow){
                    this.calculatorShow();
                }
            },
        }
...
NetstarComponent.listSelectInput = {
	...
	setDefault: function(config){
        var defaultConfig = {
            ...
            hidden:             false,          // 是否隐藏
            // 列表配置参数
            idField :           'propertyId',           // id字段
            idValField :        'propertyValue',      // id字段显示的value
            labelField :        'propertyName',        // id字段显示的label
        ...
        for(var i=0; i<list.length; i++){
            ...
            var idComponent = {
                ...
                changeHandler : function(obj){
                    ...
                    var relationId = thisConfig.plusConfig.data[thisConfig.id + '-name'];
                    var edit = {
                        id : relationId,
                        value : obj.text,
                    }
...
NetstarComponent.selectInput = {
    ...
    TEMPLATE:{
        PC:{
            panel:  '<input type="text" class="pt-form-control" :class="inputClass" :disabled="disabled" v-model="inputText" :id="id" @focus="focusHandler" @blur="blurHandler" @change="change" @keyup="keyup" @keydown="keydown">'
                    + '<div class="pt-input-group-btn">'
