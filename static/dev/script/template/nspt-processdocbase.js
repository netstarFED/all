...
NetstarTemplate.templates.processDocBase = (function ($) {
   ...
   function init(outerConfig, _componentConfig) {
      NetstarTemplate.commonFunc.setDefault(outerConfig);
      ...
      setDefaultData(function () {
         ...
         //sjj 2019119 绑定扫码事件
         if(!$.isEmptyObject(config.scancode)){
            var isUseQRInput = typeof(config.scancode.isUseQRInput)=='boolean' ? config.scancode.isUseQRInput : false;
            var recordScanArray = [];
            var recordScanCount = {};
            if(isUseQRInput){
               //使用扫码功能
               var scancodeAjax = config.scancode.ajax;
               var scancodeConfig = {
                  id:'scancode-input-'+config.containerHeaderId,
                  callBackFunc:function(codeStr){
                     ...
                     if(isExistIndex == -1){
                        ...
                        //已经扫描过的
                        var gridArr = NetStarGrid.dataManager.getData(config.mainGrid.id);
                        ...
                        if(rowIndex > -1){
                           ...
                           NetStarGrid.editRow(rowData,config.mainGrid.id);
                        }
                        //nsalert('该条形码已扫过，不能重复添加','warning');
                     }
                  }
               };
               $('#'+config.containerHeaderId).prepend('<input type="hidden" id="'+scancodeConfig.id+'" />');
                              ...
                              return function(){
                                 NetstarTemplate.draft.btnManager.save(_config, function(obj){
                                    ...
                                    if(pageConfig.draftBox.closeOrClear){
                                       NetstarTemplate.templates.processDocBase.clearByAll(pageConfig);
                                    }
                                 });
                              }
                           })(config),
   ...
   function formatForm(formConfig) {
      ...
      //判断是否定义了readonlyExpression  sjj 20191202
      var isReadonly = false;
      var isReadonlyExpression = false;
      if(formConfig.readonlyExpression){
         isReadonlyExpression = true;
         isReadonly = NetstarTemplate.commonFunc.getBooleanValueByExpression(config.serverData,formConfig.readonlyExpression);
      }
      //设置只读 值
   ...
   function getGridConfig(pageListOptions) {
      ...
      //判断是否定义了readonlyExpression  sjj 20191202
      if(options.readonlyExpression){
         var isEditMode = NetstarTemplate.commonFunc.getBooleanValueByExpression(config.serverData,options.readonlyExpression);
         gridConfig.ui.isEditMode = !isEditMode;
      }

      if(!$.isEmptyObject(options.params)){
         $.each(options.params,function(key,value){
            gridConfig.ui[key] = value;
         });
      }
      var componentConfig = {
         config: gridConfig,
         ...
         dialogBeforeHandler: (function (config) {
            return function (data) {
              ...
              //data.controllerObj.parameterFormat
               ...
               controllerObj.ajaxData = typeof(controllerObj.ajaxData) == 'object' ? controllerObj.ajaxData : {};
               controllerObj.data = typeof(controllerObj.data) == 'object' ? controllerObj.data : {};
               ...
               if(!isParmeterFormat){
                              ...
                              if(config.mainGrid){
                                 if(config.mainGrid.id){
                                    ...
                                    if(selectData.length > 0){
                                       pageData[config.mainGrid.keyField] = selectData;
                                    }
                                 }
                              }
                           }
                  ...
                  data.value = pageData;
                  if (typeof controllerObj.requestSource != 'undefined') {
                     if(config.mainGrid){
                        if(config.mainGrid.id){
                           var selectData = NetStarGrid.getSelectedData(config.mainGrid.id);
                           switch (controllerObj.requestSource) {
                              case 'selected':
                                 //vo
                     ...
                     setTimeout(function(){
                        ...
                        if (typeof config.parentSourceParam != 'undefined' && !$.isEmptyObject(config.parentSourceParam)) {
                           ...
                           NetstarTemplate.refreshByPackage(config.parentSourceParam, data,plusData);
                            ...
                            if(currentConfig && currentConfig.pageParam && currentConfig.pageParam.prevPagePackageName){
                                var parentPackageName = currentConfig.pageParam.prevPagePackageName;
         ...
         if (typeof item.functionConfig == 'object' && $.inArray(item.functionConfig.defaultMode, defaultModeArr) != -1) {
            ...
            btnsArray[i].handler = function (data) {
              // var packageName = $(data.event.currentTarget).closest('.btn-group').attr('ns-tempalte-package');
               var packageName = $('#'+data.data.id).closest('.btn-group').attr('ns-tempalte-package');
      ...
      $.each(config.components, function (index, item) {
         ...
         switch (item.type) {
            ...
            case 'btns':
               ...
               $currentPosition = $('#' + config.containerHeaderId);
               if($currentPosition.children('.pt-page-title').length == 1){
                  $currentPosition.children('.pt-page-title').after($currentContainer);
               }else{
                  $currentPosition.prepend($currentContainer);
               }
               break;
            case 'tab':
               break;
   ...
   function renderComponents(_config, cb) {
      ...
      for (var index = 0, len = config.formatComponentsArr.length; index < len; index++) {
         ...
         switch (item.renderType) {
            case 'form':
               ...
               item.config.completeHandler = (function (_formValue, formId) {
                  return function(){
                     cb && cb();
               ...
               break;
            case 'grid':
               gridComponentsArr.push(item);
               break;
            case 'btns':
               if(config.isUseBtnPanelManager){
      ...
      if(gridComponentsArr.length > 0){
         setTimeout(function(){
            var commonHeight = 0;
            var listNum = 0;
            var tabField = '';
            for(var c=0; c<config.components.length; c++){
               var cData = config.components[c];
               switch(cData.type){
                  case 'tab':
                     tabField = cData.field;
                     listNum ++;
                     break;
               }
            }
            for(var c=0; c<config.components.length; c++){
               ...
               switch(cData.type){
                  case 'vo':
                  case 'btns':
                     var $component = $('#'+cData.id);
                     var $panel = $component.closest('.pt-panel');
                     var outerHeight = $component.outerHeight();
                     if($panel){
                        var panelHeight = $panel.height();
                        var marginTop = $panel.css('margin-top');
                        var marginBottom = $panel.css('margin-bottom');
                        var topNum = Number(marginTop.substring(0, marginTop.length-2));
                        var bottomNum = Number(marginBottom.substring(0, marginBottom.length-2));
                        outerHeight = panelHeight + topNum + bottomNum;
                     }
                     commonHeight += outerHeight;
                     break;
                  case 'list':
                     // 如果tab里没有才可以增加
                     var keyField = cData.keyField;
                     if(tabField.indexOf(keyField) == -1){
                        listNum++;
                     }
                     break;
               }
            }
            var gHeight = $(window).outerHeight() - 35 - commonHeight - 40 - 20; // 20是模板容器的padding
            if(listNum > 1){
               gHeight -= 20;
            }
            if(config.title){
            ...
            for(var g=0; g<gridComponentsArr.length; g++){
               var item = gridComponentsArr[g];
               ...
               gridConfig.ui.height = parseFloat(gHeight/listNum);
               console.log(gridConfig.ui.height)
               gridConfig.ui.pageLengthDefault = Math.floor((gHeight/listNum)/29);
               gridConfig.ui.minPageLength = Math.floor((gHeight/listNum)/29);
               ...
               if(!$.isEmptyObject(config.serverData)){
                  if($.isArray(config.serverData.modifyDiffsList)){
                     NetstarComponent.setHistoryByTableID(config.serverData.modifyDiffsList, config.serverData[item.templateOptions.keyField], item.templateOptions.idField, item.config.id);
                  }
               }
            }
         },10)
   ...
   function setDefaultData(cb) {
      ...
      /**********************sjj 从上一个界面配置的ajaxNewtabCommon获取值 */
      var templateDataByAjax = {};
      if(!$.isEmptyObject(config.pageParam)){
         if(config.pageParam.templateDataByAjax){
            templateDataByAjax = config.pageParam.templateDataByAjax;
         }
      }
      if(!$.isEmptyObject(templateDataByAjax)){
         config.serverData = templateDataByAjax;
         config.defaultData = templateDataByAjax;
         delete config.pageParam.templateDataByAjax;
   ...
   function verifyGridData(gridData, config, gridConfig) {
      ...
      var isPass = true;
      for(var fieldI=0; fieldI<columns.length; fieldI++){
         var fieldConfig = columns[fieldI];
         var fieldObj = fieldConfig.editConfig ? fieldConfig.editConfig : {};
         if(fieldObj.rules){
            for(var listValI=0; listValI<gridData.length; listValI++){
               var valStr = gridData[listValI][fieldConfig.field];
               if(typeof(valStr) == "undefined"){ valStr = ''; }
               fieldObj.relationData = {
                  row : gridData[listValI],
               }
               fieldObj.formSource = 'table';
               fieldObj.formID = mainGridConfig.id;
               var passObj = NetstarComponent[fieldObj.type].validatValue(valStr, fieldObj.rules, 'object', fieldObj);
               if(!passObj.isTrue){
                  isPass = false;
                  nsalert(fieldObj.label+passObj.validatInfo, 'warning');
                  console.log(fieldObj.label+passObj.validatInfo);
                  break;
               }
            }
         }
   ...
   function getGridHeight(_config) {
      ...
      if(componentLength.grid > 1 &&  componentLength.form == 0){
         ...
         var allGridHeight = availableHeight - usedHeight.btns - usedHeight.form;
         gridHeight = allGridHeight / componentLength.grid;
         //gridHeight = gridHeight - 10;
      }else if(componentLength.grid > 1 &&  componentLength.form > 1){
         //有多个表格和表单 sjj 20191211
         gridHeight = (availableHeight - usedHeight.btns - usedHeight.form-160)/2;
      }else{
         gridHeight = availableHeight - usedHeight.btns - usedHeight.form-160;  //160是个临时的数据，用于替代掉表单的可能高度
      }
   ...
   return {
      ...
      clearByAll:function(_config){
         ...
         for(var c=0; c<componentsArr.length; c++){
            var componentData = componentsArr[c];
            var componentId = componentData.id;
            switch(componentData.type){
               case 'vo':
                  NetstarComponent.clearValues(componentId,false);
                  break;
               case 'list':
