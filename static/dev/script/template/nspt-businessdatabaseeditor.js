   ...
   var dialogForAdd = function (outerConfig) {
      ...
      var imgUpload = {
         ...
         uploadFiles: function (files) {
            ...
            formData.append('categories',['image:0.1']);//0.1|2
      ...
      function formatForm(formConfig, formName) {
         ...
         // 通过隐藏表达式设置字段显示隐藏 lyw 20200313
         var hiddenExpression = formConfig.hiddenExpression;
         var hiddenObjByHiddenExpression = {};
         if(typeof(hiddenExpression) == "string" && hiddenExpression.length > 0){
            var hiddenExpressionObj = JSON.parse(hiddenExpression);
            for(var hideFieldKey in hiddenExpressionObj){
               hiddenObjByHiddenExpression[hideFieldKey] = NetstarTemplate.commonFunc.getBooleanValueByExpression(formValue, hiddenExpressionObj[hideFieldKey]);
            }
         }
         ...
         $.each(formConfig.field, function (index, item) {
            ...
            if(typeof(hiddenObjByHiddenExpression[item.id]) == "boolean"){
               item.hidden = hiddenObjByHiddenExpression[item.id]
            }
            //设置自动聚焦
         ...
         var dialogJson = {
            ...
            height:'auto',
               ...
               if(!isShowNav){
		         ...
		         //header和body分别插入
               var headerHtml = 
                  '<div class="pt-tab-header" v-if="isShowNav">\
                     <div class="pt-nav">\
                        <ul>\
                           <li class="pt-nav-item" v-for="(item, index) in config.tabIdFields" :key="item" :id="item" :class="{\'current\' : index == config.showTab}" @click="tabClick(item,index)">\
               ...
               if(!$.isEmptyObject(config.params)){
                  if(config.params.tipContent){
                     var tipClassStr = config.params.tipClass;
                     tipContentHtml.prepend('<div class="tip-content"><span class="'+tipClassStr+'">'+config.params.tipContent+'</span></div>');
                  }
               }

               var bodyHtml = tipContentHtml+'<div class="pt-tab">\
                              <div class="pt-container">\
                        ...
                        btns: [
                           {
                              ...
                              handler: function () {
                                 ...
                                 if(config.$container){
                                    ...
                                    if(NetstarComponent.dialog[config.vueId]){
                                       NetstarComponent.dialog[config.vueId].vueConfig.close(false);
                                    }
                                 }
                                 ...
                                 if (pageData && typeof isNotVerify == 'boolean' && isNotVerify) {
                                    ...
                                    //解绑当前显示的快捷键
                                    vueButtonComponent.unbindKeydownHandler(config.package);
                                    var packageName = '';
                                    if(NetstarUI.labelpageVm.labelPagesArr[NetstarUI.labelpageVm.currentTab]){
                                       if(NetstarUI.labelpageVm.labelPagesArr[NetstarUI.labelpageVm.currentTab].config){
                                       ...
                                       if($('container:last').length > 0){
                                          if($('container:last').children('.businessdatabase').length > 0){
                                             var templateId = $('container:last').children('.businessdatabase').attr('id');
                                             packageName = NetstarTemplate.templates.businessDataBase.data[templateId].config.package;
                                          }
                                       }
                                    }
                                    if(packageName){
               ...
               save: function ($btn) {
                  ...
                  if (!!pageData && typeof saveDataConfig != 'undefined' && typeof saveDataConfig.ajax != 'undefined') {
                     ...
                     if(!$.isEmptyObject(saveOriginalParams)){
                        ...
                        for (var key in saveOriginalParams) {
                           if (ajaxParameterRegExp.test(saveOriginalParams[key])) {
                              isUseObject = false;
                              break;
                           } else {
                              saveDataConfig.ajax.data[key] = saveOriginalParams[key];
                           }
                        }
            ...
            switch (item.type) {
               case 'vo':
                  ...
                  for(var voField in voComponentConfig){
                     if(voComponentConfig[voField].hidden == false){
      ...
      function verifyGridData(gridId, config, index) {
         ...
         if(gridData.length == 0 && !$.isEmptyObject(verifyRuleObj)){
            verifyResult = true;//sjj 20191202
      ...
      function setDataByKeyField(keyField, data, pageData, config) {
         ...
         var currentRelation = config.parentChildRelation[keyField]? config.parentChildRelation[keyField] : '';
         if (currentRelation == 'root') {
            for (var key in data) {
