...
pageProperty.init = function(pageParams, callbackFunc){
	...
	function formatData(voSourceJSON, isDetails){
		...
		var cachePageConfig = $.extend(true, {}, pageParams.pageConfig);
		// for(var key in pageParams.pageConfig){
		// 	if(typeof(pageParams.pageConfig[key]) == "object" && key != "components"){
		// 		cachePageConfig[key] = $.extend(true, {}, pageParams.pageConfig[key]);
		// 	}else{
		// 		cachePageConfig[key] = pageParams.pageConfig[key];
		// 	}
		// }
...
pageProperty.getPageHtml = function(resParams){
	...
	var pageParams = {
		...
		// 页面标识用于记录缓存pageConfig
		pageIidenti : resParams.pageIidenti,
	}
...
pageProperty.getPageHtmlNew = function(resParams){
	var pageParams = {
		pageConfig:resParams.pageConfig,
		functionPointObj:resParams.functionPointObj, 	//菜单功能点矩阵参数
		templateParam:resParams.templateParam, 	//弹出页面时当前页面参数
		data_auth_code:resParams.data_auth_code,
		pageIidenti : resParams.pageIidenti,
	}
	var pageParamsStr = JSON.stringify(pageParams);
	var html = '<container>\r\n'
					+ '<script type="text/javascript">\r\n'
						+ '$(function(){\r\n'
							+ 'var pageParams = ' + pageParamsStr + '\r\n'
							+ 'var pageConfig = NetstarProject.getFormatPageConfig(pageParams);\r\n'
							+ 'if(pageConfig){\r\n'
...
// 通过模板的config获取html
pageProperty.getPageHtmlByPageConfig = function(cacheData, type){
	var templateParam = '';
	var pageIidenti = cacheData.pageIidenti;
	var debuggerHtml = '';
	var pageHtml = '';
	switch(type){
		case 'pageConfig':
			templateParam = cacheData.paramObj;
			debuggerHtml = cacheData.pageParams ? cacheData.pageParams : '';
			pageHtml = 'var pageConfig = {\r\n'
							+ 'config:config,\r\n'
						+ '}\r\n'
			break;
		case 'getById':
			templateParam = cacheData.templateParam;
			break;
	}
	var pageParams = {
		// pageConfig:cacheData.pageConfig,
		functionPointObj:cacheData.functionPointObj, 	//菜单功能点矩阵参数
		templateParam:templateParam, 	//弹出页面时当前页面参数
		data_auth_code:cacheData.data_auth_code,
	...
	var html = '<container>\r\n'
					+ '<script type="text/javascript">\r\n'
						+ '$(function(){\r\n'
							+ 'var pageParams = ' + pageParamsStr + '\r\n'
							+ 'var config = pageProperty.getPageConfigByCache(pageParams);\r\n'
							+ 'if(config){\r\n'
								+ pageHtml
								+ debuggerHtml
								+ '\r\n'
...
pageProperty.getPageConfigByCache = function(pageParams){
	var pageIidenti = pageParams.pageIidenti;
	var templateParam = pageParams.templateParam;
	var cachePageData = pageProperty.cachePageData;
	var cacheData = cachePageData[pageIidenti];
	var pageConfig = $.extend(true, {}, cacheData.pageConfig);
	var templateParamJson = {};
	if(templateParam){
		//sjj 20190327 读取界面来源参
		var tempValueName = pageParams.templateParam;
		if(NetstarTempValues[tempValueName]){
			templateParamJson = NetstarTempValues[tempValueName];
			delete NetstarTempValues[tempValueName];
		}else{
			var tempValueNameDecode = decodeURIComponent(decodeURIComponent(tempValueName));
			templateParamJson = JSON.parse(tempValueNameDecode);
		}
	}
	pageConfig.pageParam = typeof(pageConfig.pageParam) == "object" ? pageConfig.pageParam : {};
	// 矩阵参数
	var functionPointObj = pageParams.functionPointObj
	if(typeof(functionPointObj) == "string" && functionPointObj.length > 0){
		functionPointObj = JSON.parse(functionPointObj);
	}
	if(!$.isEmptyObject(functionPointObj)){
		// pageConfig.matrixVars = $.extend(true, {}, functionPointObj);
		pageConfig.pageParam = $.extend(false, pageConfig.pageParam, functionPointObj);
	}
	// 页面之间传参
	if(!$.isEmptyObject(templateParamJson)){
		pageConfig.pageParam = $.extend(false, pageConfig.pageParam, templateParamJson);
	}
	// 判断getValueAjax是否存在
	function isOnlyCodeFun(obj){
		var isOnlyCode = true;
	...
	var isOnlyCode = isOnlyCodeFun(pageConfig.pageParam);
	if($.isEmptyObject(pageConfig.pageParam) || isOnlyCode){
	...
	if(data_auth_code == ''){
		if(templateParamJson){
			if(typeof(templateParamJson.data_auth_code)=='string' && templateParamJson.data_auth_code != ''){
				data_auth_code = templateParamJson.data_auth_code;
			}
		}
	}
	// pageConfig.data_auth_code = data_auth_code;
	pageConfig.pageParam.data_auth_code = data_auth_code;
	// 处理getValueAjaxpage
	if(typeof(pageConfig.getValueAjax)=="object"){
		if(typeof(pageConfig.getValueAjax.suffix)=="string"){
			pageConfig.getValueAjax.url = pageConfig.getValueAjax.suffix;
		}
	}
	pageConfig.pageInitCompleteHandler = function(config){
		var obj = {
			name : config.package,
			processId : config.pageParam.processId,
			activityId : config.pageParam.activityId,
...
pageProperty.getAndCachePage = function(pageConfig){
	...
	if(isHaveCache){
		if(typeof(pageConfig.callBackFunc)=="function"){
			...
			var cacheType = '';
			if(pageConfig.url.indexOf('getById') > -1){
				// var pageHtml = pageProperty.getPageHtmlNew(pageData);
				cacheType = 'getById';
			}else{
	...
	if(pageConfig.url.indexOf('getById') > -1){
        ...
        //     pageId = pageConfig.url ? pageConfig.url : '';
        //     if(pageId.indexOf('?') > -1){
        ...
        NetStarUtils.ajax(ajaxConfig, function(res, _ajaxConfig){
            ...
            if(!res.success){
                if(typeof(plusData.callBackFunc) == "function"){
                    plusData.callBackFunc(false, res, _pageConfig);
				}
				return false;
			}
			var pageParams = getStrParas(_ajaxConfig.plusData.url);

			var pageId = res.data.id;
			var cacheObj = {
				isUpdate : false,
				url : _pageConfig.url,
			...
			NetstarEditorServer.getPageConfigByData([res.data], (function(resData, pageParams, pageIidenti){
				return function(resPageConfig){
					...
					// }
					resPageConfig.pageParam = typeof(resPageConfig.pageParam) == "object" ? resPageConfig.pageParam : {};
					for(var key in pageParams){
						if(key !== ''){
							resPageConfig.pageParam[key] = pageParams[key];
						}
					}
					var resParams = resData;
					resParams.pageConfig = resPageConfig;
					cacheObj.pageData = resParams;
					resParams.pageIidenti = pageIidenti;
					var pageHtml = pageProperty.getPageHtmlNew(resParams);
					if(typeof(_pageConfig.callBackFunc) == "function"){
						_pageConfig.callBackFunc(true, pageHtml, _pageConfig);
					}
				}
			})(res.data, pageParams, _pageIidenti));
        })
		return;
	}
	// 不存在缓存读取缓存数据
		...
		success: function(data){
			...
			if(typeof(data)=="object"){
				if(data.success){
					...
					resData.pageIidenti = _pageIidenti;
					var pageHtml = pageProperty.getPageHtml(resData);
					pageId = data.data.pageId;
					cacheObj.pageId = pageId;
					cacheObj.pageData = resData;
				}else{
					if(typeof(_pageConfig.callBackFunc) == "function"){
