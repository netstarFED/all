...
********/
/******************** 表单表格模板 start ***********************/
NetstarTemplate.templates.detailsDocBase = (function(){
	var config = {};//当前配置参数
	...
	function getContainerHtml(){
		...
		var contentHtml = '';
		var voHtml = '<div class="detailsdocbase-content-vo">';
		var listHtml = '';
		for(var componentI=0; componentI<components.length; componentI++){
			...
			switch(componentData.type){
				case 'vo':
					...
					voHtml += '<div class="'+classStr+'" id="'+containerId+'"></div>';
					break;
				case 'list':
					classStr += ' content-list-card';
					...
					if(isFilterList){
						...
						voHtml += '<div class="filter-form-contidion" id="'+containerId+'-ul" ns-containerid="'+containerId+'">'
										+'</div>';
						...
						filterListDataObject[containerId] = {
							...
							titleJson:titleJson,
							filterId:containerId+'-ul'
						};
						voDataArray.push({
							id:containerId+'-ul',
							//formSource:'halfScreen',
							form:[{
								id:filterFieldConfig.classField,
								type:'select',
								subdata:filterArray,
								commonChangeHandler:filterListClickHandler
							}]
						});
					}else{
						contentHtml += '<div class="'+classStr+'" id="'+containerId+'"></div>';
		...
		var html = '<div class="template-detailsdocbase" id="'+config.id+'">'+voHtml+contentHtml+btnHtml+'</div>';
		return html;
	}//得到容器html
	...
	function filterListClickHandler(data){
		var key = data.value;
		var containerId = data.dom.closest('.filter-form-contidion').attr('ns-containerid');
	...
	function filterListDataInit(){
		if(filterListDataArray.length > 0){
			...
			for(var listI=0; listI<filterListDataArray.length; listI++){
				...
				filterListDataComponentInit(0,containerId);
			}
		}
	}//带列表过滤的初始化
	function voDataInit(){
	...
	function init(_config){
		//第一次执行初始化模板
		if(typeof(NetstarTemplate.templates.detailsDocBase.data)=='undefined'){
			NetstarTemplate.templates.detailsDocBase.data = {};  
			/* 保存在该对象上的数据分为四个：
			 	* config(运行时参数)，
			 	* original（原始配置参数）
			*/
		...
		config = _config;
		//记录config
		NetstarTemplate.templates.detailsDocBase.data[config.id] = {
