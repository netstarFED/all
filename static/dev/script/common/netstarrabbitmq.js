 ...
 * @LastEditTime: 2019-11-23 10:46:30
 */
...
var NetStarRabbitMQ = (function(){
    ...
    function subscribe(subscribeConfig){
        ...
        for(var key in NetStarRabbitMQ.content){
            ...
            if(typeof(subscribeConfig.content[key]) == "undefined"){
                subscribeConfig.content[key] = NetStarRabbitMQ.content[key];
            }
        }
    ...
    function connect(){
        ...
        if(!token){
            console.error('token获取失败，不可连接');
            nsAlert('token获取失败，不可连接', 'error');
            return false;
        }
        // 清除页面上的所有订阅
        pageSubscribe = {};
        var connectConfig = NetStarRabbitMQ.connectConfig;
        var config = NetStarRabbitMQ.config;
        var url = config.ws;
        ...
        var content = NetStarRabbitMQ.content;
    ...
    function init(config){
        ...
        var on_error =  function(error) {
            ...
            // connect();      
            nsConfirm('消息推送连接失败，请重新连接。', function(isConnect){
                if(isConnect){
                    nsAlert('即将重新连接webSocket','warning','warning',10000);
                    console.warn('即将重新连接webSocket');
                    connect();
                    // if(typeof(NetStarRabbitMQ) == "object" && (NetStarRabbitMQ.device === 'mobile' || NetStarRabbitMQ.device === 'app')){
                    //     NetstarHomePage.getWaitingDetailsByAjax();
                    // }else{
                    //     NetstarHomePage.systemInfo.mqLink();
                    // }
                }else{
                    console.error('webSocket连接错误:');
                    console.error(error);
    ...
    var printSubscribeManage = {
        ...
        subscribe: function(){
            ...
            var targetUrl = '/exchange/cloudPrint/' + $.trim(toporgId) + '.cloudPrint';
            var subscribeConfig = {
                target : targetUrl + '.' + userId + '.msg',
                sendUrl : targetUrl,
                unitId : 'NetStarRabbitMQPrint',
                content : {
                ...
                callbackHandler : function(subscribeInfo){
                    ...
                    nsProject.sendPrintInfo(printInfo);//sjj 20200211 处理接受到的消息
                }
            }
            printSubscribeManage.config = subscribeConfig;
            NetStarRabbitMQ.subscribe(subscribeConfig);
        },
    }
    /*****************打印订阅结束********************/
    ...
    function setSystemSubscribe(subConfig){
        ...
        var subscribeConfig = {
            ...
            callbackHandler : function(subscribeInfo){
                ...
                if(typeof(NetStarRabbitMQ) == "object" && NetStarRabbitMQ.device === 'mobile'){
                    NetstarHomePage.refreshWaitingDetailsByAjax();
                    return;
                }
                if(typeof(NetStarRabbitMQ) == "object" && NetStarRabbitMQ.device === 'app'){
                    NetstarHomePage.refreshWaitingDetailsByAjax(function(isChange){
                        if(isChange){
