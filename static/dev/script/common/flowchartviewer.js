...
NetstarUI.flowChartViewer = (function(){
	...
	function showGroupByCells(graphConfig){
		...
		// 修改图形样式 物流子流程（Process 改为 mxgraph.flowchart.process_2;xml中Predefined Process已改为Process 2）lyw 20191202 (原因：图形初始化未知错误；解决：改变图形名称)
		for(var i=0; i<viewerCells.length; i++){
			if(viewerCells[i].style){
				viewerCells[i].style = viewerCells[i].style.replace('shape=process','shape=mxgraph.flowchart.process_2');
			}
		}
		//设置为只读
		...
		// 刷新bllName
		tab.refreshBllName(graphConfig.containerId);
		// websocket
		...
		for(var i=0; i<workitems.length; i++){
			...
			for(var j=0; j<workitems[i].length; j++){
				...
				if(workitemObj.batchNo == 1){
					...
					$mxCellDomCopyBatch.children().attr('width','15');
					$mxCellDomCopyBatch.children().attr('height','15');
					// $mxCellDomCopyBatch.children().attr('x',viewerCellCopy.geometry.x+viewerCellCopy.geometry.width-10);
					// $mxCellDomCopyBatch.children().attr('y',viewerCellCopy.geometry.y-5);
					$mxCellDomCopyBatch.children().attr('x',viewerCellCopy.geometry.x);
					$mxCellDomCopyBatch.children().attr('y',viewerCellCopy.geometry.y+viewerCellCopy.geometry.height-15);
					var mxCellDomCopyBatch = codec.decode($mxCellDomCopyBatch[0]);
					var mxCellDomCopyBatchStyle = mxCellDomCopyBatch.getStyle();
					mxCellDomCopyBatch.setStyle(mxUtils.setStyle(mxCellDomCopyBatchStyle,'shape','mxgraph.flowchart.start_1'));
					var chunkState = workitemObj.workitemstate; // 原始块元素状态
					setWorkFlowColor({workitemstate:'mark', chunkState:chunkState}, mxCellDomCopyBatch); // 设置复制块的颜色
					viewerCells.push(mxCellDomCopyBatch);
				}
			}
		}
		return viewerCells;
	...
	// tab
	var tab = {
		rootWorkitemUrlName : 'workitemId=',
		// 弹框数据地址
		...
		// 刷新bllName
		refreshBllName : function(containerId){
			var tabConfig = tab.tabConfigs[containerId];
			var workflowData = tabConfig.workflowData ? tabConfig.workflowData : {};
			var bllName = workflowData.bllName ? workflowData.bllName : '';
			var $flowchartBllname = tabConfig.$workflowContainer.parent().children('[ns-type="flowchart-bllname"]');
			$flowchartBllname.text(bllName);
		},
		// 刷新树进程
		...
		// 	return typeWord;
		// },
		getTemplateHtml : function(containerHtml, templateObj) {
			...
			while ((result = patt.exec(html)) != null) {
				...
				if(subTemplateName.indexOf('.') > -1){
					var subTemplateNameArr = subTemplateName.split('.');
					type = subTemplateNameArr[0];
					subTemplateName = subTemplateNameArr[1];
				}
				var value = '';
				switch(type){
					case 'date':
						if(templateObj[subTemplateName]){
							value = moment(templateObj[subTemplateName]).format('YYYY-MM-DD HH:mm:ss');
						}
						break;
					case 'dict':
						if(templateObj[subTemplateName] != undefined &&　nsVals.dictData.nsWorkflowRollbackReason){
							var nsWorkflowRollbackReason = nsVals.dictData.nsWorkflowRollbackReason.jsondata;
							value = nsWorkflowRollbackReason[templateObj[subTemplateName]];
						}
						break;
					default:
						value = templateObj[subTemplateName];
						break;
				}
		...
		getWordByOperaType:function(timeData){
			...
			var type = timeData.wfOperationLogOperationType;
			var typeWord = this.TEMPLATEWORD[type];
			var html = this.getTemplateHtml(typeWord, timeData);
			return html;
		},
		// 获取显示时间线的html
		getShowTimelineHtml: function(timeLineData){
			...
			for(var i=0;i<timeLineData.length;i++){
				...
				var liHtml = this.getWordByOperaType(timeData);
				// 根据状态获取类名
				var classStr = '';
				if(i==0){
					classStr = 'current';
		...
		refreshWorkflow : function(containerId){
			...
			if(advanceWorkflow.urlName == 'workitemId' || index==0){
				...
				tabConfig.workitemUrlName = tab.rootWorkitemUrlName;
			}else{
				tabConfig.workitemUrlName = 'parentWorkitemId=';
			}
		...
		getWorkflowContainer: function(viewerConfig){
			...
			var html = '<div class="flowchart">'
							...
							+ '<div class="" ns-type="flowchart-bllname"></div>'
		...
		getUrlIndexByInstanceId : function(instanceIds, containerId){
			...
			for(var i=0; i<labelPagesArr.length; i++){
				...
				if(iContainerId != containerId && labelPage.attrs.instanceIds == instanceIds && labelPage.attrs.pagename == "workflow"){
					urlIndex = i;
					break;
				}
			}
		...
		init: function(config){
			...
			tab.rootWorkitemUrlName = config.rootWorkitemUrlName + '=';
			// 记录显示tab所有参数
			// if(!tab.tabConfigs[containerId]){
				tab.tabConfigs[containerId] = {
					...
					workitemUrlName : tab.rootWorkitemUrlName,
				};
			// }
			// tab页配置
			var tabConfig = tab.tabConfigs[containerId];
