...
nsComponent.getAjaxSubdata = function(componentConfig, components, callbackFunc){
	...
	NetStarUtils.ajax(_ajaxConfig, function(res, ajaxData){
		...
		if(res.success){
			...
			// 判断是否存在默认选中 如果存在不处理；不存在通过defaultSelectedIndex设置默认选中
			if(typeof(_componentConfig.defaultSelectedIndex) == "number" && $.isArray(subdata)){
				var isSetDefCheck = true;
				for(var i=0; i<subdata.length; i++){
					if(subdata[i].selected){ isSetDefCheck = false; break; }
				}
				if(isSetDefCheck){
					if(typeof(subdata[_componentConfig.defaultSelectedIndex]) == "object"){
						subdata[_componentConfig.defaultSelectedIndex].selected = true;
					}
				}
			}
...
nsComponent.getTagHtml = function(tagConfig){
	...
	if(type == 'acts'){
		...
		// 判断是否有 formatHandler
		if(typeof(tagConfig.config) == "object" && typeof(tagConfig.config.formatHandler) == "function"){
			acts = 'formatFunc';
		}
		// 标签位置 默认空
		...
		switch(acts){
			case 'formatFunc':
				// 标签 返回简单的<span>标签
				html = tagConfig.config.formatHandler(text, tagConfig.config);
				break;
			case 'label':
				// 标签 返回简单的<span>标签
				tagClass += "show-text";
		...
		if(config.formSource == 'inlineScreen'||config.formSource == 'fullScreen'){
			...
			$input.on('change', { config:config }, function(ev){
				...
				if(_config.value != sourceVal){
					...
					/*********sjj 20200106 针对表达式计算的回调函数 start************************* */
					if(typeof(_config.countFuncHandler)=='function'){
						_config.countFuncHandler();
					}
					/*********sjj 20200106 针对表达式计算的回调函数 end************************* */
				}
			});
			$input.off('focus');
...
nsUI.radioInput = {
				...
				nsComponent.getAjaxSubdata(_config, formJson.component, function(resConfig, isSuccess, errorInfo){
					if(isSuccess){
						...
						switch(resConfig.formSource){
							...
							case 'inlineScreen':
							case 'staticData':
								var valObj = nsUI.radioInput.getShowValObj(resConfig);
								var content = valObj[resConfig.textField] ? valObj[resConfig.textField] : '';
								$('#'+resConfig.fullID+'-loading').parent().html(nsUI.radioInput.getContentHtml(resConfig, content));//填充元素
								break;
							default:
	...
	getShowValObj : function(_config){
		var value = _config.value;
		var valObj = {};
		var isHaveVal = false;
		if(typeof(value)!="undefined" && value!==""){
				...
				nsComponent.getAjaxSubdata(_config, formJson.component, function(resConfig, isSuccess, errorInfo){
					if(isSuccess){
						...
						switch(resConfig.formSource){
							...
							case 'inlineScreen':
							case 'staticData':
								var valObj = nsUI.checkboxInput.getShowValObj(resConfig);
								var content = '';
								for(var i=0; i<valObj.length; i++){
									if(valObj[i][resConfig.textField]){
										content += valObj[i][resConfig.textField] + ',';
									}
								}
								if(content.length > 0){ content = content.substring(0, content.length-1) }
								$('#'+resConfig.fullID+'-loading').parent().html(nsUI.checkboxInput.getContentHtml(resConfig, content));//填充元素
								break;
							default:
...
nsUI.uploadImageInput = {
	...
	setConfig:function(config){
		...
		config.btnsId = config.fullID + '-btns';
		config.valueFiles = [];
	...
	setValueListByValue : function(config){
		...
		$valueContainer.html($li);
	},
	// 获取移动端html
	getMobileHtml : function(_config){
		this.setConfig(_config);
		var cameraHtml = '';
		var uploadimageBoxClass = "";
		if(typeof(NetstarMain) == "object" && typeof(NetstarMain.cameraName) == "string" && NetstarMain.cameraName == "app"){
			uploadimageBoxClass = "camera";
			cameraHtml = '<div class="input-group-btn " id="'+ _config.btnsId +'">'
							+ '<button type="button" class="btn btn-white btn-icon" ns-name="camera">'
        ...
        html = '<div class="uploadimage-box ' + uploadimageBoxClass + '">'
                + input
				+ cameraHtml
			+ '</div>'
    ...
    uploadFile : function(files, config, callBackFunc){
        ...
        for (var i = 0; i < files.length; i++) {
			var item = files[i];
			var name = item.name ? item.name : new Date().getTime().toString();
            formData.append('files', item, name);
        }
	...
	// app相机获取图片上传
	appUploadField : function(fileURL, config, callBackFunc){
		var url = config.ajax.url;
		var type = config.ajax.type;
		...
		ft.upload(fileURL, encodeURI(url), function(res){
			if(res.success){
                nsalert("上传成功");
                if(typeof(callBackFunc) == "function"){
                    callBackFunc(res[config.ajax.dataSrc], {
						originalFiles : fileURL,
						componentId : config.id,
						formId : config.formID,
						callBackFunc : callBackFunc,
					});
	...
	init:function(config,formJson){
		switch(config.formSource){
			case 'inlineScreen':  	// 行内
				...
				config.$btnContainer = $('#' + config.btnsId);
	...
	dataURLtoBlob : function(dataurl) {
		if(dataurl.indexOf("data:image/jpeg;base64,") == -1){
	...
	initMobile:function(config){
		...
		if(formSource == 'inlineScreen'){
			...
			if(config.$btnContainer && config.$btnContainer.length > 0){
				var $btnContainer = config.$btnContainer;
						...
						case 'camera':
							navigator.camera.getPicture(
								function(path){
									// alert('拍照成功');
									// alert(path);
									var file = _this.dataURLtoBlob(path);
									_this.uploadFile([file], config, function(resData, plusData){
										var _config = nsForm.data[plusData.formId].formInput[plusData.componentId];
										if(_config.isMultiple){
											// 多选
											_config.valueFiles = _config.valueFiles.concat(resData);
								...
								{
									quality:30,//照片质量0--100
									// sourceType:Camera.PictureSourceType.PHOTOLIBRARY, //等于给拍摄好的照片设置的来源
									// destinationType: Camera.DestinationType.FILE_URI,
									destinationType: Camera.DestinationType.DATA_URL,
									// destinationType:Camera.PictureSourceType.PHOTOLIBRARY,//目标路径   enum枚举  PictureSourceType.PHOTOLIBRARY 放到图片库
									// targetWidth:320,
...
nsUI.businessInput = {
	...
	getValStr : function(_config){
		...
		var valStr = '';
		if(value.length > 0){
			if(_config.showExpression.length > 0){
				valStr = nsUI.businessInput.getShowStrByExpVal(_config.showExpression, value);
			}else{
				for(var i=0; i<value.length; i++){
