	...
	*/
	...
	function commonTableBtnsHandler($dom){
		...
		if(typeof($dom)=='number'){
			nIndex = $dom;
			delete commonBtn.getFuncConfigHandler;
		}else{
...
nsTemplate.getBtnArrayByBtns = function(btnData,options){
	...
	*/
	...
	for(var btnI=0; btnI<btnData.length; btnI++){
		...
		switch(btn.text){
			case '留样确认':
				btn.index.iconCls = '<i class="icon-import"></i>';
				break;
			case '留样邮寄':
				btn.index.iconCls = '<i class="icon-truck"></i>';
				break;
			case '留样销毁':
				btn.index.iconCls = '<i class="icon-trash"></i>';
				break;
			case '留样自取':
				btn.index.iconCls = '<i class="icon-file-badge"></i>';
				break;
			case '入库':
				btn.index.iconCls = '<i class="icon-import"></i>';
				break;
			case '问题样品':
				btn.index.iconCls = '<i class="icon-file-warning"></i>';
				break;
			case '查看照片':
				btn.index.iconCls = '<i class="icon-image"></i>';
				break;
			case '出库确认':
				btn.index.iconCls = '<i class="icon-export"></i>';
				break;
			case '样品详情':
				btn.index.iconCls = '<i class="icon-file-check"></i>';
				break;
			case '红冲申请':
