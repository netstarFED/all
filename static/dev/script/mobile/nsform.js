...
// 获得表达式方法对象 计算对象{字段:{name(结果字段):计算方法数组}}  sjj 20200106
nsForm.getCountObj = function(formConfig){
...
// 设置表达式方法 countHandler 通过total:{{price}}*{{number}}参数 sjj 20200106
nsForm.setCountFuncByTotal = function(formJson){
	 // formConfig form表单组件配置
	 var formConfig = formJson.form;
	 var countObj = nsForm.getCountObj(formConfig); // 计算对象{字段:{name(结果字段):计算方法数组}}
	 var rexContent = {
		 field : /\{\{(.*?)\}\}/g,                    // 字段
		 fieldStr : /\{\{(.*?)\}\}/,                  // 字段
		 markStr : /[\+\-\*\/\(\)\[\]]/,               // +-*/()[]
	...
	for(var componentI=0; componentI<formConfig.length; componentI++){
		...
		if(typeof(countObj[componentId])=="object"){
			...
			componentConfig.countFuncHandler = (function(fieldCountFuncObj){
				return function(){
					var formData = nsForm.getValues(this.formID, false);
					var countData = {};
					...
					nsForm.edit(editDataArr, this.formID);
				}
			})(fieldCountFuncObj)
		}
	}
...
nsForm.setDefault = function(formJson){
	...
	*/
	...
	/********************************* *sjj 20200106 添加手机端表单支持计算表达式 ***************************start */
	// 设置表达式方法 total:{{price}}*{{number}}
	nsForm.setCountFuncByTotal(formJson);
	/********************************* *sjj 20200106 添加手机端表单支持计算表达式 ***************************end */
	return formJson;
}
//初始化form的初始化对象
...
nsForm.setValueByValueExpression = function(componentConfig, formConfig){
	...
	// 系统参数对象
	var systemInfo = {};
	if(typeof(NetstarMainPage)!='undefined'){
		systemInfo = NetstarMainPage && typeof(NetstarMainPage.systemInfo) == "object" ? NetstarMainPage.systemInfo : {};
	}
	var pageData = {};
