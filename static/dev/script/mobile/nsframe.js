...
nsFrame.loadPageVRouter = function(url){
	...
	if($('container .businessdatabaseeditor').length > 0){
		var templateId = $('container .businessdatabaseeditor').attr('id');
		nsFrame.cacheUrlVRouter.pageParam[nsFrame.cacheUrlVRouter.counter] = {
			templateId:templateId,
			config:NetstarTemplate.templates.businessDataBaseEditorMobile.data[templateId].config
		};
	}
	if($('container .doclistviewermobile').length > 0){
		var templateId = $('container .doclistviewermobile').attr('id');
		nsFrame.cacheUrlVRouter.pageParam[nsFrame.cacheUrlVRouter.counter] = {
			templateId:templateId,
			config:NetstarTemplate.templates.docListViewerMobile.data[templateId].config
		};
	}
	...
	if($('container .listmobile').length > 0){
		var templateId = $('container .listmobile').attr('id');
		nsFrame.cacheUrlVRouter.pageParam[nsFrame.cacheUrlVRouter.counter] = {
			templateId:templateId,
			config:NetstarTemplate.templates.listMobile.data[templateId].config
		};
	}
	$('container').remove();
...
nsFrame.getTemplatePageStrByUrlStr = function(pageStr, urlStr){
	// 通过？截断url 地址？参数
	var paramStr = urlStr.substring(urlStr.indexOf('?') + 1);
	// 获取第一个分号之前的参数  用来区分传参类型
	var pegeUrlTypeStr = paramStr.substring(0, paramStr.indexOf(';'));
	paramStr = paramStr.substring(paramStr.indexOf(';') + 1);
	pegeUrlType = pegeUrlTypeStr.split('=')[1];
	var isAddPageParams = false;
	var pageparams = {};
	switch(pegeUrlType){
		case 'W':
			// 问号 通过&区分出对象
			isAddPageParams = true;
			var paramArr = paramStr.split('&');
			for(var i=0; i<paramArr.length; i++){
				if(paramArr[i].length > 0 && paramArr[i].indexOf('=') > 0){
					var _paramArr = paramArr[i].split('=');
					pageparams[_paramArr[0]] = _paramArr[1];
				}
			}
			break;
		case 'F':
			...
			for(var i=0; i<paramArr.length; i++){
				if(paramArr[i].length > 0 && paramArr[i].indexOf('=') > 0){
					var _paramArr = paramArr[i].split('=');
					if(typeof(pageparams[_paramArr[0]]) == "undefined"){
						pageparams[_paramArr[0]] = _paramArr[1];
					}else{
						if(!$.isArray(pageparams[_paramArr[0]])){
							var prevData = pageparams[_paramArr[0]];
							pageparams[_paramArr[0]] = [prevData];
						}
						pageparams[_paramArr[0]].push(_paramArr[1]);
					}
				}
			}
			break;
	...
	if(isAddPageParams){
		// 截取页面的 NetstarTemplate.init(****); 替换
		var templateRex = /NetstarTemplate\.init\((.*?)\)/;
		if(templateRex.test(pageStr)){
			var templateRexData = pageStr.match(templateRex);
			var templateStr = templateRexData[0];
			var templateConfigStr = templateRexData[1];
			templateConfigStr += '.pageParam=' + JSON.stringify(pageparams) + ';';
			var replaceStr = templateConfigStr;
			var currentStr = '// NETSTAR添加url参数即pageParam';
			pageStr = pageStr.replace(currentStr, replaceStr);
		}
	}
	return pageStr;
}
nsFrame.getTemplatePageStrByUrlVariable = function(pageStr, urlStr){
	// templateparam=nscloud.samplecustomlist21576821186248
	var urlEndStr = urlStr.substring(urlStr.indexOf('?') + 1)
	if(urlEndStr.indexOf('templateparam') === 0){
		var templateparamArr = urlEndStr.split('=');
		var pageParams = {};
		var tempValueName = templateparamArr[1];
		if(NetstarTempValues[tempValueName]){
			tempValueName = decodeURIComponent(tempValueName);
			pageParams = NetstarTempValues[tempValueName];
	...
	var ajaxConfig = {
		...
		callBackFunc:function(isSuccess, data, _pageConfig){
			if(isSuccess){
				...
				formatStr = nsFrame.filterCode(formatStr,config.filterContainer);
				var containerDom = $("#"+containerID+isWithContent);
				/*****lyw 切换静态页 时数据参数传递 start******/
				var getPageUrl = _pageConfig.url;
				if(formatStr.indexOf('pageProperty.init') == -1){
					// 页面为静态页不执行pageProperty.init
					if(getPageUrl.indexOf('NETSTAR-URL-TYPE') > 0){
						// 地址参数
						formatStr = nsFrame.getTemplatePageStrByUrlStr(formatStr, getPageUrl);
					}else{
						if(getPageUrl.indexOf('?') > 0){
							// 弹出页面时当前页面参数
							formatStr = nsFrame.getTemplatePageStrByUrlVariable(formatStr, getPageUrl);
						}
					}
				}
				/*****lyw 切换静态页 时数据参数传递 end******/
				containerDom.html(formatStr);
				//移除无用DOM和重复DOM
				containerDom.find("#dialog-more").remove();
				containerDom.find("#placeholder-popupbox").remove();
