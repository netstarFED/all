...
NetstarTemplate.templates.processDocBaseMobile = (function(){
	...
	function getPageData(_value,_isValid){
		...
		if(data == false){
			...
			NetStarUtils.deleteAllObjectState(data);
			NetStarUtils.deleteAllObjectState(_config.serverData);
			...
			if(!$.isEmptyObject(_config.pageParam)){
				...
				var isSendPageParam  = typeof(_config.isSendPageParam)=='boolean' ? _config.isSendPageParam : false;
				if(isSendPageParam){
					NetStarUtils.setDefaultValues(pageData,_config.pageParam);
				}else{
					var pageFieldArr = ['processId','activityId','activityName','workItemId','workItemIds','workflowType','acceptFileIds'];
					for(var paramI=0; paramI<pageFieldArr.length; paramI++){
					   if(_config.pageParam[pageFieldArr[paramI]]){
							pageData[pageFieldArr[paramI]] = _config.pageParam[pageFieldArr[paramI]];
					   }
					}
				}
	...
	function init(_config){
		...
		function changeTabHandler(ev){
			...
			// 	nsFrame.cacheUrlVRouter.pageOperate[containerId] = {
			// 		func : function(){
			// 			var $li = $('li[ns-componentid="'+panelId+'"]');
		...
		function initContainer(){
			...
			var layoutPlanesClass = '';
			if(!config.isTab){
				layoutPlanesClass = 'no-mobile-search';
			}
			var containerHtml = '<div class="row layout-planes processdocbasetab ' + layoutPlanesClass + '" id="'+config.id+'" ns-package="'+config.package+'"></div>';
		...
		function voInit(voJson){
			...
			for(var voId in voJson){
				...
				if($.isEmptyObject(voData)){if(!$.isEmptyObject(config.pageParam)){voData = config.pageParam;}}
		...
		function blockListInit(listJson){
			...
			for(var listId in listJson){
				...
				if(typeof(listConfig.rowTapHandler) == "function"){
					gridConfig.ui.rowTapHandler = listConfig.rowTapHandler;
				}
				var vueObj = NetstarBlockListM.init(gridConfig);	
				
				if($.isEmptyObject(listConfig.businessConfig)){
					var isInlineDialog = false;
					if(!$.isEmptyObject(listConfig.params)){
						if(listConfig.params.editMode == 'inlineDialog'){
							isInlineDialog = true;
						}
					}
					if(isInlineDialog){
						$('#'+listId+'-addbtn-container button').on('click',function(ev){
							var formId = 'form-'+listConfig.id;
							var btnId = 'btn-'+listConfig.id;
							var appendHtml = '<div class="mobilewindow-fullscreen">'
												+'<div id="'+formId+'" component-type="vo">'
												+'</div>'
												+'<div id="'+btnId+'" component-type="btns" class="nav-form">'
												+'</div>'
											+'</div>';
							$('container').append(appendHtml);
							var formFieldArray = [];
							var fieldArray = $.extend(true,[],listConfig.field);
							  NetstarTemplate.setFieldHide('form',fieldArray,listConfig.hide);
							for(var fieldI=0; fieldI<fieldArray.length; fieldI++){
								...
								if(fieldJson.type == 'select'){
									fieldJson.type = 'radio';
								}
								if(fieldJson.type == 'select2'){
								...
								fieldJson.acts = 'formlabel';
								if(fieldJson.type == 'date'){
									fieldJson.acts = 'date-label';
								}else if(fieldJson.type == 'datetime'){
									fieldJson.acts = 'datetime-label';
								}
								formFieldArray.push(fieldJson);
							}
							var formJson = {
								id:formId,
								form:$.extend(true,[],formFieldArray),
								formSource:'inlineScreen',
							};
							nsForm.init(formJson);
							var navJson = {
								...
								btns: [
									[{
										...
										isReturn:true,
										handler:function(){
											var formJson = nsForm.getValues(formId);
											if(formJson){
												NetstarBlockListM.dataManager.addRow(formJson,listConfig.id);
						...
						$('[nsgirdcontainer="grid-body-addbtn"]').remove();
					}
				}else{
					$('#'+listId+'-addbtn-container button').on('click',function(ev){
						var $this = $(this);
		...
		function moreBtnsHandler(dom){
			...
			if(!$.isEmptyObject(config.draftBox)){
				var btnLength = moreBtnsArray.length;
				moreBtnsArray.push({
					text:'保存草稿',
					isReturn:true,
					index:{
						customerIndex:btnLength,
