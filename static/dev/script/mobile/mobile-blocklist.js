...
var NetstarBlockListM = (function () {
		...
		PC: {
			...
			BODY: {
				...
				CONTENTTABLE:
					...
					+ '<div class="pt-grid" :style="domParams.contentTable.style" :id="domParams.contentTable.id">' +
						'<div v-for="(row,index) in rows" :ns-rowindex="index" :ns-id="row[idField]" class="pt-block-list" :class="[{\'selected\':row.netstarSelectedFlag},{\'disabled\':row[\'NETSTAR-TRDISABLE\']},plusClass,netstarRowStateFlag(row),netstarRowClassFlag(row,index)]">' +
							...
							'</div>' +
							//@dblclick="rowdbClickHandler" $click="rowClickHandler"
							'<div class="pt-block-content" v-else v-html="netStarColumnText(row,index)" @touchstart="touchstartHandlerByRow" @touchmove="touchmoveHandlerByRow" @touchend="touchendHandlerByRow">' +
							'</div>' +
							'<template v-for="columnConfig in columns">' +
								...
								// '<template v-else-if="columnConfig.columnType === \'editDelBtns\'">\
								// 	<div v-html="row[columnConfig.field]" class="rowbtns"></div>\
								// </template>' +
                            '</template>'
                            + '<div class="rowbtns" v-if="ui.isHaveEditDeleteBtn || ui.isHaveEditBtn">'
                                + '<template v-for="columnConfig in columns">'
                                    + '<template v-if="columnConfig.columnType === \'editDelBtns\'">\
				...
				ROWDELETEBUTTON: '<button \
					...
					title="{{text}}" \
					ns-index="{{index}}" \
					onclick="javascript:NetstarBlockListM.rowBtnsHandler(event, \'{{gridId}}\',this)" \
					class="pt-btn pt-btn-icon pt-btn-default">\
					{{html}}\
				</button>',
				//行内编辑按钮
				ROWEDITBUTTON: '<button \
					type="button" \
					title="{{text}}" \
					ns-index="{{index}}" \
					onclick="javascript:NetstarBlockListM.rowBtnsHandler(event, \'{{gridId}}\',this)" \
	...
	var methodsManager = {
		...
		body: {
			rowLongtapHandler: function (ev, _vueData) {
				ev.preventDefault();
				ev.stopPropagation();
				...
				//回调行选中事件
				if (gridConfig.ui.rowLongtapHandler) {
					...
					var callbackData = gridConfig.ui.rowLongtapHandler(rowData, _vueData.$data, _vueData, gridConfig)
					//如果有返回值，则刷新行数据
			...
			touchEventHandler:function(touchEvent,_vueData){
				var clickTimes = 1;
				var firstClick = new Date().getTime();
				var finished = false;
				var timeoutId;
				function fireEvent(_event,_vueObj,_methodsManager) {
					var now = new Date().getTime();
					if(now - firstClick > 500) {
						finished = true;
						//console.log('longtap')
						_methodsManager.rowLongtapHandler(_event,_vueObj);
					}else if(clickTimes > 1) {
						finished = true;
						//console.log('dblclick')
						_methodsManager.rowdbClickHandler(_event,_vueObj);
					}else {
						timeoutId = setTimeout(function(){
							finished = true;
							//console.log('click')
							_methodsManager.rowClickHandler(_event,_vueObj);
						},200);
					}
				}
				return{
					...
					touchend:function(event,_vueData,_methods){
						fireEvent(event,_vueData,_methods);
					},
					isFinished:function(){
						return finished;
					}
			...
			touchstartHandlerByRow:function(touchStartEvent,_vueData){
				// touchStartEvent.preventDefault();  // 阻止其他点击事件
				// touchStartEvent.stopPropagation();
				if(_vueData.prevTouchEvent && !_vueData.prevTouchEvent.isFinished()){
					_vueData.prevTouchEvent.touchstart(touchStartEvent,_vueData);
				}else{
					_vueData.prevTouchEvent = new this.touchEventHandler(touchStartEvent,_vueData);
				}
			},
			touchmoveHandlerByRow:function(touchMoveEvent,_vueData){
				// touchMoveEvent.preventDefault();
				// touchMoveEvent.stopPropagation();
				_vueData.prevTouchEvent = undefined;
			},
			touchendHandlerByRow:function(touchEndEvent,_vueData){
				// touchEndEvent.preventDefault();
				// touchEndEvent.stopPropagation();
				if(_vueData.prevTouchEvent && !_vueData.prevTouchEvent.isFinished()){
					_vueData.prevTouchEvent.touchend(touchEndEvent,_vueData,this);
				}
			},
			//选中行的操作
				...
				if(expression){
					...
					if(rex2.test(expression)){
						...
						/*if(data.propertyValue){
							//sjj 20191223 存在属性值的概念
							var propertyValueArray = JSON.parse(data.propertyValue);
							...
							listHtml += '<div class="pt-mobile-propertyvalue">'
											+propertyValueHtml
										+'</div>';
						}*/
			...
			touchendHandler:function(ev,_vueData){
				...
				if(diff > 1000 && changedTouches.length == 1 && changedTouches[0].pageX == pageX){
					...
					//回调行选中事件
					if(typeof(gridConfig.ui.rowTapHandler) == "function"){
						gridConfig.ui.rowTapHandler(rowIndex, rowData, _vueData.$data, _vueData, gridConfig);
					}
				}
			},
		},
		...
		rowBtnsHandler: function (event, gridId, _this) {
			...
			var configs = NetstarBlockListM.configs[gridId];
			//定位操作数据
			var rowIndex = rowIndex = $btn.closest('div[ns-rowindex]').attr('ns-rowindex');
		...
		// 新增按钮事件
		addRowBtnsHandler : function(event, gridConfig, vueGridConfig){
			console.log(gridConfig);
			console.log(vueGridConfig);
			methodsManager.editRowDataFunc({}, gridConfig);
		},
		// 行编辑方法 编辑/新增
		editRowDataFunc : function(rowData, gridConfig, rowIndex){
			var isAdd = typeof(rowIndex) == 'number' ? false : true;
			var editFields = gridConfig.editFields;
			var $gridContainer = $('#' + gridConfig.id);
			var $container = $gridContainer.closest('container');
			if($container.length == 0){
				$container = $('body');
			}
			var editContainerId = gridConfig.id + '-edit';
			var editFormId = gridConfig.id + '-edit-form';
			var editbtnsId = gridConfig.id + '-edit-btns';
			// 容器
			var html = '<div class="mobilewindow-fullscreen" id="' + editContainerId + '">'
							+ '<div class="" id="' + editFormId + '">'
							+ '</div>'
							+ '<div class="" id="' + editbtnsId + '">'
								+ '<div class="btn-group ">'
									+ '<button class="btn btn-info" ns-name="callback">'
										+ '<span>返回</span>'
									+ '</button>'
									+ '<button class="btn btn-default" ns-name="save">'
										+ '<span>保存</span>'
									+ '</button>'
								+ '</div>'
							+ '</div>'
			...
			var formJson = {
				...
				form : $.extend(true, [], editFields),
			};
			nsForm.initByValues(formJson, rowData);
			// 按钮
			var $btns = $('#' + editbtnsId).find('button');
			$btns.off('click');
			$btns.on('click', function(ev){
				...
				switch(nsName){
					case 'save':
						var formData = nsForm.getFormJSON(editFormId, true);
						if(formData){
							if(isAdd){
								dataManager.addRow(formData, gridConfig.id);
							}else{
								dataManager.editRow(formData, gridConfig.id, rowIndex);
							}
	...
	var configManager = {
		...
		// 获取表格编辑列 编辑配置
		getEditConfig : function(gridConfig){
			var columns = gridConfig.columns;
			if(!$.isArray(columns)) {
				if (debugerMode) {
					console.error('表格Grid配置参数错误，columns应当是数组');
					console.error(gridConfig);
				}
				return [];
			}
			var editField = [];
			for(var i=0; i<columns.length; i++){
				if(!columns[i].editable){ continue; }
				var editConfig = $.extend(true, {}, columns[i].editConfig);
				if($.isEmptyObject(editConfig)){
					editConfig = {
						type : 'text',
					}
				}
				...
				editConfig.label = columns[i].title;
				editConfig.id = columns[i].field;
				editField.push(editConfig);
			}
			return editField;
		},
			...
			if (_gridConfig.ui.isHaveEditDeleteBtn) {
				...
				var delConfig = {
					...
					handler: function (data) {
						dataManager.delRow(data.rowData, data.gridId, data.rowIndex);
					},
					isUiDeleteBtn : true,
					index: _gridConfig.ui.tableRowBtns.length.toString(),
					gridId: _gridConfig.id,
					html: '<i class="icon-close"></i><span></span>'
				};
				//插入当前删除按钮事件到按钮组中
				_gridConfig.ui.tableRowBtns.push(delConfig);
			}
			//编辑按钮
			if (_gridConfig.ui.isHaveEditBtn) {
				var editColumn = $.extend(true, {}, this.SYSTEMCOLUMNS.ROWEDITBUTTON);
				editColumn.styleObj = { 'min-width': editColumn.width + 'px', 'width': editColumn.width + 'px' };
				_columnsConfig.push(editColumn);
				tableTotalWidth += editColumn.width;
				//插入当前删除按钮事件到按钮组中
				if ($.isArray(_gridConfig.ui.tableRowBtns) == false) {
					_gridConfig.ui.tableRowBtns = [];
				}
				var editConfig = {
					text: '编辑',
					handler: function (data) {
						var grid = NetstarBlockListM.configs[data.gridId].gridConfig;
						methodsManager.editRowDataFunc(data.rowData, grid, data.rowIndex);
					},
					isUiEditBtn : true,
					index: _gridConfig.ui.tableRowBtns.length.toString(),
					gridId: _gridConfig.id,
					html: '<i class="icon-edit-o"></i><span></span>'
				};
				//插入当前删除按钮事件到按钮组中
				_gridConfig.ui.tableRowBtns.push(editConfig);
			}
		...
		getConfig:function(_gridConfig){
			...
			gridConfig.editFields = this.getEditConfig(gridConfig);
	...
	var dataManager = {
		...
		getRowData: function (_rowData, _gridConfig) {
			...
			var tableRowBtns = _gridConfig.ui.tableRowBtns;
			if($.isArray(tableRowBtns) && tableRowBtns.length > 0){
				for(var i=0; i<tableRowBtns.length; i++){
					var rowBtn = tableRowBtns[i];
					if(rowBtn.isUiDeleteBtn){
						var btnsHtml = NetStarUtils.getTemplateHtml(vueTemplate.BODY.ROWDELETEBUTTON, rowBtn, false);
						rowData['NETSTAR-DELETE'] = btnsHtml;
					}
					if(rowBtn.isUiEditBtn){
						var btnsHtml = NetStarUtils.getTemplateHtml(vueTemplate.BODY.ROWEDITBUTTON, rowBtn, false);
						rowData['NETSTAR-EDIT'] = btnsHtml;
					}
				}
			}
		...
		//修改行数据
		editRow: function (rowData, gridId, index) {
			//tableRowsData:array [{id:'',value:""},{....}] 整个表格的所有行数据
			//gridId:string gird的id
			var grid = NetstarBlockListM.configs[gridId];
			if (typeof (grid) != 'object') {
				...
				var originalRows = $.extend(true, [], grid.vueConfig.data.originalRows);//返回的总数据
				var rows = $.extend(true, [], grid.vueConfig.data.rows);  //当前显示的行数据
				var idField = grid.gridConfig.data.idField;
				...
				if (dataNsIndex > -1) {
					rows[dataNsIndex] = rowData; //删除其对应位置
					originalRows[originalNsIndex] = rowData;
					grid.vueConfig.data.rows = rows;
					grid.vueConfig.data.originalRows = originalRows;
				} else {
					console.error('delIndex:' + dataNsIndex + ' 找不到对应行数据，请核实');
				}
			}
            ...
            NetStarUtils.ajax(ajaxOptions, function(res, _ajaxOptions){
                ...
                if(res.success){
					...
					if(dataSrc.indexOf('.')>-1){
						var listObject = NetStarUtils.getListDataByExp(res,dataSrc);
						var keyField = dataSrc.substring(dataSrc.lastIndexOf('.')+1,dataSrc.length);
						rows = listObject[keyField];
					}
			...
			if(gridConfig.data.src){
				...
				if(gridConfig.data.dataSource.length == 0){
					if(!gridConfig.ui.isEditMode){
						...
						if(gridConfig.placeholder){
							gridConfig.domParams.panelOfEmptyRows.info = gridConfig.placeholder;
						}
					}
				}
                //如果定义了数据源
		...
		getVueConfig:function(_gridConfig){
			var vueConfig = {
				...
				methods:{
					...
					touchstartHandlerByRow:function(ev){
						methodsManager.body.touchstartHandlerByRow(ev, this);
					},
					touchmoveHandlerByRow:function(ev){
						methodsManager.body.touchmoveHandlerByRow(ev, this);
					},
					touchendHandlerByRow:function(ev){
						methodsManager.body.touchendHandlerByRow(ev, this);
					},
					...
					addRowBtnsHandler : function(ev){
						methodsManager.addRowBtnsHandler(ev, _gridConfig, this);
					},
				},
				mounted:function() {
	...
	return{
		...
		rowBtnsHandler: methodsManager.rowBtnsHandler, //行内按钮点击事件
	}
})(jQuery)