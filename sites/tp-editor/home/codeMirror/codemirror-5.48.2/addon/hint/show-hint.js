 ...
 * @LastEditTime: 2019-08-15 09:29:50
...
(function (mod) {
  ...
  CodeMirror.defineExtension("showHint1", function (options, datas) {
    // console.log("datas",datas.list)
    ajaxExplain(datas.list);
    options = parseOptions(this, this.getCursor("start"), options);
    var selections = this.listSelections();
    if (selections.length > 1) return;
    // By default, don't allow completion when something is selected.
  ...
  function ajaxData() {
    $.ajax({
      type: 'GET',
      url: 'codeMirror/keyValueExplain.json',
      dataType: "json",
      async: false,
  ...
  // console.log(ajaxExplainData)
  function ajaxExplain(data) {
    objContent = {};
    var arr = [];
    var arrKey = [];
    for (var i = 0; i < data.length; i++) {
      for (var key in ajaxExplainData) {
        var dataDeal = data[i].replace(/\"/g, "");
        if (("" + key) == ("" + dataDeal)) {
          var keyDeal = key.replace(/\"/g, "");
          arrKey.push(keyDeal);
          arr.push(ajaxExplainData[key]);
          continue;
        }
      }
    }
