var ganttComponents = {
    ...
    getGanttModel: function($,cb){
        ...
        $.ajax({
            ...
            // type : "GET",
            url: getRootPathUrl() + "/zc/sampleAccepts/getAppointmentPage",
            type: "POST",
            headers:{
                "Authorization":JSON.parse(getLocalStorge("Authorization")).code,
            ...
            if(res.success){
                ...
                for(var i = 0,len = ganttData.length; i < len; i++){
                    var obj = {
                        parent:0,
                        id:ganttData[i].id,
                        start_date: formatDate(ganttData[i].whenBegin),// 开始时间
                        end_date: formatDate(ganttData[i].whenEnd),// 结束时间
                        duration: dateDiff(ganttData[i].whenBegin,ganttData[i].whenEnd),// 持续时间
    ...
    init:function(gantt,tasks,id){
        ...
        gantt.templates.tooltip_text = function (start, end, task) {
            ...
            return "<b>项目名称:</b> " + task.text + 
                ...
                "<br/><b>工况详情:</b>" + task.operatingMode;
        };
