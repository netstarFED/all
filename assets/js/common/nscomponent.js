 ...
 * @Desription: 组件
 * @Author: netstar.lyw
 * @Date: 2018-11-22 16:43:58
 * @LastEditTime: 2019-11-06 10:18:11
 */
nsComponent = {
    codeMirror: {}, //编辑器
};
...
nsComponent.part = (function($) {
    ...
    function getFormLabel(html) {
        ...
        if (label === '') {
            ...
            if (config.formSource == 'staticData') {
                if (config.acts != 'map' && config.acts != 'formlabel' && config.acts != 'date-label' && config.acts != 'datetime-label' && config.showState != 'fullScreen') {
                    classStr += ' hidden';
                } else {
                    label += ':';
                }
    ...
    function getDefalutHtml(html) {
        ...
        switch (config.formSource) {
            ...
            case 'staticData':
                ...
                var deleteBtn = '';
                if (config.formSource == 'inlineScreen' && config.mindjetFieldPosition == 'field-more' && config.showState != "more") {
                ...
                var nsType = config.type;
                switch(typeClass){
                    case 'daterangepicker':
                        typeClass = 'ns-mb-daterangepicker';
                        break;
                    case 'adderSubtracter':
                        typeClass = typeClass.toLowerCase();
                        nsType = nsType.toLowerCase();
                        break;
                }
                html = '<div class="form-group" ns-type="'+nsType+'">' +
                    deleteBtn +
                    nsComponent.part.getFormLabel() +
                    '<div class="form-item ' + typeClass + ' ' + formItemClass + '" ns-id="' + config.id + '">' +
                    html +
                    '</div>' +
                    '</div>'
                var newline = '';
    ...
    function getProvinceSelect() {
		switch(config.formSource){
            ...
            case 'inlineScreen':   	// 行内
                var html = nsUI.provinceselectInput.getHtml(config);
                html = getDefalutHtml(html);
                return html;
                break;
        }
    ...
    function getMapInput(){
        var html = nsUI.mapInput.getHtml(config);
        html = getDefalutHtml(html);
        return html;
    }
    //scientific-input
    ...
    function getDateRangePicker() {
        var html = nsUI.daterangepickerInput.getHtml(config);
        html = getDefalutHtml(html);
        return html;
    }
    //typeahead
    ...
    function getSelect() {
        ...
        if (typeof(configUrl) == 'string') {
            ...
            if(typeof(NetStarUtils) == "object"){
                // 兼容前后端分离
                ajaxData.contentType = config.contentType ? config.contentType : 'application/x-www-form-urlencoded';
                delete ajaxData.success;
                delete ajaxData.error;
                delete ajaxData.context;
                ajaxData.plusData = {
                    configId : config.id,
                    formId : config.formID,
                }
                NetStarUtils.ajax(ajaxData, function(data, ajaxConfig){
                    // config = ajaxConfig;
                    var plusData = ajaxConfig.plusData;
                    config = nsForm.data[plusData.formId].formInput[plusData.configId];
                    if (data.success) {
                        var subdata;
                        if (typeof(config.dataSrc) != 'string') {
                            subdata = data;
    ...
    function getSelect2() {
        ...
        if (typeof(configUrl) == 'string') {
            ...
            if (config.isServiceMode == false) {
                ...
                if(typeof(NetStarUtils) == "object"){
                    // 兼容前后端分离
                    ajaxConfig.contentType = config.contentType ? config.contentType : 'application/x-www-form-urlencoded';
                    delete ajaxConfig.success;
                    delete ajaxConfig.error;
                    delete ajaxConfig.context;
                    ajaxConfig.plusData = {
                        configId : config.id,
                        formId : config.formID,
                    }
    ...
    //sortAtHalfScreen sjj 20190417 手机端半屏模式下的排序
    function getSortAtHalfScreenHtml(){
        var html = nsUI.sortAtHalfScreen.getHtml(config,formJson);
        html = getDefalutHtml(html);
        return html;
    }
    //日期区间radio
    ...
    function getUploadImage() {
        if( config.formSource == "halfScreen" ||
            ...
            if(config.formSource == "inlineScreen"){
                var html = nsUI.uploadImageInput.getHtml(config);
                html = getDefalutHtml(html);
                return html;
            }
        }
    ...
    // radioString checkString组件
    function getRadioCheckStringHtml() {
        var configObj = $.extend(true, {}, config);
        //必填   required="required"
        var html = "";
        var perfix = configObj.formSource + "-" + configObj.formID;
        var textField = configObj.textField;
        var inputType = configObj.inputType;
        var placeHolder = configObj.placeholder;
        var isHasChecked = false;
        $.each(configObj.subdata, function(index, item) {
            html += '<label class="' + (item.isChecked ? !isHasChecked ? "checked " : "" : "") + '' + inputType + '-inline ' + (item.isDisabled ? "disabled" : "") + '">' + item[textField] + '<input ns-type="' + inputType + '" ns-id="'+config.id+'" class="' + inputType + '-options" type="' + inputType + '" name="' + perfix + '" value="' + item[textField] + '" ' + (item.isDisabled ? "disabled" : "") + ' ' + (item.isChecked ? "checked" : "") + '/></label>';
            //如果是radio，则只默认选中第一个设置选中的
        	if(inputType == 'radio' && item.isChecked){
        		isHasChecked = true;
        	}
        });
        html += '<label id="' + perfix + '-other-label" class="' + inputType + '-inline">其他</label><input placeholder="'+ placeHolder +'" type="text" class="' + inputType + '-options" id="' + perfix + '-other" disabled>';
        return getDefalutHtml(html);
    }
    // 规则内容组件
    ...
    // 加减数字组件 lyw 20190702
    function getDdderSubtracterMobileHtml() {
        var html = '';
        html = nsUI.adderSubtracterInput.getHtml(config);
        html = getDefalutHtml(html);
        return html;
    }
    // 业务组件 lyw 20190708
    function getBusinessHtml() {
        var html = '';
        html = nsUI.businessInput.getHtml(config);
        html = getDefalutHtml(html);
        return html;
    }
    return {
        ...
        getDaterange: getDaterange, //日期区间
        ...
        getRadioCheckStringHtml: getRadioCheckStringHtml, //radioString checkString组件
        ...
        getDdderSubtracterMobileHtml: getDdderSubtracterMobileHtml, // 加减数字组件
        getMapInput : getMapInput,                       // 地图组件
        getSortAtHalfScreenHtml:getSortAtHalfScreenHtml,//获取半屏模式下的排序输出sjj 20190417
        getBusinessHtml : getBusinessHtml, // 业务组件
    }
})(jQuery);
//组件数组容器 返回html，用于多数组
// html：已经生成的组件代码HTML
...
nsComponent.init = (function($) {
    ...
    function transactorBtn(config) {
        ...
        $closeBtn.on('click', function(event) {
            ...
            // 	}
            // }
            var index = $this.parents('.form-control').children().index($this.parents('.transactor-span'));
    ...
    //日期区间组件 移动端
    function daterangepickerMobile(config) {
        nsUI.daterangepickerInput.init(config, formJson);
    }
                ...
                if (config.subdata.length > 0) {
                    ...
                    $select2.on('select2:close', function(ev) {
                        ...
                        if($.isArray(value)){
                            if(value.length > 0){
                                for (var i = 0; i < config.subdata.length; i++) {
                                    if (value.indexOf(config.subdata[i][config.valueField]) > -1) {
                                        selectedArr.push(config.subdata[i]);
                                    }
                                }
                            }
                        ...
                        if (typeof(config.changeHandler) == 'function') {
                            config.changeHandler(value, textStr,selectedArr);
                        }
                        //模拟一个公用的事件回调
                        if (typeof(config.commonChangeHandler) == 'function') {
                            var obj = {
    ...
    function uploadImage(config) {
        if( config.formSource == "halfScreen" ||
            ...
            if(config.formSource == "inlineScreen"){
                nsUI.uploadImageInput.init(config, formJson);
                return;
            }
        }
    ...
    function radiocheckstring(config) {
        var configObj = $.extend(true, {}, config);
        nsUI.radioCheckString.init(configObj);
    }
    ...
    // 地图
    function map(config){
        nsUI.mapInput.init(config, formJson);
        return;
    }
    //sjj 20190417 半屏模式下的排序
    function sortAtHalfScreen(config){
        nsUI.sortAtHalfScreen.init(config,formJson);
    }
    return {
        ...
        daterangepickerMobile: daterangepickerMobile,
        ...
        numrange: numrange,
        radiocheckstring: radiocheckstring,
        ...
        assistantBtnInit: assistantBtnInit,
        map : map,
        sortAtHalfScreen:sortAtHalfScreen, //sjj 20190417 半屏模式下的排序
    }
})(jQuery);
// 不规范值的转换
nsComponent.setIrregularCharge = function(config, formJson) {
    if (formJson.formSource == 'form' || formJson.formSource == 'inlineScreen') {
...
nsComponent.setDefault = function(config, formJson) {
    ...
    if (config.formSource == 'inlineScreen') {
        if (config.rules) {
            ...
            if(typeof(config.mobileSign) == "string"){
                config.mobileSign = config.mobileSign.replace('required', '');
            }
        }
    ...
    if(config.hidden || config.readonly || config.disabled){
        config.sourceRules = config.sourceRules ? config.sourceRules : config.rules;
        config.rules = '';
        config.mobileSign = config.mobileSign ? config.mobileSign.replace('required', '') : '';
    }
    return config;
}
/**
...
nsComponent.getHTML = function(config, formJson) {
    ...
    switch (config.type) {
        ...
        case 'transactor':
            ...
            var validNameObj = {
                ...
                post: ['post', 'candidateDept', 'deptOfOtherActivity'],
                dept: ['candidateDept'],
            ...
            if (config.value) {
                ...
                for (var i = 0; i < valueArr.length; i++) {
                    ...
                    if ($.isEmptyObject(formatValObj)) {} else {
    ...
    switch (config.type) {
        ...
        case 'datetime':
            ...
            break;
        case 'daterange':
            inputHtml = nsComponent.part.getDaterange();
            break;
        case 'daterangepicker':
        ...
        case 'map':
            // config.column = 12;
            inputHtml = nsComponent.part.getMapInput();
            break;
        case 'webupload':
        ...
        case 'numrange':
            ...
            break;
        case 'radiocheckstring':
            inputHtml = nsComponent.part.getRadioCheckStringHtml();
            break;
        case 'multiselect':
        ...
        case 'adderSubtracter':
            // 加减数字组件
            inputHtml = nsComponent.part.getDdderSubtracterMobileHtml();
            break;
        case 'business':
            // 业务组件
            inputHtml = nsComponent.part.getBusinessHtml();
            break;
        case 'sortAtHalfScreen':
            //sjj 20190417 半屏模式下的排序组件
            inputHtml = nsComponent.part.getSortAtHalfScreenHtml();
            break;
        default:
            if (debugerMode) {
    ...
    $.each(data, function(key, value) {
        ...
        switch (key) {
            ...
            case 'map':
                //数组型
                for (var textI = 0; textI < value.length; textI++) {
                    nsComponent.init.map(value[textI]);
                }
                break;
            case 'daterangepicker':
                //数组型
                for (var textI = 0; textI < value.length; textI++) {
                    nsComponent.init.daterangepickerMobile(value[textI]);
                }
                break;
            case 'textBtn':
            ...
            case 'provincelinkSelect':
                if ($.isArray(value)) {
                    for (var p = 0; p < value.length; p++) {
                        ...
                        if(value[p].formSource == "staticData" || value[p].formSource == "halfScreen" || value[p].formSource == "fullScreen" || value[p].formSource == "inlineScreen"){
                            nsUI.provinceselectInput.initMobile(value[p]);
                        }else{
            ...
            case 'radiocheckstring':
                for (var i = 0; i < value.length; i++) {
                    nsComponent.init.radiocheckstring(value[i]);
                }
                break;
            case 'expression':
            ...
            case 'adderSubtracter':
                for (var flowI = 0; flowI < value.length; flowI++) {
                    nsUI.adderSubtracterInput.init(value[flowI]);
                }
                break;
            case 'business':
                for (var flowI = 0; flowI < value.length; flowI++) {
                    nsUI.businessInput.init(value[flowI]);
                }
                break;
            case 'sortAtHalfScreen':
                //sjj 20190417 半屏模式下的排序
                for(var valueI in value){
                    nsComponent.init.sortAtHalfScreen(value[valueI]);
                }
                break;
            default:
                //还有很多form属性 不需要处理
...
nsComponent.edit = function(id, formID, config) {
    ...
    if(usedConfig.sourceRules){
        config.rules = usedConfig.sourceRules;
    }
    if(config.hidden || config.readonly || config.disabled){
        usedConfig.sourceRules = usedConfig.sourceRules ? usedConfig.sourceRules : usedConfig.rules;
        config.rules = '';
    }
    // lyw
    if (typeof(config.hidden) == 'boolean') {
        switch(usedConfig.formSource){
			case 'halfScreen':  // 半屏
			case 'fullScreen':  // 全屏
			case 'inlineScreen':  	// 行内
			case 'staticData':    // 功能
                // 移动端
                if (config.hidden) {
                    usedConfig.$container ? usedConfig.$container.addClass('hide') : '';
                } else {
                    usedConfig.$container ? usedConfig.$container.removeClass('hide') : '';
                }
				break;
			default:
                // pc端
    ...
    switch (formJson.formSource) {
        ...
        case 'staticData':
            if (usedConfig.type == 'hidden') {
                ...
                if(typeof(usedConfig.$container)!="object"){
                    console.error('组件未初始化完成，不可修改');
                    console.error(usedConfig);
                }else{
                    if(formJson.formSource == "halfScreen" || formJson.formSource == "fullScreen" || formJson.formSource == "inlineScreen" || formJson.formSource == "staticData"){
                        usedConfig.$container.removeClass('required');
                        if(usedConfig.rules && (usedConfig.rules.indexOf('required') > -1 || usedConfig.rules.indexOf('radio') > -1 || usedConfig.rules.indexOf('checkbox') > -1)){
                            usedConfig.$container.addClass('required');
                        }
                    }
    ...
    switch (config.type) {
        ...
        case 'map':
            if (config.formSource == 'halfScreen' || // 半屏
                ...
                nsUI.mapInput.setValue(config, value);
            } else {
                // $("#" + id).val(value);
            }
            break;
        ...
        case 'hidden':
            ...
            break;
        case 'radiocheckstring':
    		nsUI.radioCheckString.setValue(config, value);
            break;
        case 'select':
            $("#" + id).val(value);
        ...
        case 'provincelinkSelect':
            if( config.formSource == 'halfScreen' || // 半屏
                ...
                nsUI.provinceselectInput.setValue(config, value);
            }else{
        ...
        case 'uploadImage':
            if( config.formSource == 'halfScreen' || // 半屏
                ...
                if(config.formSource == 'inlineScreen'){
                    nsUI.uploadImageInput.setValue(config, value);
                }
            }else{
