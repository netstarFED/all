...
nsForm.setDefault = function(formJson){
	...
	*/
	...
	//  计算宽度 css计算了所以没用了
	// var fieldShowNum = 0; // 显示的字段数
	// var isHaveMore = false; // 是否存在更多字段
	// for(var arrI=0; arrI<formJson.form.length; arrI++){
	// 	if($.isArray(formJson.form[arrI])){
	// 		for(var arrII=0; arrII<formJson.form[arrI].length; arrII++){
	// 			if(formJson.form[arrI][arrII].mindjetFieldPosition=='field-more'){
	// 				isHaveMore = true;
	// 			}else{
	// 				fieldShowNum ++;
	// 			}
	// 		}
	// 	}else{
	// 		if(formJson.form[arrI].mindjetFieldPosition=='field-more'){
	// 			isHaveMore = true;
	// 		}else{
	// 			fieldShowNum ++;
	// 		}
	...
	// 	fieldShowNum += 1;
	// }
	// var fieldWidth = 100/fieldShowNum + '%';
	// formJson.fieldWidth = fieldWidth;
...
//初始化form表单类型
nsForm.organizaData = {};
nsForm.formInit = function(formJson,formContainer){
	...
	if(formJson.formSource=='inlineScreen'&&data.data.moreFieldArr.length > 0){
		moreHtml = '<div class="mobile-expand-more">'+formJson.moreText+'</div>';
	}
	// 功能模式更多
	if(formJson.formSource=='staticData'){
		moreHtml = '<div class="mobile-expand-more">'+formJson.moreText+'<i class="fa fa-chevron-right"></i></div>';
	}
	if(formContainer){
			...
			nsComponent.mobileFullContainer.show(config, html, function(resConfig, isConfirm){
				...
				if(isConfirm){
					...
					for(var fieldI=0;fieldI<showMoreContainerArr.length;fieldI++){
						if(showMoreContainerArr[fieldI].value.length>0){
...
nsForm.modalInit = function(_formJson){
	...
	if(_formJson.fieldMore.length > 0){
		var expandStr = formJson.fieldMoreTitle ? formJson.fieldMoreTitle : language.common.dialog.showMore;
		moreHtml = '<div class="modal-expand-more" nstype="expand">'+expandStr+'<i class="fa-caret-up"></i></div>';
	}
...
nsForm.getFormJSON = function(formID,isNeedValid){
	if($("#form-"+formID).length==0){
		nsalert('表单ID：'+formID+' 不存在','error');
		return false;
	}
			...
			switch(inputType){
				...
				case 'daterangeRadio':
					...
					if(config.formSource=='halfScreen'||
						...
						formDataJson[inputkey] = config.value?config.value:'';
					}else{
				...
				case 'datetime':
					//日期时间组件
					var $date = $container.find('input.datepicker');
					var $timer = $container.find('input.timepicker');
				...
				case 'codeMirror':
					...
					break;
				case 'numrange':
					var minNum = $("."+inputkey).find("#minNum").val();
					var maxNum = $("."+inputkey).find("#maxNum").val();
					inputValue = {
