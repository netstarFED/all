var NetstarComponentEditor = (function($){
	...
	var getTypeData = {
		...
		'columnFormat-type':{
			...
			subdata:[
				...
				{ id : 'cubesInput' , name : '多维输入', },
				{ id : 'standardInput' , name : '标准值输入', },
				{ id : 'timeUint' , name : '时间单位', },
				{ id : 'buildPdf' , name : '生成/预览pdf', },
				{ id : 'textFieldReplace' , name : 'textField替换', },
				{ id : 'htmlRender' , name : 'html渲染', },
				{ id : 'jsonToList' , name : 'json数据转list表格', },
			],
		},
		'columnFormat-format':{
			id : 'format',
		...
		templateName : {
			...
			subdata : [
				...
				{ id : 'workRecordSimple', name : '简单作业记录模板' },//sjj 20200106
		...
		displayMode : {
			...
			subdata: 	[
				...
				{ text : 'blockList', value : 'blockList'},
			],
		},
		// 下拉组件参数 defaultSelectedIndex
		defaultSelectedIndex : {
			id:'defaultSelectedIndex',
			label:'默认选择',
			type: 		'number',
			column: 	12,
		},
		...
		isReadDefaultWidth : {
			id:'isReadDefaultWidth',
			label:'isReadDefaultWidth',
			type: 		'radio',
			column: 	12,	
			value :  	'true',
			subdata: 	[
		...
		linkDefaultMode : {
			id:'linkDefaultMode',
			label:'linkDefaultMode',
			type: 		'radio',
			column: 	12,	
			value :  	'',
			subdata: 	[
				{ text : 'dialog', value : 'dialog'},
				{ text : 'newtab', value : 'newtab'},
			],
		},
		linkUrlSuffix : {
			id:'linkUrlSuffix',
		...
		isDeleteObjectState : {
			id:'isDeleteObjectState',
			label:'是否删除objectState',
			type: 		'radio',
			column: 	12,	
			value :  	'false',
			subdata: 	[
	...
	// type下拉 内容
	var variableType = {
		string:['text','number','select','radio','checkbox','radio-dict','select-dict','checkbox-dict','business','date','textarea','hidden','provinceselect','map','businessSelect','valuesInput','treeSelect','upload','timeUnit','uploadImage','adderSubtracter','cubesInput','standardInput','password','listSelectInput','selectInput'],
		date:['date', 'dateRangePicker','hidden'],
		number:['text','number','select','radio','checkbox','radio-dict','select-dict','checkbox-dict','business','hidden','provinceselect','businessSelect','valuesInput','treeSelect','timeUnit','adderSubtracter','cubesInput','standardInput','password','listSelectInput','selectInput'],
	...
	var formConfig = {
		...
		uploadImage : ['textField','valueField','isMultiple','fileTypeField','uploadAjaxData', 'visibilityLevel','align','scale','outputFields','innerFields'],
	...
	var container = {
		...
		preview:function(_saveData){
			...
			if(saveData.form){
				...
				var attrUrlArr = ['selectConfig','saveAjax','personAjax','groupAjax','source','search','subdataAjax','getRowData','getFormData','getAjax','saveAjax','produceFileAjax'];
    ...
    var commonTabPanel = {
                ...
                switch(arr[index]){
					...
					case 'columnFormat-format':
						...
						if(	config.data[type].type == 'columnState' ||
							...
							config.data[type].type == 'textFieldReplace'||
							config.data[type].type == 'jsonToList'||
							config.data[type].type == 'buildPdf'
						  ){
							obj.hidden = false;
						}
						break;
					...
					case 'urlSubdata':
					case 'subdataField':
					case 'isAllwaysNewTab':
					case 'isFirstUseRow':
						//sjj 20200109 添加isAlwaysNewTab字段
		...
		isHideColumnTypeData:function(columnFormat, formID, componentId){
			...
			var hideObj = {
				...
				subdataField : true,
				templateName : true,
				isAllwaysNewTab:true,//sjj 20200109
				isSendQueryModel : true,
				isFirstUseRow : true,
			}
			var valueObj = {
				places : '',
			...
			switch(columnFormat){
				case 'columnState':
					...
					break;
				case 'href':
					var showArr = ['url','title','readonly','field','parameterFormat','templateName','isAllwaysNewTab','urlSubdata','subdataField','isSendQueryModel','isFirstUseRow'];//sjj 20200109 添加是否标签页多开属性
					showById(showArr);
					break;
				case 'input':
					dataHidden = false;
	...
	function validationSaveJson(saveJson){
		...
		switch(formatJson.type){
			...
			case 'checkbox':
				...
				break;
			case 'uploadImage':
				returnBool = isHaveFields(formConfig, ['valueField','textField']);
				break;
			case 'uploadSingle':
			case 'upload-single':
		...
		switch(columnType){
			...
			case 'href':
				...
				obj.formatHandler.data = {
					...
					isSendQueryModel : columnFormatObj.isSendQueryModel,
					isFirstUseRow: columnFormatObj.isFirstUseRow,
				}
				break;
			case 'upload':
				obj.formatHandler = $.extend(true,{},columnTypeData[columnType]);
