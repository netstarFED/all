...
nsProjectPagesManager.pages.voList = (function($) {
	...
	function init(_config){
		config = _config;
		if(typeof(NetstarMainPage)=="object" && typeof(NetstarMainPage.config)=="object" && NetstarMainPage.config.topOrgId != '1'){
			isTopLevel = false;
		}
		//服务器事件初始化
	...
	var editorDataSwitch = {
		saveAllElements : {},
		data : {
		},
		// vo名 vo全名 实体名 数据库返回类型 根据数据库返回类型格式化的前端类型 中文名字 英文名字
		souceFieldKey : ['voName','voFullName','entityName','className','variableType','chineseName','englishName'],
		// 获取保存字段(原始)
		getSaveSourceFields : function(fields){
			var source = [];
			for(var i=0; i<fields.length; i++){
				var fieldData = fields[i];
				var sourceConfig = {
					gid: fieldData.gid,
					voName: fieldData.voName,       		// vo名
					voFullName: fieldData.voFullName,		// vo全名
					entityName: fieldData.entityName,		// 实体名
					className: fieldData.className,			// 数据库返回类型
					variableType: fieldData.variableType,	// 根据数据库返回类型格式化的前端类型
					chineseName: fieldData.chineseName,		// 中文名字
					englishName: fieldData.englishName,		// 英文名字
				};
				var sourceData = {
					type : 'source',
		...
		// 获取状态中使用的所有字段
		getStatesFields : function(states){
			var fieldsByStates = {};
			var fieldsByStatesArr = [];
			for(var i=0; i<states.length; i++){
				var state = states[i];
				var fieldNames = state.fieldNames;
				for(var j=0; j<fieldNames.length; j++){
					var staField = $.extend(true, {}, fieldNames[j]);
					staField.voName = state.voName;
					staField.voFullName = state.voFullName;
					fieldsByStates[staField.gid] = staField;
					fieldsByStatesArr.push(staField);
				}
				
			}
			return {
				fieldsByGid : fieldsByStates,
				fields : fieldsByStatesArr
			};
		},
		// 获取保存字段(编辑) 
		// 同时获取状态中的字段配置 原因：有可能状态编辑了字段，原始字段未编辑；
		getSaveFields : function(sourceData, fields, states){
			var saveFields = [];
			for(var i=0; i<sourceData.length; i++){
				var childrenId = sourceData[i].id;
				var fieldCon = {
					form : {},
					table : {},
					displayType : 'all',
				...
				if(fields[i].type){
					if(fields[i].editBtnName == "edit2"){
						fieldCon = NetstarComponentEditor.getFormatData(fields[i]);
					}else{
						fieldCon = nsComponentEditor.getFormatData(fields[i]);
					}
				}
				// var fieldConStr = JSON.stringify(fieldCon);
				var fieldConStr = fieldCon;
				var obj = {
					type : 'field',
					config : fieldConStr,
					children : [childrenId],
		...
		// 获取字段gid与id的对应关系
		getGidToId : function(resFields, fields){
			var gidToId = {};
			for(var i=0; i<resFields.length; i++){
				gidToId[fields[i].gid] = resFields[i].id;
			}
			return gidToId;
		},
		// 获取保存状态(默认)
		getDefaultStates : function(resFields){
			var ids = [];
			for(var i=0; i<resFields.length; i++){
				ids.push(resFields[i].id);
			}
			var obj = {
				type : 'fieldGroup',
				config : '',
        ...
        // 比较对象获取不同的属性 比较默认字段配置和状态编辑字段配置
        getCompareField : function(current, source){
            var res = {};
			var souceFieldKey = editorDataSwitch.souceFieldKey;
            function run(souObj, currObj, save){
                for(var key in currObj){
					if(souceFieldKey.indexOf(key) > -1){
						continue;
					}
                    // 特殊字段特殊处理
                    switch(key){
		...
		// 比较默认字段配置和状态编辑字段配置
		getCompareField2 : function(edit, def){
			var res = {};
			var isEdit = false;
			var souceFieldKey = editorDataSwitch.souceFieldKey;
			for(var typeName in edit){
				// typeName : displayType(all/table/form)/type
				if(typeof(edit[typeName]) != "object"){
					continue;
				}
				// typeName : form/table
				var editCon = edit[typeName];
				var defCon = def[typeName];
				res[typeName] = {};
				for(var key in editCon){
					if(souceFieldKey.indexOf(key) == -1 && editCon[key] != defCon[key]){
						res[typeName][key] = editCon[key];
						isEdit = true;
					}	
				}
			}
		...
		// 获取保存状态
		getSaveStates : function(states, resFieldGroup, gidToId, resFields){
			var saveStates = [];
			var resFieldsById = {};
			for(var i=0; i<resFields.length; i++){
				resFieldsById[resFields[i].id] = resFields[i];
			}
			for(var i=0; i<states.length; i++){
				var stateConfig = states[i];
				var fieldNames = stateConfig.fieldNames;
				var stateFields = {};
				var sort = [];
				for(var j=0; j<fieldNames.length; j++){
					stateFields[gidToId[fieldNames[j].gid]] = {
						"objectState": NSSAVEDATAFLAG.ADD,
					};
					sort.push(gidToId[fieldNames[j].gid]);
				}
				var editFields = stateConfig.editFields;
				if(editFields){
					for(var gid in editFields){
						var editField = editFields[gid];
						var field = resFieldsById[gidToId[gid]];
						var defCon = field.config;
						if(typeof(field.config) == "string"){
							defCon = JSON.parse(field.config);
						}
						// 比较
						var editCon = {}
						if(editField.editBtnName == "edit2"){
							editCon = NetstarComponentEditor.getFormatData(editField);
						}else{
							editCon = nsComponentEditor.getFormatData(editField);
						}
						var compareField = editorDataSwitch.getCompareField(editCon, defCon);
						if(compareField){
							stateFields[gidToId[gid]] = {
		...
		// 获取所有字段
		getGidFields : function(fields){
			var fieldsByGid = {};
			for(var i=0; i<fields.length; i++){
				fieldsByGid[fields[i].gid] = fields[i];
			}
			return fieldsByGid;
		},
		// 获取当前配置的所有字段 vos中config2中字段配置
		getFields : function(fields, vos){
			var resFields = $.extend(true, [], fields);
			function setConField(conFields){
				for(var gid in conFields){
					for(var i=0; i<resFields.length; i++){
						if(resFields[i].gid == gid){
							resFields[i] = conFields[gid];
							break;
						}
					}
				}
			...
			for(var i=0; i<vos.length; i++){
				var config2 = vos[i].config2;
				if(typeof(config2) == "undefined" ||config2 == null || config2 == "null" || config2 === ''){
					continue;
				}
				if(typeof(config2) == "string"){
					config2 = JSON.parse(config2);
				}
				var conFields = config2.fields;
				setConField(conFields);
			}
			return resFields;
		},
		// 获取所有状态 一个vos只有一个默认状态
		getStates : function(vos){
			var states = [];
			for(var i=0; i<vos.length; i++){
				var config2 = vos[i].config2;
				if(config2 == null || config2 == "null" || config2 === ''){
					continue;
				}
				if(typeof(config2) == "string"){
					config2 = JSON.parse(config2);
				}
				var conStates = config2.states;
				states = states.concat(conStates);
			}
			// 设置默认状态
			var entityName = vos[0].entityName;
			var fieldNames = [];
			var gidArr = [];
			for(var i=0; i<states.length; i++){
				var _fieldNames = states[i].fieldNames;
				for(var j=0; j<_fieldNames.length; j++){
					if(gidArr.indexOf(_fieldNames[j].gid) == -1){
						gidArr.push(_fieldNames[j].gid);
						fieldNames.push(_fieldNames[j]);
					}
				}
			}
			var defState = {
		...
		getSource : function(fields){
			...
			for(var i=0; i<fields.length; i++){
				...
				for(var j=0; j<souceFieldKey.length; j++){
					sourceConfig[souceFieldKey[j]] = fieldData[souceFieldKey[j]];
				}
				source.push(sourceConfig);
			}
			return source;
		},
		// 同够上次保存数据格式化当前数据 暂时把id存储为 NETSTARSAVEID
		getFormatElements : function(){
			var data = editorDataSwitch.data;
			var fields = data.fields;
			var states = data.states;
			var sources = data.sources;
			var sourceElements = data.sourceElements;
			var elements = {
				source : sources,
				field : fields,
				fieldGroup : {
			...
			// 格式化原始字段
			if(typeof(sourceElements.source) == "undefined" || $.isEmptyObject(sourceElements.source)){
				return elements;
			}
			for(var i=0; i<sources.length; i++){
				for(var j=0; j<sourceElements.source.length; j++){
					var souCon = sourceElements.source[j].config;
					if(typeof(souCon) == "string"){
						souCon = JSON.parse(souCon);
					}
					if(souCon.gid == sources[i].gid){
						sources[i].NETSTARSAVEID = sourceElements.source[j].id;
					}
				}
			}
			// 格式化编辑字段
			if(typeof(sourceElements.field) == "undefined" || $.isEmptyObject(sourceElements.field)){
				return elements;
			}
			for(var i=0; i<fields.length; i++){
				for(var j=0; j<sourceElements.field.length; j++){
					var fieldCon = sourceElements.field[j].config;
					if(typeof(fieldCon) == "string"){
						fieldCon = JSON.parse(fieldCon);
					}
					if(fieldCon.gid == fields[i].gid){
						fields[i].NETSTARSAVEID = sourceElements.field[j].id;
					}
				}
			}
			// 格式化默认状态
			if(typeof(sourceElements.fieldGroup) == "undefined" || $.isEmptyObject(sourceElements.fieldGroup)){
				return elements;
			}
			// elements.fieldGroup.id = sourceElements.fieldGroup[0].id;
			// 格式化状态
			if(typeof(sourceElements.state) == "undefined" || $.isEmptyObject(sourceElements.state)){
				return elements;
			}
			for(var i=0; i<states.length; i++){
				for(var j=0; j<sourceElements.state.length; j++){
					var stateCon = sourceElements.state[j].config;
					if(typeof(stateCon) == "string"){
						stateCon = JSON.parse(stateCon);
					}
					// 如果没有config表示状态为测试配置未配置config 直接跳过
					if(typeof(stateCon.config) != "object"){
						continue;
					}
					// 默认状态的gid是变化的所以根据只有实例名判断
					if(stateCon.config.entityName == states[i].entityName){
						if(stateCon.config.englishName == states[i].englishName && states[i].englishName == "default"){
							states[i].NETSTARSAVEID = sourceElements.state[j].id;
							continue;
						}
					}
					if(stateCon.config.gid == states[i].gid){
						states[i].NETSTARSAVEID = sourceElements.state[j].id;
						elements.fieldGroup.id = sourceElements.state[j].childrenIds;
					}
				}
			}
			return elements;
		...
		// 判断状态中是否有该字段
		getIsUseField : function(field){
			var states = editorDataSwitch.data.states;
			var isHave = false;
			for(var i=0; i<states.length; i++){
				var state = states[i];
				var fieldNames = state.fieldNames;
				for(var j=0; j<fieldNames.length; j++){
					if(fieldNames[j].gid == field.gid){
						isHave = true;
						break;	
					}
				}
		...
		saveDataFuncs : {
			// 获原始字段 
			getSource : function(){
				var elements = editorDataSwitch.data.elements;
				var source = elements.source;
				var saveSource = [];
				for(var i=0; i<source.length; i++){
					var fieldData = $.extend(true, {}, source[i]);
					var sourceData = {
						type : 'source',
					}
					if(fieldData.NETSTARSAVEID){
						sourceData.id = fieldData.NETSTARSAVEID;
						delete fieldData.NETSTARSAVEID;
					}
					sourceData.config = JSON.stringify(fieldData);
					saveSource.push(sourceData);
				}
				return saveSource;
			},
			// 获取编辑字段 包括编辑的和状态中含有的未编辑的
			getFields : function(){
				var elements = editorDataSwitch.data.elements;
				var sourceData = editorDataSwitch.data.resElement.source;
				var fields = elements.field;
				var saveFields = [];
				var saveFieldsConfig = [];
				for(var i=0; i<sourceData.length; i++){
					var fieldEdit = $.extend(true, {}, fields[i]);
					var childrenId = sourceData[i].id;
					var obj = {
						type : 'field',
						children : [childrenId],
					}
					if(fieldEdit.NETSTARSAVEID){
						obj.id = fieldEdit.NETSTARSAVEID;
						delete fieldEdit.NETSTARSAVEID;
					}
					var fieldConfig = {
						form : {},
						table : {},
					...
					if(fieldEdit.type){
						if(fieldEdit.editBtnName == "edit2"){
							fieldConfig = NetstarComponentEditor.getFormatData(fieldEdit);
						}else{
							fieldConfig = nsComponentEditor.getFormatData(fieldEdit);
						}
					}else{
						// 没有编辑的字段判断在状态中是否应用
						var isSave = editorDataSwitch.getIsUseField(fieldEdit);
						if(!isSave){
							continue;
						}
					}
					fieldConfig.gid = fieldEdit.gid;
					obj.config = JSON.stringify(fieldConfig);
					saveFields.push(obj);
					saveFieldsConfig.push(fieldEdit);
				}
				return {
			...
			getFieldGroup : function(){
				var elements = editorDataSwitch.data.elements;
				var fieldGroup = elements.fieldGroup;
				var field = editorDataSwitch.data.resElement.field;
				var children = [];
				for(var i=0; i<field.length; i++){
					children.push(field[i].id);
				}
			...
			// 获取状态
			getStates : function(){
				var gidToId = editorDataSwitch.data.gidToId;
				var elements = editorDataSwitch.data.elements;
				var resElement = editorDataSwitch.data.resElement;
				var states = elements.state;
				var resFields = resElement.field;
				var resFieldGroup = resElement.fieldGroup;
				var saveStates = [];
				var resFieldsById = {};
				var groupChildren = resFieldGroup[0].children;
				var entityName = '';
				for(var i=0; i<resFields.length; i++){
					resFieldsById[resFields[i].id] = resFields[i];
				}
				for(var i=0; i<states.length; i++){
					var state = $.extend(true, {}, states[i]);
					var obj = {
						type : 'state',
						children : [resFieldGroup[0].id],
					}
					if(state.NETSTARSAVEID){
						obj.id = state.NETSTARSAVEID;
						delete state.NETSTARSAVEID;
					}
					var fieldNames = state.fieldNames;
					var stateFields = [];
					var sort = [];
					var stateFieldsIds = [];
					var stateFieldsIdsObj = {};
					for(var j=0; j<fieldNames.length; j++){
						// stateFields[gidToId[fieldNames[j].gid]] = {
						// 	"objectState": NSSAVEDATAFLAG.ADD,
						// };
						stateFields.push({
							id : gidToId[fieldNames[j].gid],
							"objectState": NSSAVEDATAFLAG.ADD,
						})
						stateFieldsIds.push(gidToId[fieldNames[j].gid]);
						sort.push(gidToId[fieldNames[j].gid]);
						stateFieldsIdsObj[gidToId[fieldNames[j].gid]] = j;
					}
					var editFields = state.editFields;
					if(editFields){
						for(var gid in editFields){
							var editField = editFields[gid];
							var field = resFieldsById[gidToId[gid]];
							...
							if(compareField){
								...
								// 	"config" : compareField,
								// }
								stateFields[stateFieldsIdsObj[gidToId[gid]]] = {
									id : gidToId[gid],
									"objectState" : NSSAVEDATAFLAG.EDIT,
									"config" : compareField,
								}
					...
					// 存储删除字段 即添加NSSAVEDATAFLAG.DELETE
					for(var j=0; j<groupChildren.length; j++){
						if(stateFieldsIds.indexOf(groupChildren[j]) == -1){
							stateFields.push({
								id : groupChildren[j],
					...
					delete state.editFields;
					delete state.fieldNames;
					var config = {
						// sort : sort,
						fields : stateFields,
						config : state,
				...
				// saveStates.push(defState);
				return saveStates;
			},
		},
		save : function(processContent, sourceElements){
			var vos = processContent.vo;
			// 所有字段，当前配置
			var fields = editorDataSwitch.getFields(processContent.field, vos);
			// 所有状态
			var states = editorDataSwitch.getStates(vos);
			// 原始字段
			var sources = editorDataSwitch.getSource(fields);
			var num = 0;
			editorDataSwitch.data = {
				fields : fields,
				states : states,
				sources : sources,
			...
			editorDataSwitch.data.elements = editorDataSwitch.getFormatElements();
			// 获取保存的原始字段
			var saveSources = editorDataSwitch.saveDataFuncs.getSource();
			// 原始字段
			NetstarEditorAjax.savePageData(saveSources, {}, function(resData){
				// for(var i=0; i<resData.length; i++){
				// 	resData[i].id = resData[i].id ? resData[i].id : num ++;
				// }
				editorDataSwitch.data.resElement.source = resData;
				var saveFieldsObj = editorDataSwitch.saveDataFuncs.getFields();
				var saveFields = saveFieldsObj.saveFields;
				editorDataSwitch.data.saveFields = saveFieldsObj.saveFieldsConfig;
				// 编辑字段
				NetstarEditorAjax.savePageData(saveFields, {}, function(resData2){
					// for(var i=0; i<resData2.length; i++){
					// 	resData2[i].id = resData2[i].id ? resData2[i].id : num ++;
					// }
					editorDataSwitch.data.resElement.field = resData2;
					editorDataSwitch.data.gidToId = editorDataSwitch.getGidToId(resData2, saveFieldsObj.saveFieldsConfig);
					var defaultState = editorDataSwitch.saveDataFuncs.getFieldGroup();
					// 默认状态
					NetstarEditorAjax.savePageData(defaultState, {}, function(resData3){
						// for(var i=0; i<resData3.length; i++){
						// 	resData3[i].id = resData3[i].id ? resData3[i].id : num ++;
						// }
						editorDataSwitch.data.resElement.fieldGroup = resData3;
						// var saveStates = editorDataSwitch.getSaveStates(states, resData3, gidToId, resData2);
						var saveStates = editorDataSwitch.saveDataFuncs.getStates();
						// 状态
		...
		// 获取已经保存的数据
		saveBySaveElement : function(callBackFunc){
			var arr = ["source", "field", "fieldGroup", "state"];
			var index = 0;
			for(var i=0; i<arr.length; i++){
				...
				NetstarEditorAjax.getList(ajaxConfig, function(resDataState, plusData){
					editorDataSwitch.saveAllElements[plusData.type] = resDataState;
					index ++;
					if(arr.length == index){
						var processContent = nsProjectPagesManager.pages.voList.voMapTable.processContent;
						editorDataSwitch.save(processContent, editorDataSwitch.saveAllElements);
					}
				});
			}
		},
	...
	var listTable = {
		...
		columnBtns:[
			...
			},
			{
				...
				"数据转换":function(rowDatas){
					var processContent = nsProjectPagesManager.pages.voList.voMapTable.processContent;
					console.warn(processContent);
					// editorDataSwitch.save(processContent, {source:[{id:'niohao',config:{gid:'37960669-cb2d-9b56-ec73-6d707ec9a075'}}]});
					// editorDataSwitch.save(processContent, {});
					editorDataSwitch.saveBySaveElement()
				}
			},
			/*,{
				"配置":function(data){
		...
		init:function(){
			...
			var columnBtns = $.extend(true, [], this.columnBtns);
			if(!isTopLevel){
				btnsConfig.selfBtn.shift();
				columnBtns.shift();
			}
			//添加按钮  
			columnConfig[columnConfig.length - 1].formatHandler.data =  columnBtns;;
	...
	var voMapTable = {
		...
		columnBtns:{
			...
			state:[
				...
				{
					"顺序":function(rowDatas){
						var values = $.extend(true, {}, rowDatas.rowData);
						delete values.id;
						delete values.btns;
						values.index = rowDatas.rowIndexNumber;
						setStateDialog.editOrder(values);
					}
				},
				{
					"删除":function(rowDatas){
							...
							var object = {
								...
								confirmHandler:function(saveData){
									if(saveData){
										...
										if(typeof(prevVo.config2) == 'object' && typeof(vo.config2.fields) == "object"){
											if(typeof(prevVo.config2.fields[saveData.gid]) == "object"){
												saveData.objectState = NSSAVEDATAFLAG.EDIT;
											}else{
		...
		getVoMapArraysById:function(id, callBackFunc){
			...
			server.voMap.getListById(id, function(resList){
				...
				server.voMap.mindMap(id, function(mindMapObj){
					...
					var _resList = $.extend(true, [], resList);
					var prevArrays = _this.getVoMapFormatArraysByRes(_resList,entityName, false, true);
					voMapTable.prevProcessContent = prevArrays;
					if(typeof(callBackFunc)=='function'){
						callBackFunc(arrays);
						//关联页面拼接
					}
	...
	var setStateDialog = {
		...
		fieldOrederConfig : {
			dialogId : "dialog-state-field-order",
			tableId : 'dialog-state-field-order-table',
			tableContainerId : 'dialog-state-field-order-container',
			dialog : {
				id: 				"dialog-state-field-order",
				title: 				"编辑字段顺序",
				size: 				"m",
				form: [
					{
						...
						html: '<div id="dialog-state-field-order-container">'
								+'<div class="col-sm-12 main-panel">'
									+'<div class="panel panel-default">'
										+'<div class="panel-body">'
											+'<div class="table-responsive">'
												+'<table cellspacing="0" class="table table-singlerow table-hover table-bordered table-striped" id="dialog-state-field-order-table">'
												+'</table>'
											+'</div>'
										+'</div>'
									+'</div>'
			...
			columnBtns:[
				{'上移':function(rowData){
						var tableId = setStateDialog.fieldOrederConfig.tableId;
						...
						if(rowData.rowIndexNumber==0){
							...
							nsTable.originalConfig[tableId].dataConfig.dataSource = tableData;
		...
		// 获取字段排序数据
		getEditOrderData : function(sourceValues){
			var dialogId = setStateDialog.fieldOrederConfig.dialogId;
			var values = nsForm.getFormJSON(dialogId);
			if(!values){
				return false;
			}
			var tableId = setStateDialog.fieldOrederConfig.tableId;
			var tableData = baseDataTable.allTableData(tableId);
			var fields = [];
			var editFields = sourceValues.editFields;
			for(var i=0; i<tableData.length; i++){
				if(tableData[i].variableType == 'other'){
					editFields[tableData[i].gid].fieldIndex = tableData[i+1] ? tableData[i+1].gid : tableData[i-1].gid;
					continue;
				}
				var field = {
					id: tableData[i].gid,
					text: tableData[i].chineseName,
				}
				fields.push(field);
			}
			if(!$.isEmptyObject(editFields)){
				values.editFields = editFields;
			}
		...
		// 获取需要排序的字段
		getOrderFields : function(values){
			var fields = $.extend(true, [], values.fieldNames);//
			var fieldsObj = {};
			for(var i=0; i<fields.length; i++){
				fieldsObj[fields[i].gid] = fields[i];
			}
			// 添加新增的字段
			var editFields = values.editFields ? values.editFields : {};
			var englishNameNum = 0;
			for(var gid in editFields){
				if(editFields[gid].variableType == 'other'){
					var afterField = fieldsObj[editFields[gid].fieldIndex];
					var editCon = {
						gid: gid,
						chineseName: editFields[gid].label,
						englishName: 'addFieldName' + englishNameNum,
						mindjetIndexState: afterField.mindjetIndexState-0.5,
						variableType : editFields[gid].variableType,
					}
					fields.push(editCon);
					englishNameNum ++;
				}
		...
		editOrder : function(_values){
			if(_values.voId){
				...
				values.index = _values.index;
				var _this = this;
				var fieldOrederDefConfig = _this.fieldOrederConfig; // 字段排序的默认配置
				var dialogConfig = $.extend(true, {}, fieldOrederDefConfig.dialog); // 弹框配置
				dialogConfig.btns[0] = {
					text:'修改',
					handler:function(){
						var editValues = _this.getEditOrderData(values);
						if(editValues){
							editValues.type = 'edit';
							_this.stateBtnsFunc(editValues, values);
						}else{
				...
				nsForm.resetValues(values, dialogConfig);
				// 表格数据
				var tableDataSource = _this.getOrderFields(values);
				var dataConfig = $.extend(true,{},fieldOrederDefConfig.data);
				var tableId = setStateDialog.fieldOrederConfig.tableId;
				var tableContainerId = setStateDialog.fieldOrederConfig.tableContainerId;
				dataConfig.tableID = tableId;
				dataConfig.dataSource = tableDataSource;
				var columnConfig = $.extend(true,[],fieldOrederDefConfig.column);
				columnConfig[columnConfig.length-1].formatHandler.data = fieldOrederDefConfig.columnBtns;
				var uiConfig = $.extend(true,{},fieldOrederDefConfig.ui);
				dialogConfig.shownHandler = function(){
					if($('#' + tableId).children().length>0){
						uiConfig.$container = $('#' + tableContainerId);
					}
	...
	var voDataManager = {
		...
		getDetailById:function(id, callBackFunc){
			...
			NetStarUtils.ajax(getDetailAjax, function(res){
				//保存用于省略访问
				this.detailRes = {
					id:id,
					res:res,
