	...
	nsVals.ajax(listAjax,function(res,plusData){
			...
			if(handlerObj.afterHandler){
				if(runningConfig.dataSrc){
					/**lxh 添加plusData */
					var returnData;
					if(typeof(returnObjectState)=='number'){
						if(typeof(res[runningConfig.dataSrc])=='undefined'){
							res.objectState = returnObjectState;
							returnData = res;
						}else{
							res[runningConfig.dataSrc].objectState = returnObjectState;
							returnData = res[runningConfig.dataSrc];
						}
					}else{
						returnData = res[runningConfig.dataSrc];
					}
					handlerObj.afterHandler(returnData,plusData.plusData);
				}else{
					if(typeof(returnObjectState)=='number'){
						res.objectState = returnObjectState;
					}
...
//--------------------------------------------------wxk公共弹窗------------------------------------------------------------		
    ...
    function confirmCommon(callback,obj){
		...
		if(!$.isEmptyObject(obj.rowData)){
			confirmdata.value = obj.rowData;
		}
    ...
    function loadPageCommon(callback,obj){
		...
		if($('.mobile-menu-buttons').length > 0){
			$('.mobile-menu-buttons').remove();
		}
		...
		if(typeof(configObj.value)=='boolean'){
			if(configObj.value == false){return;}
		}
