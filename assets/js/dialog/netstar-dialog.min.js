popupBox.initShow=function(e){if($(".modal-backdrop").length>0){}else{popupBox.init(e);var t="m";if(typeof e.size!="undefined"){t=e.size}popupBox.show("#"+e.id,t)}};popupBox.init=function(e,t){this.btnHandlerArr=[];this.config=e;var o="";var r;if(typeof e.title=="undefined"){r=""}else{r=e.title}if(typeof e.form=="undefined"){e.form=[]}if(typeof e.btns=="undefined"){e.btns=[]}var n="";var a=[];var i=[];var l=[];var f=[];var d=[];var s=[];var p=[];var m={};var c="";var u="";var h="";var v=[];var g={};var b={};var y=[];for(var x=0;x<e.form.length;x++){if(typeof e.form[x].length=="undefined"){h="modal-body";var S="";if(typeof e.form[x].id!="undefined"){n+=commonConfig.component(e.form[x],e.id,"modal",e.size);a.push(e.form[x]);if(e.form[x].type=="date"){var P={id:e.form[x].id,format:e.form[x].format,changeHandler:e.form[x].changeHandler};i.push(P)}if(e.form[x].type=="select"){y.push(e.form[x])}if(e.form[x].type=="tree-select"){m[e.form[x].id]=e.form[x]}if(e.form[x].type=="datetime"){l.push(e.form[x])}if(e.form[x].type=="select2"){s.push(e.form[x])}if(e.form[x].type=="typeahead"){p.push(e.form[x])}if(e.form[x].type=="upload_single"){b[e.form[x].id]=e.form[x]}if(typeof e.form[x].changeHandler!="undefined"){d.push(e.form[x])}if(e.form[x].type=="upload"){g=e.form[x];v.push(e.form[x])}else if(e.form[x].type=="table"){v.push(e.form[x])}}else{if(typeof e.form[x].html!="undefined"){n+=e.form[x].html}else if(typeof e.form[x].note!="undefined"){n+="<blockquote>"+"<p>"+e.form[x].note+"</p>"+"</blockquote>"}}}else{u="panel-form";h="panel-body";for(var z=0;z<e.form[x].length;z++){if(typeof e.form[x][z].id=="undefined"){if(typeof e.form[x][z].html!="undefined"){n+=e.form[x][z].html}else if(typeof e.form[x][z].note!="undefined"){n+="<blockquote>"+"<p>"+e.form[x][z].note+"</p>"+"</blockquote>"}else if(typeof e.form[x][z].element!="undefined"){if(e.form[x][z].element=="hr"){n+='<div class="col-xs-12 element-hr"><hr></div><div class="clearfix"></div>'}else if(e.form[x][z].element=="label"){n+='<label class="grouplable">'+e.form[x][z].label+"</label>"}else if(e.form[x][z].element=="br"){n+='<div class="col-xs-12 element-br"></div><div class="clearfix"></div>'}else if(e.form[x][z].element=="title"){n+='<div class="col-xs-12 element-title"><label>'+'<i class="fa fa-arrow-circle-down"></i> '+e.form[x][z].label+"</label></div>"}}}else{a.push(e.form[x][z]);if(e.form[x][z].type=="date"){var P={id:e.form[x][z].id,format:e.form[x][z].format,changeHandler:e.form[x][z].changeHandler};i.push(P)}if(e.form[x][z].type=="select"){y.push(e.form[x][z])}if(e.form[x][z].type=="tree-select"){m[e.form[x][z].id]=e.form[x][z]}if(e.form[x][z].type=="datetime"){l.push(e.form[x][z])}if(e.form[x][z].type=="select2"){s.push(e.form[x][z])}if(e.form[x][z].type=="typeahead"){p.push(e.form[x][z])}if(typeof e.form[x][z].changeHandler!="undefined"){d.push(e.form[x][z])}if(e.form[x][z].type=="upload_single"){b[e.form[x][z].id]=e.form[x][z]}if(e.form[x][z].type=="upload"){g=e.form[x][z];v.push(e.form[x][z])}else if(e.form[x][z].type=="table"){v.push(e.form[x][z])}n+=commonConfig.component(e.form[x][z],e.id,"form",e.size)}}}}var B="";for(var H=0;H<e.btns.length;H++){if(typeof e.btns[H].handler!="undefined"){f.push(e.btns[H])}B+=commonConfig.getBtn(e.btns[H],"modal",H);if(typeof e.btns[H].handler=="function"){this.btnHandlerArr.push(e.btns[H].handler)}else{this.btnHandlerArr.push("")}}var w=typeof e.note=="undefined"?"":'<small> <i class="fa fa-angle-double-right"></i> '+e.note+" </small>";var F="";if(r!=""){F='<div class="modal-header">'+'<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'+'<h4 class="modal-title">'+r+w+"</h4>"+"</div>"}var C,D;var k=true;if(e.form.length==1){if(e.form[0].html){k=false}}if(k){C='<form class="form-horizontal" role="form" id="form-'+e.id+'" method="get" action="">';D="</form>"}else{C="";D=""}o+='<div id="'+e.id+'" class="modal fade modal-ns">'+'<div class="modal-dialog">'+'<div class="modal-content '+u+'">'+F+'<div class="'+h+'">'+C+'<div class="row row-close fillbg">'+n+"</div>"+D+"</div>"+'<div class="modal-footer">'+B+'<button type="button" class="btn btn-white" data-dismiss="modal"><i class="fa fa-ban"></i> 取消</button>'+"</div>"+"</div>"+"</div>"+"</div>";$("#placeholder-popupbox").html(o);for(var I=0;I<this.btnHandlerArr.length;I++){var E=$("#"+e.id+" .modal-footer .btn")[I];if(typeof f[I].handler=="function"){$(E).on("click",function(){var e=parseInt($(this).attr("fid"));var t=popupBox.btnHandlerArr[e];t()})}}if(a.length>0){formPlane.validateForm(e.id,a,"dialog")}$("#form-"+e.id).on("keydown",function(e){if(e.keyCode==13){return false}});if(v.length>0){popupBox.initTable(v)}if(!$.isEmptyObject(b)){formPlane.componentUpload(e.id,b)}if(!$.isEmptyObject(m)){treeUI.componentSelectPlane(e.id,m)}if(!$.isEmptyObject(g)){var T=1;var A=0;$example_dropzone_filetable=$("#"+g.id);formPlane.dropzoneGetFile={};formPlane.dropzoneGetFile["advancedDropzone"]={};formPlane.dropzoneFile={};formPlane.dropzoneFile["advancedDropzone"]={};var M=g.isAllowFiles?Number(g.isAllowFiles):1;example_dropzone=$("#advancedDropzone").dropzone({url:g.uploadsrc,maxFiles:M,dictMaxFilesExceeded:"您一次最多只能上传{{maxFiles}}个文件",addRemoveLinks:true,dictInvalidFileType:"不支持上传的格式",dictResponseError:"文件上传失败",dictInvalidFileType:"文件名和类型不匹配",autoProcessQueue:true,init:function(){var e=this;formPlane.dropzoneFile["advancedDropzone"]=e},addedfile:function(e){if(T==1){$example_dropzone_filetable.find("tbody").html("")}var t=parseInt(e.size/1024,10);t=t<1024?t+" KB":parseInt(t/1024,10)+" MB";var o=$('<tr>\t\t\t\t\t\t\t\t<th class="text-center">'+T++ +"</th>\t\t\t\t\t\t\t\t<td>"+e.name+'</td>\t\t\t\t\t\t\t\t<td><div class="progress progress-striped"><div class="progress-bar progress-bar-warning"></div></div></td>\t\t\t\t\t\t\t\t<td>'+t+"</td>\t\t\t\t\t\t\t\t<td>Uploading...</td>\t\t\t\t\t\t\t</tr>");$example_dropzone_filetable.find("tbody").append(o);e.fileEntryTd=o;e.progressBar=o.find(".progress-bar")},removedfile:function(e){A--;if(A<0){A=0;formPlane.dropzoneGetFile["advancedDropzone"]={}}},uploadprogress:function(e,t,o){e.progressBar.width(t+"%")},success:function(e,t){e.fileEntryTd.find("td:last").html('<span class="text-success">上传成功</span>');e.progressBar.removeClass("progress-bar-warning").addClass("progress-bar-success");var o='<td class="td-button">'+'<button type="button" class="btn btn-warning  btn-icon" fid="'+A+'" nstype="file-upload">'+'<i class="fa-trash"></i>'+"</button>"+"</td>";e.fileEntryTd.find("td:last").after(o);if(typeof g.changeHandler!="undefined"){var r=g.changeHandler;r(t,e)}formPlane.dropzoneGetFile["advancedDropzone"][A]=e;A++;e.fileEntryTd.find('button[nstype="file-upload"]').on("click",function(){var e=$(this).attr("fid");var t=formPlane.dropzoneGetFile["advancedDropzone"][e];if(t){formPlane.dropzoneFile["advancedDropzone"].removeFile(t)}})},error:function(e,t){e.fileEntryTd.find("td:last").closest("tr").remove();this.removeFile(e);nsalert(t)}});$("#advancedDropzone").css({minHeight:200})}if(i.length>0){formPlane.componentDate(e.id,i)}if(l.length>0){formPlane.componentDatetime(e.id,l)}if(d.length>0){formPlane.componentHandler(e.id,d)}if(p.length>0){formPlane.componentTypeahead(e.id,p)}$(".form-item.select select").selectBoxIt().on("open",function(){$(this).data("selectBoxSelectBoxIt").list.perfectScrollbar()});if(s.length>0){formPlane.componentSelect2(e.id,s)}if(y.length>0){formPlane.validateSelectPlane(e.id,y)}};popupBox.initTable=function(e){var t={};var o={};var r={};for(var n=0;n<e.length;n++){if(typeof e[n].id=="undefined"){nsalert("表格ID未定义");return false}if(typeof e[n].src=="undefined"&&typeof e[n].dataSource=="undefined"){nsalert("表格数据来源未定义");return false}var a="GET";if(typeof e[n].srctype!="undefined"){a=e[n].srctype}var i="data";if(typeof e[n].dataSrc!="undefined"){i=e[n].dataSrc}var l="搜索";if(typeof e[n].searchTitle!="undefined"){l=e[n].searchTitle}var f="";if(typeof e[n].searchPlaceholder!="undefined"){f=e[n].searchPlaceholder}var d=false;if(typeof e[n].scrollX!="undefined"){d=e[n].scrollX}var s=e[n].column;for(var p=0;p<s.length;p++){if(s[p].removeHandler){s[p]={title:"操作",formatHandler:{type:"upload",data:s[p].removeHandler}}}}o.columns=s;t={tableID:e[n].id,src:e[n].src,type:a,data:e[n].data,dataSrc:i,isServerMode:typeof e[n].isServerMode=="undefined"?false:e[n].isServerMode,isSearch:typeof e[n].isSearch=="undefined"?false:e[n].isSearch,isPage:typeof e[n].isPage=="undefined"?false:e[n].isPage,dataSource:e[n].dataSource,info:typeof e[n].info=="undefined"?true:e[n].info,isLengthChange:e[n].isLengthChange?e[n].isLengthChange:true};r={searchTitle:l,searchPlaceholder:f,isSelectColumns:e[n].isSelectColumns?e[n].isSelectColumns:false,isAllowExport:typeof e[n].isAllowExport=="undefined"?false:e[n].isAllowExport,pageLengthMenu:e[n].pageLengthMenu?e[n].pageLengthMenu:["auto",20,50,"all"],auto:e[n].auto?e[n].auto:false,isMulitSelect:typeof e[n].isMulitSelect=="undefined"?false:e[n].isMulitSelect,isSingleSelect:typeof e[n].isSingleSelect=="undefined"?false:e[n].isSingleSelect,scrollX:d,scrollY:e[n].scrollY?e[n].scrollY:false,isorder:e[n].isorder?e[n].isorder:false,dragWidth:e[n].dragWidth?e[n].dragWidth:false,orderBtn:e[n].orderBtn?e[n].orderBtn:"[]"};if(typeof e[n].onSingleSelectHandler!="undefined"){r.onSingleSelectHandler=e[n].onSingleSelectHandler}if(typeof e[n].onDoubleSelectHandler!="undefined"){r.onDoubleSelectHandler=e[n].onDoubleSelectHandler}baseDataTable.init(t,o.columns,r)}};popupBox.show=function(e,t){var o=arguments[1]?arguments[1]:"m";this.modal=$(e).modal("show",{backdrop:"fade"});if(!$(e).hasClass("custom-width")){$(e).addClass("custom-width")}var r="600px";switch(o){case"s":r="400px";break;case"m":r="600px";break;case"b":r="800px";break;case"f":r="100%";break;default:r=t;break}$(e+" .modal-dialog").attr("style","width:"+r)};popupBox.hide=function(){this.config=undefined;this.modal.modal("hide")};popupBox.getFormJson=function(){if(this.config){return nsForm.getFormJSON(this.config.id)}};popupBox.edit=function(e,t,o){for(var r=0;r<e.length;r++){var n=e[r];var a=formPlane.getInputContainerAndData(n.id,t);var i=a[0];var l=a[1];$.each(n,function(e,t){if(e!="id"){l[e]=t}});var f=commonConfig.component(l,t,"modal",o);var d=f.substring(f.indexOf("=")+1,f.indexOf(">"));f=f.substring(f.indexOf(">")+1,f.lastIndexOf("<"));var s="form-"+t+"-"+l.id;if(l.type=="hidden"){if(i.parent().hasClass("row row-close")){$("#"+s).val(l.value)}}inputContainerAttr=d.substring(d.indexOf('"')+1,d.length-1);i.closest(".form-group").attr("class",inputContainerAttr);i.closest(".form-group").html(f)}};popupBox.formEdit=function(e,t,o){var r=[];var n=[];var a=[];for(var i=0;i<e.length;i++){var l=e[i];var f=formPlane.getInputContainerAndData(l.id,t);var d=f[0];var s=f[1];$.each(l,function(e,t){if(e!="id"){s[e]=t}});var p=commonConfig.component(s,t,"modal",o);p=p.substring(p.indexOf(">")+1,p.lastIndexOf("<"));d.closest(".form-group").html(p);if(typeof s.id!="undefined"){if(typeof s.changeHandler!="undefined"){r.push(s)}if(s.type=="select"){n.push(s)}if(s.type=="date"){if(typeof s.readonly=="undefined"){a.push(s)}else{if(s.readonly==true){a.push(s)}}}}}if(a.length>0){formPlane.componentDate(t,a)}for(refreshSel in n){var m="form-"+t+"-"+n[refreshSel].id;var c=false;if(typeof n[refreshSel].disabled!="undefined"){c=n[refreshSel].disabled}if(c){$("#"+m).attr("disabled",true)}else{$("#"+m).attr("disabled",false);$("#"+m).selectBoxIt().on("open",function(){$(this).data("selectBoxSelectBoxIt").list.perfectScrollbar()})}}formPlane.componentHandler(t,r)};
