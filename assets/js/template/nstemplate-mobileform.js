...
nsTemplate.templates.mobileForm = (function(){
	...
	function formColumnInit(configObj){
		...
		if(config.pageParam.data){
			...
			dataParamsObject = config.pageParam.data;
		}
		//获取form的html
		function getFormHtml(){
			...
			for(var formI=0; formI<config.form.length; formI++){
				...
				for(var fieldI=0; fieldI<formData.field.length; fieldI++){
					...
					if(dataParamsObject[formData.field[fieldI].id]){
						formData.field[fieldI].value = dataParamsObject[formData.field[fieldI].id];
					}
					//sjj 20190626
					if(formData.field[fieldI].url){
						if(!$.isEmptyObject(formData.field[fieldI].data)){
							var ajaxParameterRegExp = /\{?\}/;  //识别{aaaa}的数据
							var isUseObject = true;
							for (var key in formData.field[fieldI].data) {
								if (ajaxParameterRegExp.test(formData.field[fieldI].data[key])) {
									isUseObject = false;
									break;
								}
							}
							if(isUseObject == false){
								var objData = {
									page:dataParamsObject,
								};
								formData.field[fieldI].data = nsVals.getVariableJSON(formData.field[fieldI].data, objData);
							}
						}
					}
				}
		...
		function saveFormAjax(){
			...
			if(paramsJson[config.parentIdField]){
				//存在主键id的时候当前状态为编辑状态
				paramsJson.objectState = 2;
			}
			if(config.isExistPageParam){
				...
				if(res.success){
					...
					if(typeof(ajaxData.plusData)=='object'){
						if(ajaxData.plusData.url){
							...
							var jumpUrl = nsFrame.cacheUrlVRouter.urlObject[nsFrame.cacheUrlVRouter.counter];
							var tempValueName = jumpUrl.substring(jumpUrl.lastIndexOf('=')+1,jumpUrl.length);
			...
			nsVals.ajax(listAjax, function(res){
				if(res.success){
					...
					for(var formId in config.formJson){
						...
						config.formJson[formId].data.getPageDataFunc = (function(serverData){
							return function(){
								return serverData;
							}
						})(serverData);
