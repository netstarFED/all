 ...
 * @LastEditTime: 2019-12-16 18:12:34
 */
...
//入口的初始化方法
nsUI.resultTable.init = function(config, reIntoData){
	if(debugerMode){
	...
	//保存原始对象
	nsUI.resultTable.originalConfig = $.extend(true,{},config);
	nsUI.resultTable.setDefalut(config);
	//如果是离线复原
	if(typeof(reIntoData) == 'object' && reIntoData.isReInto){
		//是否跨域请求上传
		nsUI.resultTable.isJsonp = true;//是否跨域上传
		nsUI.resultTable.cacheAjaxData = typeof(reIntoData.cacheAjaxData)=='string'?JSON.parse(reIntoData.cacheAjaxData):reIntoData.cacheAjaxData;
		var keys = Object.keys(nsUI.resultTable.cacheAjaxData);
		if(keys.length>0){
			nsUI.resultTable.loadPadData(nsUI.resultTable.cacheAjaxData[keys[0]]);
			nsUI.resultTable.setCacheList(keys[0]);
		}
		nsUI.resultTable.fullScreenMode();
		
	}else{
		nsUI.resultTable.loadData(config);
	}
	if(nsUI.resultTable.runAddKey.length == 0){
		for(key in nsUI.resultTable){
			nsUI.resultTable.runAddKey.push(key);
		}
	}
}
//重新加载时需删除非必要属性
nsUI.resultTable.clearKey = function(){
	if(nsUI.resultTable.runAddKey.length>0){
		for(key in nsUI.resultTable){
			if($.inArray(key, nsUI.resultTable.runAddKey) < 0){
				delete nsUI.resultTable[key];
			}
		}
	}
}
//设置默认值和所需值
nsUI.resultTable.setDefalut = function(config){
	...
	nsUI.resultTable.config = config;

	//如果有缓存列表组件存在则删除
	nsUI.resultTable.fullScreenState = false; //是否全屏状态
	nsUI.resultTable.offlineState = false; //是否离线状态
	nsUI.resultTable.isJsonp = false;//是否跨域上传
	if($('#resultTableContainer-cachePanel-cacheSelect')){
		$('#resultTableContainer-cachePanel-cacheSelect').remove();
	}
}
//加载数据文件
nsUI.resultTable.loadData = function(config){
	nsUI.resultTable.clearKey();
	//添加基础样式和loading
	...
	nsVals.ajax(config.tableAjax, function(data){
		if(data.success){
			...
			nsUI.resultTable.data = data[config.tableAjax.dataSrc];
			nsUI.resultTable.vaildData(nsUI.resultTable.data);
			nsUI.resultTable.initPanel(config, nsUI.resultTable.data);
		}
	})
}
//加载pad数据
nsUI.resultTable.loadPadData = function(rowAjaxCacheData){
	nsUI.resultTable.clearKey();
	var config = nsUI.resultTable.config;
	nsUI.resultTable.offlineState = true;
	//添加基础样式和loading
	config.$container.addClass('resultinput');
	...
	nsUI.resultTable.initPanel(config, nsUI.resultTable.data);
	//是否跨域请求，需刷新数据
	if(nsUI.resultTable.isJsonp){
		//所有修改后缓存数据
        nsUI.resultTable.getAllStorage(function(data){
            if(data && $.isArray(data.rows) && data.rows.length>0){
                for(i = 0; i<data.rows.length; i++){//循环修改记录，依次更新表格
                    ...
                    if(body.jsonRows && body.jsonRows.length>0){
          			...
          			nsUI.resultTable.serverDataHandler(obj);     
				}
            }
        }, function(error){
        },{});
...
//检查是否Data文件是否有效
nsUI.resultTable.vaildData = function(data){
	if(debugerMode){
...
//初始化结果录入面板
nsUI.resultTable.initPanel = function(config,data){
	nsUI.resultTable.initTablesData(config,data);
	nsUI.resultTable.initTables(config,data);
	nsUI.resultTable.initInputPanel(config,data);
	nsUI.resultTable.initHistoryPanel(config,data);
	nsUI.resultTable.initControlPanel(config,data);
	nsUI.resultTable.initShortkey();
	if(nsUI.resultTable.saveTimer){
		window.clearTimeout(nsUI.resultTable.saveTimer)
	}
	nsUI.resultTable.autoSaveTimerInit(config,data);
	nsUI.resultTable.mulitSelectTD();
	nsUI.resultTable.autoSubmitHandler();
}
//初始化表格数据
nsUI.resultTable.initTablesData = function(config,data){
	...
	nsUI.resultTable.setSimpleIdOrderData = {current:0, length:0, order:[]};  	//添加排序
	...
	$.each(data.tableData,  function(key,tableData){
		...
		for(var trI=0; trI<$trs.length; trI++){
			...
			for(var tdI = 0; tdI<$tds.length; tdI++){
				...
				//整理表格数据
				var tdData = nsUI.resultTable.getTdData(data.tableData[tableID].rows[trID][tdID],tdID);
				data.tableData[tableID].rows[trID][tdID] = tdData;
				
				//填充表格数据
				var styleStr = '';
				if(typeof(tdData.style)=='object'){
					styleStr = nsUI.resultTable.getStyleString(tdData.style,config);
				...
				switch(tdData.originalType){
					...
					case 'select':
						break;
					case 'number':
						tdData.value.visible = nsUI.resultTable.getVisibleFromReal(tdData);
						break;
					case 'date':
						//先设置默认时间格式
					...
					case 'uploadtitle':
						//添加上传标题的类型
						var formatValueObj = nsUI.resultTable.getUploadVisible(tdData);
					...
					case 'checkbox':
						...
						var formatValueObj = nsUI.resultTable.getCheckboxVisible(tdData);
						tdData.value.visible = formatValueObj.visible;
						tdData.value.edit = formatValueObj.edit;
						break;
					case 'checkboxajax':
						
						//通过对value.format 参数进行格式化查找默认的选中值 cy start 20191105-----
						var _caformat = tdData.value.format;
						if(typeof(_caformat) == 'string'){
							var formatKeyword = 'defaultSelectedIndexs:';
							if(_caformat.indexOf(formatKeyword) > -1){
								var _caDefaultSelected = _caformat.substr(_caformat.indexOf(formatKeyword) + formatKeyword.length);  //以 defaultSelectedIndexs 开始参数
								_caDefaultSelected = _caDefaultSelected.substr(0, _caDefaultSelected.indexOf(',')); //以逗号结束参数
								_caDefaultSelected = _caDefaultSelected.replace(/\'|\"/g, ''); //替换掉双引号和单引号
								var _caDefaultSelectedIndexs = _caDefaultSelected.split(',');
								tdData.defaultSelectedIndexs = [];
								for(var _casI = 0; _casI<_caDefaultSelectedIndexs.length; _casI ++ ){
									var selectedIndex = _caDefaultSelectedIndexs[_casI];
									if(/^[+]{0,1}(\d+)$/.test(selectedIndex) == true){
										//如果拆分完后是正整数 则从下拉列表中默认选中
										tdData.defaultSelectedIndexs.push(parseInt(selectedIndex));
									}else{
										//默认选中项必须是
									}
								}
								//如果有默认选中值 defaultSelectedIndexs，且当前没有值，则需要打标记用于输出
								if(tdData.defaultSelectedIndexs.length > 0){
									if(typeof(tdData.value.real)=='string'){
										if(tdData.value.real == ''){
											//对value.real的处理需要等待ajax载入完成后再执行，先仅标记需要保存
											tdData.state = 'edit';
										}
									}
								}
						...
						//通过对value.format 参数进行格式化查找默认的选中值 cy end 20191105 -------
						...
						var formatValueObj = nsUI.resultTable.getCheckboxAjaxVisible(tdData,config);
						tdData.value.visible = formatValueObj.visible;
						tdData.value.edit = formatValueObj.edit;
						break;
					case 'datestring':
						...
						//找到过滤用字符，向后传值要删除过滤字符
						var filterStringArray = formatStr.match(/[^A-Za-z]/g);
						//格式化数据的字符串
						var maskString = formatStr.replace(/[A-Za-z]/g,'9');
					...
					case 'scientificInput':
						...
						visibleStr = nsUI.resultTable.getScientificValue(tdData.subdata, tdData.value.real);
						tdData.value.visible = visibleStr;
						break;
					case 'multiselect':
						var valueStr = nsUI.resultTable.getMultiselectValue(tdData);
				...
				if(tdData.value.autoSubmit){
					if(typeof(nsUI.resultTable.autoSubmitData)=='undefined'){
						nsUI.resultTable.autoSubmitData = [];
					}
					nsUI.resultTable.autoSubmitData.push(tdData);
				}
				//判断历史记录
				...
				//设定默认操作对象，第一个
				if(nsUI.resultTable.currentTdID==''){
					nsUI.resultTable.currentTdID = tdID;
				}
	...
	for(ctdID in data.tdData){
		...
		var tableData = nsUI.resultTable.data.tableData[ctdData.tableID];
	...
	for(var simpleID in nsUI.resultTable.data.tdDataBySimpleId){
		simpleTotalLength++;
	}
	nsUI.resultTable.setSimpleIdOrderData.length = simpleTotalLength;
}
//根据类型格式化不同组件的基本属性 获取visible的值
nsUI.resultTable.getVisibleFromReal = function(tdData){
	var config = nsUI.resultTable.config;
	var tdID = tdData.id;
...
nsUI.resultTable.getEditFromReal = function(tdData){
	var config = nsUI.resultTable.config;
	var tdID = tdData.id;
...
//初始化表格
nsUI.resultTable.initTables = function(config,data){
	var html = ''
	$.each(data.tableData,  function(key,tableData){
	...
	for(tableID in nsUI.resultTable.data.tableData){
		config.$tables[tableID] = config.$container.children('#'+tableID);
		var settingTableWidth;
		if(config.isUseSettingWidthSize){
			settingTableWidth = nsUI.resultTable.data.tableData[tableID].base.tableWidth;
		...
		config.$tables[tableID].children('tbody').children('tr').children('td').not('.first-rowtd').on('click', nsUI.resultTable.tdClickHandler);
		config.$tables[tableID].children('tbody').children('tr').children('td').not('.first-rowtd').off('mouseenter')
		config.$tables[tableID].children('tbody').children('tr').children('td').not('.first-rowtd').on('mouseenter', nsUI.resultTable.tdMouseenterHandler);
		config.$tables[tableID].children('tbody').children('tr').children('td').not('.first-rowtd').off('mouseleave')
		config.$tables[tableID].children('tbody').children('tr').children('td').not('.first-rowtd').on('mouseleave', nsUI.resultTable.tdMouseleaveHandler);
	}
	config.$container.find('.history').on('click',nsUI.resultTable.historyClickHandler);
	config.$container.find('.notes').on('click',nsUI.resultTable.notesClickHandler);

	//重新格式化需要的组件代码
	setTimeout(resizeTdByType, 10)
	function resizeTdByType(){
		for(tdID in nsUI.resultTable.data.tdData){
			var tdData = nsUI.resultTable.data.tdData[tdID]
			if(tdData.type == 'checkboxajax'){
				...
				if(tdData.isWithMoreOption && tdData.selectIndex == 0){
					...
					var html = nsUI.resultTable.getCheckboxAjaxVisible(tdData, config);
...
//返回输入组件HTML
nsUI.resultTable.getInputComponentHtml = function(config,data){
	var html = '';
...
//初始化输入面板
nsUI.resultTable.initInputPanel = function(config,data){
	config.$editPanel = config.$container.children('#'+config.inputcomponentID);
	var html = nsUI.resultTable.getInputComponentHtml(config,data);
...
//多选事件
nsUI.resultTable.mulitSelectTD = function(){
	...
	function getSelectTD(zone){
		...
		for(tdID in nsUI.resultTable.data.tdData){
			var tdPosition = {};
			var $td = nsUI.resultTable.data.tdData[tdID].$td
		...
		nsUI.resultTable.selected = selectedTdID;
	}
	//添加单击取消选择事件
...
//初始化历史记录面板
nsUI.resultTable.initHistoryPanel = function(config,data){
}
//控制面板
nsUI.resultTable.initControlPanel = function(config,data){
...
//发送自动保存的数据
nsUI.resultTable.autoSubmitHandler = function(){
	if($.isArray(nsUI.resultTable.autoSubmitData) == false){
		return;
	}
	var config = nsUI.resultTable.config;
	var formatSubmitData = [];
	
	for(var sdI = 0; sdI<nsUI.resultTable.autoSubmitData.length; sdI++){
		cdata = nsUI.resultTable.autoSubmitData[sdI];
		switch(cdata.type){
			case 'multiinput':
				//multiinput要拆分数据后发送
	...
	//在线和离线两种模式
	if(nsUI.resultTable.offlineState){
		console.log('offlineState');
		nsUI.resultTable.addStorage(function(data){
			console.log('OK');
			//成功了啥都不干
		}, function(error){
			console.log(error);
...
/**2017-11-24***/
//单元格悬停事件
nsUI.resultTable.tdMouseenterHandler = function(ev){
	var $td = $(this);
	nsUI.resultTable.showContentPanel($td);
}
//单元格移除悬停事件
nsUI.resultTable.tdMouseleaveHandler = function(ev){
...
//单元格单击事件
nsUI.resultTable.tdClickHandler = function(ev){
	var $td = $(this);
	var inputResizeData = nsUI.resultTable.getResizeDataByDom($td);
	inputResizeData.$target = $(ev.target);
	nsUI.resultTable.showEditPanel(inputResizeData);
}
//根据目标ID，返回输入组件所需的基础数据
nsUI.resultTable.getResizeDataByDom = function($dom){
	var cid = $dom.attr('id');
	var tdData = nsUI.resultTable.data.tdData[cid];
	//top值要区分 是否全屏模式全屏模式要添加滚动偏差值
	//debugger
	// console.log('$dom.position().top;:'+$dom.position().top);
	// console.log('scrollTop:'+nsUI.resultTable.config.$container.scrollTop());
	var topNumber = $dom.position().top;
	// if(nsUI.resultTable.fullScreenState){
		topNumber += nsUI.resultTable.config.$container.scrollTop(); //lyw
	// }
	var inputResizeData = {
		id:cid,
		nsID:$dom.attr('ns-id'),
	...
	// inputResizeData.top = inputResizeData.top + nsUI.resultTable.config.$container.scrollTop();
	// inputResizeData.top = inputResizeData.top;
	inputResizeData.left = inputResizeData.left + nsUI.resultTable.config.$container.scrollLeft();
...
/*********2017-11-24***********************/
//鼠标悬停事件
nsUI.resultTable.showContentPanel = function($td){
	if($('.resultTable-tooltip-title').length > 0){
		$('.resultTable-tooltip-title').remove()
	}
	...
	timer = setTimeout(function() {
		var config = nsUI.resultTable.config;
		var inputResizeData = nsUI.resultTable.getResizeDataByDom(that);
...
//改变input输入组件大小
nsUI.resultTable.showEditPanel = function(resizeData){
	//inputResizeData = {
	// 	id：tdID
	...
	nsUI.resultTable.config.$editPanel.css(cssObj);
	
	//快捷键重置
	nsUI.resultTable.shortkeyType = 'table';	
	switch(type){
		case 'none':
			break;
		case 'string':
			nsUI.resultTable.stringInit(resizeData);
			break;
		case 'number':
			nsUI.resultTable.numberInit(resizeData);
			break;
		case 'select':
			nsUI.resultTable.inputSelectInit(resizeData);
			break;
		case 'date':
			nsUI.resultTable.dateInit(resizeData);
			break;
		case 'textarea':
			nsUI.resultTable.textareaInit(resizeData);
			break;
		case 'multiinput':
			nsUI.resultTable.multiinputInit(resizeData);
			break;
		case 'uploadtitle':
			nsUI.resultTable.uploadtitleInit(resizeData);
			break;
		case 'checkbox':
			nsUI.resultTable.checkboxInit(resizeData);
			break;
		case 'checkboxajax':
			nsUI.resultTable.checkboxAjaxInit(resizeData);
			break;
		case 'multiselect':
			nsUI.resultTable.inputMultiselectInit(resizeData);
			break;
		case 'multiupload':
			//多图片上传 sjj20180319
			nsUI.resultTable.multiuploadInit(resizeData);
			break;
		default:
			if(debugerMode){
	...
	nsUI.resultTable.currentTdID = resizeData.id;
}
//添加验证规则
nsUI.resultTable.addRules = function(tdData){
...
//文本框初始化
nsUI.resultTable.stringInit = function(resizeData){
	var tdData = resizeData.tdData;
	var $currentInput = this.config.$editPanelStringInput;
	
	//如果数量多了就应该清除干净
	
	if($currentInput.parent().children().length >1){
		var inputHtml = $currentInput[0].outerHTML;
		$currentInput.parent().html(inputHtml);
		this.config.$editPanelStringInput = $('#resultTableContainer-inputcomponent-input-string');
		$currentInput = this.config.$editPanelStringInput;
	}
	//在某些情况下回出现errorlabel不能正常清除的情况 做额外判断清除 cy 20191116
	var $errorLabel = $currentInput.closest('.input-string').find('label.has-error');
	$errorLabel.remove();
	...
	$currentInput.select();
	nsUI.resultTable.addRules(tdData);
	//lyw
	...
	if(tdData.originalType == "string"){
		...
		if(typeof(tdData.subdata) == "object"){
			if(tdData.subdata.isUseHtmlInput){
				...
				nsUI.htmlInput.init(tdDataConfiger,"resulttable");
			}else{
				// $("#resultTableContainer-inputcomponent-input-string").off('keyup');
				// $("#resultTableContainer-inputcomponent-input-string").off('keydown');
			}
...
//数字框初始化
nsUI.resultTable.numberInit = function(resizeData){
	var tdData = resizeData.tdData;
...
//转化成科学计数法的显示值
nsUI.resultTable.getScientificValue = function(subdata, realValue){
	var visibleStr = '';
	var realNumber = 0;
		...
		if(realValue==''){
			...
			if(realNumber==0){
				...
				if(realNumber<subdata.min || realNumber>subdata.max){
					var n = 0;
					var n = nsUI.resultTable.floatCharge(Math.log(realNumber),Math.LN10);	//返回指定数字以 10 为底的对数 
					...
					if(integerPlace == 2){
						...
						//四舍六入
						//保留几位小数
						realNumber = nsUI.resultTable.formatFloat(realNumber,pointPlace);
					}
...
//四舍六入
nsUI.resultTable.formatFloat = function(num,precision){
	var carry = 0; //存放进位标志
...
//浮点数精确转换
nsUI.resultTable.floatCharge = function(a,b){
...
nsUI.resultTable.floatToInteger = function(floatNum){
...
//文本下拉框初始化
nsUI.resultTable.inputSelectInit = function(resizeData){
	//select 下拉框选中事件
	function selectClick(ev){
		...
		nsUI.resultTable.saveTdData(tdData.id,value);
		nsUI.resultTable.refreshTd(tdData.id);
		closeSelect();
	}
	...
	//初始化过程
	var $editPanel = nsUI.resultTable.config.$editPanel;
	...
	$currentInput.on('click',function(ev){
		if($currentInputPanel.children('div.select-list').hasClass('hide')){
			...
			nsUI.resultTable.shortkeyType = 'select';
...
//筛选多选值
nsUI.resultTable.inputMultiselectFilter = function(multifilterJson,selectValue,selectId){
...
//多选下拉框
nsUI.resultTable.inputMultiselectInit = function(resizeData){
	var $editPanel = nsUI.resultTable.config.$editPanel;
	var $currentInputPanel = $editPanel.children('.component-container').children('div[ns-class="'+resizeData.tdData.type+'"]');
	var tdData = resizeData.tdData;
	...
	$currentInput.on('click',function(ev){
		if($currentInputPanel.children('div.select-list').hasClass('hide')){
			$currentInput.select();
			$currentInputPanel.children('div.select-list').removeClass('hide');
			nsUI.resultTable.shortkeyType = 'multiselect';
			$(document).on('mousedown',outerClickHandler);
		}
	})
	nsUI.resultTable.shortkeyType = 'multiselect';
	...
	nsUI.resultTable.multiselectJson = multifilterJson;
	function selectClick(ev){
		var $this = $(this);
		...
		var valueStr = nsUI.resultTable.inputMultiselectFilter(multifilterJson,selectValue,selectId);
		nsUI.resultTable.multiselectJson = valueStr;
...
//文本下拉框快捷键
nsUI.resultTable.inputSelectShortkey = function(direction){
	var $inputPlane = nsUI.resultTable.config.$editPanel;
	...
	switch(direction){
		...
		default:
			if(debugerMode){
				console.error('nsUI.resultTable.inputSelectShortkey 中不能识别的方向:'+direction)
			}
	}
	...
	if(isMove){
		...
		nsUI.resultTable.saveTdData(nsUI.resultTable.currentTdID,value);
		nsUI.resultTable.refreshTd(nsUI.resultTable.currentTdID);
	}
}
nsUI.resultTable.inputMultiselectShortKey = function(direction,event){
	var $inputPlane = nsUI.resultTable.config.$editPanel;
	...
	switch(direction){
		...
		case 'enter':
			isMove = false;
			//非移动 回车操作
			var valArr = nsUI.resultTable.inputMultiselectFilter(nsUI.resultTable.multiselectJson);
			var valueStr = valArr[1];
			var idsStr = valArr[0];
			var valueJson = {visible:valueStr, real:idsStr};
			nsUI.resultTable.saveTdData(nsUI.resultTable.currentTdID,valueJson);
			nsUI.resultTable.refreshTd(nsUI.resultTable.currentTdID);
			break;
		case 'space':
			//空格
			...
			var valArr = nsUI.resultTable.inputMultiselectFilter(nsUI.resultTable.multiselectJson,value,id);
			var valueStr = valArr[1];
			var idsStr = valArr[0];
			$('#'+nsUI.resultTable.currentTdID).html(valueStr);
			$('#'+nsUI.resultTable.currentTdID).attr('ns-id',idsStr);
			break;
		default:
			if(debugerMode){
				console.error('nsUI.resultTable.inputSelectShortkey 中不能识别的方向:'+direction)
...
//日期
nsUI.resultTable.dateInit = function(resizeData){
	...
	// 	nsUI.resultTable.shortkeyType = 'date-input';
	// 	$currentInput.on('keydown',function(ev){
	// 		ev.stopPropagation(); 
	// 	});
	// })
	nsUI.resultTable.shortkeyType = 'date';
}
//textarea初始化
nsUI.resultTable.textareaInit = function(resizeData){
	var tdData = resizeData.tdData;
	var $editPanel = nsUI.resultTable.config.$editPanel;
	...
	$currentInput.select();
	nsUI.resultTable.shortkeyType = 'textarea';
	//lyw
	if($("#resultTableContainer-inputcomponent").find("div.shortcut-html-input")){
		$("#resultTableContainer-inputcomponent").find("div.shortcut-html-input").remove();
	}
...
//multiinput 多输入框组件
nsUI.resultTable.multiinputInit = function(resizeData){
	var $container = nsUI.resultTable.config.$editPanelMultiinputInput;
	var tdData = resizeData.tdData;
	...
	$focusInput.focus();
	$focusInput.select();
	nsUI.resultTable.shortkeyType = 'multiinput';
			...
			case 'tab':
				...
				if($currentInput.length>0){
					...
					if($tabTargetInput.length==0){
						nsUI.resultTable.shortkeyMoveInput('enter');
					}
				}else{
			...
			case 'enter':
				...
				for(inputKey in tdData.value.real){
					...
					if(tdData.value.real[inputKey]!=keyValue){
						...
						visibleHtml = nsUI.resultTable.getMultiinputVisible(inputKey, keyValue, visibleHtml);
					}
				}
	...
	nsUI.resultTable.multiinputShortkey = multiinputShortkey;
}
//获取多输入框组件visible
nsUI.resultTable.getMultiinputVisible = function(inputKey, keyValue, visibleHtml){
	//获取visible 
...
//处理多图片上传 sjj20180319
nsUI.resultTable.multiuploadInit = function(resizeData){
	var tdData = resizeData.tdData;
	...
	if(resizeData.$target){
		if(resizeData.$target.is('img')){
			...
			var uploadData = nsUI.resultTable.data.tdData[tdData.id].subdata;
			var uploadField = nsUI.resultTable.config.default.uploadSaveAjax.field;
				...
				btns:[
					{
						...
						handler:function(){
							...
							if(nsIndex > 0){
	                            ...
	                            var _postData = $.extend({},nsUI.resultTable.config.saveAjax.data,postData);
								$.ajax({
									url: 		nsUI.resultTable.config.default.uploadSaveAjax.src,
									data: 		_postData,
									type: 		'POST',
									dataType: 	'json',
									success:function(data){
										if(data.success){
											...
											var rowsData = data[nsUI.resultTable.config.default.uploadSaveAjax.dataSrc];
											//var configField = data[nsUI.resultTable.config.default.uploadSaveAjax].field;
											tdData.subdata[nsIndex].orderId = orderId - 1;
											tdData.subdata[nsIndex-1].orderId = orderId;
											nsdialog.hide();
											nsUI.resultTable.getUploadResult(resizeData);
										}else{
											nsalert(data.result);
										}
									},
					...
					},
					{
						...
						handler:function(){
							...
							var downConfig = nsUI.resultTable.config.default.uploadAjax.downLoadAjax;
							...
							$.ajax({
								...
								success:function(data){
									if(data.success){
										var nsIndex = $('.img-big.show').attr('ns-index');
										var rowsData = data[nsUI.resultTable.config.default.uploadSaveAjax.dataSrc];
										var configField = nsUI.resultTable.config.default.uploadSaveAjax.field;
					...
					},
					{
						...
						handler:function(){
							...
							$.ajax({
								url: 		nsUI.resultTable.config.default.uploadDelAjax.src,
								data: 		{id:nsId},
								type: 		'post',
								dataType: 	'json',
								success:function(data){
					...
					},
					{
						text:'关闭',
						handler:function(){
							var $container = nsUI.resultTable.config.$editPanelMultiupload;
							nsUI.resultTable.config.$editPanel.removeAttr('style');
							nsUI.resultTable.config.$editPanel.removeClass('multiupload');
	...
	//testDataId:tdData.nsId
	//var _postData = $.extend({},nsUI.resultTable.config.uploadAllAjax.data,postData);
	/*var uploadParams = '?testDataId='+tdData.nsId;
	if(typeof(nsUI.resultTable.config.uploadAllAjax.data)=='object'){

	}
	for(var params in nsUI.resultTable.config.uploadAllAjax.data){
		uploadParams +='&'+params+'='+nsUI.resultTable.config.uploadAllAjax.data[params];
	};
	...
	function uploadSuccessHandler(data){
		var uploadField = nsUI.resultTable.config.default.uploadAllAjax.field;
		var rowData = data[nsUI.resultTable.config.default.uploadAllAjax.dataSrc];
		/*return {
	...
	function saveHandler(){
		...
		var _postData = $.extend({},nsUI.resultTable.config.saveAjax.data,saveObject);
		$.ajax({
			url: 		nsUI.resultTable.config.default.uploadAllSaveAjax.src,
			data: 		_postData,
			type: 		'POST',
			dataType: 	'json',
			success:function(data){
...
//上传添加标题初始化
nsUI.resultTable.uploadtitleInit = function(resizeData){
	var tdData = resizeData.tdData;
			...
			var imageInfo = {
				...
				btns:[
					...
					},
					{
						text:'关闭',
						handler:function(){
							var $container = nsUI.resultTable.config.$editPanelUploadtitle;
							nsUI.resultTable.config.$editPanel.removeAttr('style');
							nsUI.resultTable.config.$editPanel.removeClass('uploadtitle');
							$container.removeClass('show');
							nsdialog.hide();
						}
					}
			...
			form:[
					{
						...
						column: 	8,
						rules: 		'upload',
						uploadSrc: 	nsUI.resultTable.config.default.uploadAjax.src,
						ismultiple:false,
						textField:'name',
						valueField:'id',
						isAllowFiles:1,
						...
						changeHandler:function(data){
							var uploadField = nsUI.resultTable.config.default.uploadAjax.field;
							var rowData = data.data[nsUI.resultTable.config.default.uploadAjax.dataSrc];
							return {
								id:rowData[0][uploadField.id],
								name:rowData[0][uploadField.name]
							}
...
nsUI.resultTable.getUploadResult = function(resizeData){
	var tdData = resizeData.tdData;
	...
	nsUI.resultTable.data.tdData[tdData.id].subdata = tdData.subdata;
	nsUI.resultTable.data.tdData[tdData.id].value.visible = value.visible;
	nsUI.resultTable.data.tdData[tdData.id].value.real = value.real;
}
//checkbox 多选输入组件
nsUI.resultTable.checkboxInit = function(resizeData){
	var tdData = resizeData.tdData;
	...
	nsUI.resultTable.shortkeyType = 'checkbox';	
	//点击事件
	...
	function checkboxInputShortkey(keyType, event){
		switch(keyType){
			...
			case 'left':
				if(tabIndex==0){
					//如果是第一个，那就上一个单元格
					nsUI.resultTable.shortkeyMoveInput('left');
				}else{
					//上一个option
					tabIndex--;
			...
			case 'right':
				if(tabIndex==$container.children('label.checkbox-inline').length){
					//如果是最后一个，那就上一个单元格
					nsUI.resultTable.shortkeyMoveInput('right');
				}else{
					//下一个option
					tabIndex++;
	...
	nsUI.resultTable.checkboxInputShortkey = checkboxInputShortkey;
}
//根据real和subdata获取value.visible和value.edit
nsUI.resultTable.getUploadVisible = function(tdData){
	var visibleHtml = '';
	var editHtml = '';
	var imageHtml = '';
	//tdData.subdata存放的是默认的上传文件
	var uploadConfig = nsUI.resultTable.config.default.uploadSaveAjax;
    if(typeof(tdData.subdata) == 'undefined'){
        tdData.subdata = [];
    }
    if(tdData.type == 'uploadtitle'){
...
//multiselect获取值
nsUI.resultTable.getMultiselectValue = function(tdData){
...
nsUI.resultTable.getCheckboxVisible = function(tdData){
...
//checkboxajax
nsUI.resultTable.checkboxAjaxInit = function(resizeData){ 
	var tdData = resizeData.tdData;
	...
	//快捷键类型
	nsUI.resultTable.shortkeyType = 'checkboxajax';	
	//点击事件
	$container.children('input[type="checkbox"]').on('change',changeHandler);
	//格式化输入框
	...
	function initValuePanel(){
		var nameHtml = nsUI.resultTable.checkboxAjaxGetName(selectValues, tdData.formatData, '', tdData);
	...
	function initInputTextHandler(eventName){
		...
		if(nsUI.resultTable.config.default.checkboxajax.isNumberID){
			...
			$.each(tdData.formatData[dataI], function(keyname,value){
				...
				if(typeof(value)=='string'){
					if(keyname!='id'&&keyname!='nsIndex'){
						...
						if(keyname == nsUI.resultTable.config.default.checkboxajax.field.py){
	...
	nsUI.resultTable.checkboxajaxInputShortkey = checkboxajaxInputShortkey;
}
//根据real和subdata获取value.visible和value.edit
nsUI.resultTable.getCheckboxAjaxVisible = function(tdData,config){
	var visibleHtml = '';
	var editHtml = '';
		...
		switch(configOption.subType){
			...
			case 'select':
				...
				if($.isArray(tdData.formatData)){
					valueStr = nsUI.resultTable.checkboxAjaxGetName(configOption.value, tdData.formatData, '', tdData);
				};
	...
	if(isNeedGetData){
		...
		nsVals.ajax(ajaxConfig, function(data,ajax){
			...
			//获取值
			var nameStr = nsUI.resultTable.checkboxAjaxGetName(option.value, formatData, tdData.value.real, tdData);
			if(nameStr==''){
				nameStr = '　';
			}
...
//根据id数组，返回显示名称
nsUI.resultTable.checkboxAjaxGetName = function(idArray,formatData, realValue, tdData){
	var nameStr = '';
	var spliceIndexArray = [];  //存放无法匹配结果的数组
	
	//如果有默认的选择 且idArray没有值，则使用默认值 cy 191105 start -----
	if($.isArray(tdData.defaultSelectedIndexs) && idArray.length == 0 ){
		if(tdData.defaultSelectedIndexs.length > 0){
			tdData.value.real = [];
			for(var dsi = 0; dsi<tdData.defaultSelectedIndexs.length; dsi++){
				var listData = formatData[tdData.defaultSelectedIndexs[dsi]];
				tdData.value.real.push(listData.id);
			}
		}
	}
	//如果有默认的选择 且idArray没有值，则使用默认值 cy 191105 end   -----
	if(idArray.length>0){
...
nsUI.resultTable.initShortkey = function(){
	$(document).off('keydown');
	$(document).on('keydown',function(ev){
		var tdData = nsUI.resultTable.data.tdData[nsUI.resultTable.currentTdID];
		function moveInput(direction){
			var moveTargetID = tdData.neighbours[direction];
			if(moveTargetID){
				var inputResizeData = nsUI.resultTable.getResizeDataByDom($('#'+moveTargetID));
				nsUI.resultTable.showEditPanel(inputResizeData);
			}
		}
		nsUI.resultTable.shortkeyMoveInput = moveInput;
		switch(ev.keyCode){
			case 38:
				//上
				switch(nsUI.resultTable.shortkeyType){
					case 'select':
						//下拉框状态下是
						ev.stopPropagation(); 
						nsUI.resultTable.inputSelectShortkey('up');
						ev.preventDefault(); 
						break;
					case 'date':
						//日期状态下组件接管上下左右
					...
					case 'multiselect':
						ev.stopPropagation();
						nsUI.resultTable.inputMultiselectShortKey('up');
						ev.preventDefault();
						break;
					default:
						break;	
			...
			case 40:
				...
				switch(nsUI.resultTable.shortkeyType){
					case 'select':
						//下拉框状态下是
						ev.stopPropagation(); 
						nsUI.resultTable.inputSelectShortkey('down');
						ev.preventDefault(); 
						break;
					case 'date':
						//日期状态下组件接管上下左右
					...
					case 'multiselect':
						ev.stopPropagation();
						nsUI.resultTable.inputMultiselectShortKey('down');
						ev.preventDefault();
						break;
					default:
						break;	
			...
			case 37:
				...
				switch(nsUI.resultTable.shortkeyType){
					...
					case 'checkbox':
						//移动到上一个option
						nsUI.resultTable.checkboxInputShortkey('left',ev);
						break;
					case 'checkboxajax':
						//移动到上一个option
						nsUI.resultTable.checkboxajaxInputShortkey('left',ev);
						break;
					case 'select':
					case 'table':
					case 'multiselect':
						...
						var currentInputPanel = nsUI.resultTable.config.$editPanel.children('.component-container').children('.show');
			...
			case 39:
				...
				switch(nsUI.resultTable.shortkeyType){
					...
					case 'checkbox':
						//移动到上一个option
						nsUI.resultTable.checkboxInputShortkey('right',ev);
						break;
					case 'checkboxajax':
						//移动到上一个option
						nsUI.resultTable.checkboxajaxInputShortkey('right',ev);
						break;
					case 'select':
					case 'table':
					case 'multiselect':
			...
			case 13:
				...
				if(currentInputPanel.length != 0){
					ev.preventDefault();
					var isValid = nsUI.resultTable.validValue(tdData);
					if(isValid==false){
						return false;
					}
					switch(nsUI.resultTable.shortkeyType){
						case 'select':
							//下拉框状态下的确认
							nsUI.resultTable.inputSelectShortkey('enter');
							moveInput('enter');
							break;
						case 'date':
							moveInput('enter');
						...
						case 'textarea':
							// if(ev.shiftKey){
								//如果是shift+enter 则添加回车符号  nsUI.htmlInput.init中添加了alt+enter这里没用了
								// var currentText = nsUI.resultTable.config.$editPanelTextareaInput.val();
								// var textarea = nsUI.resultTable.config.$editPanelTextareaInput
							...
							// }else{
								var value = nsUI.resultTable.getInputValue();
								nsUI.resultTable.saveTdData(nsUI.resultTable.currentTdID,value);
								nsUI.resultTable.refreshTd(nsUI.resultTable.currentTdID);
								moveInput('enter');
							// }
							break;
						case 'multiinput':
							var value = nsUI.resultTable.multiinputShortkey('enter');
							if(value){
								nsUI.resultTable.saveTdData(nsUI.resultTable.currentTdID,value);
								nsUI.resultTable.refreshTd(nsUI.resultTable.currentTdID);
							}
							...
							break;
						case 'checkbox':
							var checkboxValue = nsUI.resultTable.checkboxInputShortkey('enter');
							var value = {
							...
							break;
						case 'checkboxajax':
							var idArray = nsUI.resultTable.checkboxajaxInputShortkey('enter',ev);
							var value = {
								real:idArray,
								visible:'temp'
							}
							nsUI.resultTable.saveTdData(nsUI.resultTable.currentTdID,value);
							var htmlObj = nsUI.resultTable.getCheckboxAjaxVisible(tdData, this.config);
						...
						case 'table':
							...
							break;
						case 'multiselect':
							nsUI.resultTable.inputMultiselectShortKey('enter');
							moveInput('enter');
							break;
						default:
							break;	
			...
			case 9:
				...
				switch(nsUI.resultTable.shortkeyType){
					case 'multiinput':
						nsUI.resultTable.multiinputShortkey('tab');
						break;
					case 'checkbox':
						nsUI.resultTable.checkboxInputShortkey('tab');
						break;
					case 'checkboxajax':
						nsUI.resultTable.checkboxajaxInputShortkey('tab',ev);
						break;
					default:
						moveInput('enter');
						break;
			...
			case 32:
				switch(nsUI.resultTable.shortkeyType){
					case 'checkbox':
						nsUI.resultTable.checkboxInputShortkey('space',ev);
						break;
					case 'checkboxajax':
						nsUI.resultTable.checkboxajaxInputShortkey('space',ev);
						break;
					case 'multiselect':
						nsUI.resultTable.inputMultiselectShortKey('space',ev);
						break;
					default:
						break;
				}
			...
			case 70:
				if(ev.ctrlKey && ev.altKey){
					...
					if($container.hasClass('fullscreen')){
						nsUI.resultTable.exitFullScreenMode();
					}else{
            			nsUI.resultTable.fullScreenMode();
            		}
				}
...
//验证输入值是否合法
nsUI.resultTable.validValue = function(tdData){
	//标准valid验证
	if(typeof(tdData.rules)=='string'){
	...
	function getInputValue(){
		...
		switch(tdData.type){
			case 'string':
				...
				$validInput = nsUI.resultTable.config[inputDomName];
...
//获取输入组件的值
nsUI.resultTable.getInputValue = function(){
	var currentInputPanel = nsUI.resultTable.config.$editPanel.children('.component-container').children('.show');
	...
	switch(currentInputType){
		...
		case 'number':
			...
			//科学计数法组件重新获取格式化后的字符串
			if(tdData.originalType == 'scientificInput'){
				value.visible = nsUI.resultTable.getScientificValue(tdData.subdata, valueStr)
			}else{
				value.visible = valueStr;
			}
			if(isNaN(value.real)){
...
nsUI.resultTable.saveTdData = function(tdID,value){
	var tdData = nsUI.resultTable.data.tdData[tdID];
	var isEdit = false;
	switch(tdData.type){
		case 'checkboxajax':
...
//刷新单元格
nsUI.resultTable.refreshTd = function(tdID){
	var tdData = nsUI.resultTable.data.tdData[tdID];
	...
	var classStr = tdData.type+' '+tdData.state;
	tdData.$td.attr('class',classStr)
	tdData.$td.find('.history').on('click',nsUI.resultTable.historyClickHandler)
}

//获取style字符串
nsUI.resultTable.getStyleString = function(styleJson,config){
	var styleStr = ''
...
//统一单元格数据的value和默认值
nsUI.resultTable.getTdData = function(tdData,tdID){
...
//获取保存数值
nsUI.resultTable.getSubmitData = function(){
	var values = [];
	for(tdID in nsUI.resultTable.data.tdData){
		var tdData = nsUI.resultTable.data.tdData[tdID];
...
//保存结果处理
nsUI.resultTable.savedHandler = function(config,submitData){
	//整理要发送出去的数据
	var formatSubmitData = [];
	...
	for(submitI = 0; submitI<submitData.length; submitI++){
		...
		nsUI.resultTable.data.tdDataBynsId[tdID].state = 'process';
		nsUI.resultTable.refreshTd(nsUI.resultTable.data.tdDataBynsId[tdID].id);
	}
	//ajax发送错误
	function ajaxerror(){
		for(submitI = 0; submitI<submitData.length; submitI++){
			var tdID = submitData[submitI].id;
			nsUI.resultTable.data.tdDataBynsId[tdID].state = 'edit';
			nsUI.resultTable.refreshTd(tdID);
		}
	}
	//在线和离线两种模式 
	if(nsUI.resultTable.offlineState == false){
		$.ajax({
			url: 		config.saveAjax.url,
			data: 		config.saveAjax.data,
			type: 		config.saveAjax.type,
				...
				if(data.success){
					...
					if($.isArray(resultData)){
						if(resultData.length>0){
							nsUI.resultTable.serverDataHandler(resultData);
		...
		nsUI.resultTable.addStorage(function(data){
			...
			for(submitI = 0; submitI<submitData.length; submitI++){
				var tdID = submitData[submitI].id;
				nsUI.resultTable.data.tdDataBynsId[tdID].state = 'complete';
				nsUI.resultTable.refreshTd(nsUI.resultTable.data.tdDataBynsId[tdID].id);
			}
		}, function(error){
			console.log(error);
...
//发送结果处理 修改数据，刷新单元格
nsUI.resultTable.serverDataHandler = function(resultDatas){
	for(var valueI = 0; valueI<resultDatas.length; valueI++){
		var resultData = resultDatas[valueI];
		var tdID = resultData.id;
		var tdData = nsUI.resultTable.data.tdDataBynsId[tdID];
		if(typeof(tdData)=='object'){
			//如果存在该单元格，则刷新
...
//自动保存，如果有变化则保存
nsUI.resultTable.autoSaveTimerInit = function(config,data){
	nsUI.resultTable.saveTimer =  window.setTimeout(function(ev){
		//如果有变化则保存
		var editValues = nsUI.resultTable.getSubmitData();
		var config = nsUI.resultTable.config;
		if(editValues!=false){
			nsUI.resultTable.savedHandler(config,editValues);
		}
		//如果表格还存在则继续启动自动保存
...
//历史记录
nsUI.resultTable.historyClickHandler = function(ev){
	...
	positionTop = positionTop + nsUI.resultTable.config.$container.scrollTop();
	positionLeft = positionLeft + nsUI.resultTable.config.$container.scrollLeft();
	//超出屏幕隐藏
	if(positionLeft<20){
		positionLeft = 20;
	}
	...
	$.ajax({
		...
		success:function(data){
			if(data.success){
				...
				if($.isArray(resultData)){
					if(resultData.length>0){
						nsUI.resultTable.refreshHistoryPlane(resultData,tdData);
					}
				}
			}else{
				nsalert(data.result);
...
//批注回调
nsUI.resultTable.notesClickHandler = function(ev){
	var tdID = $(this).closest('td').attr('id');
	var config = nsUI.resultTable.config;
	//返回值
	var returnValue = nsUI.resultTable.getOutputData(tdID);
...
//批注状态修改
nsUI.resultTable.notesChangeState = function(nsId, type){
	//批注状态修改 
	...
	var tdData = nsUI.resultTable.data.tdDataBynsId[nsId];
	if(debugerMode){
	...
	if(originalNoteType == 0){
		$td.children('a.notes').on('click', nsUI.resultTable.notesClickHandler)
	}
}
//获取当前选中的数据，批注新建调用方法
nsUI.resultTable.getCurrentData = function(){
	...
	var outputValue = nsUI.resultTable.getOutputData(nsUI.resultTable.currentTdID);
...
//获取基础回调数据
nsUI.resultTable.getOutputData = function(tdID){
	var tdData = nsUI.resultTable.data.tdData[tdID];
	var config = nsUI.resultTable.config;
	var $td = tdData.$td;
	var position = $td.position();
...
//刷新历史面板
nsUI.resultTable.refreshHistoryPlane = function(historyArray,tdData){
 	var html = '';
	...
	nsUI.resultTable.config.$historyPanel.html(html);
}
//获取选中单元的值
nsUI.resultTable.getSelectValue = function(isIncludeNone){
...
//复制
nsUI.resultTable.copy = function(isIncludeNone){
	//是否支持对静态单元格的赋值
	var _isIncludeNone = true;
	if(typeof(isIncludeNone)=='boolean'){
		_isIncludeNone = isIncludeNone;
	}
	var copyValue = nsUI.resultTable.getSelectValue(_isIncludeNone);
	nsalert('复制成功 '+copyValue.visible, 'success');
	nsUI.resultTable.copyValue = copyValue;
}
//黏贴
nsUI.resultTable.paste = function(){
	//是否有要复制的
	if(typeof(nsUI.resultTable.copyValue)=='undefined'){
		nsalert('请先选择要复制的单元格','error');
		return;
	}
	//是否有选中的单元格
	if(nsUI.resultTable.config.$container.find('td.inselectzone').length==0){
		nsalert('请先选择要粘贴的单元格','error');
		return;
	}
	//批量赋值操作
	var filterSelectArray = [];
	for(var selectI = 0; selectI<nsUI.resultTable.selected.length; selectI++){
		//如果不是手工输入的值 只对同类型的进行粘贴操作
		if(nsUI.resultTable.copyValue.type!='notd'){
			//类型相同
			if(nsUI.resultTable.data.tdData[nsUI.resultTable.selected[selectI]].type == nsUI.resultTable.copyValue.type){
				//不是只读
				if(nsUI.resultTable.data.tdData[nsUI.resultTable.selected[selectI]].readonly){
					//如果是只读，就过滤掉
				}else{
					filterSelectArray.push(nsUI.resultTable.selected[selectI]);
				}
			}
		}else if(nsUI.resultTable.copyValue.type=='notd'){
		//如果是手工输入的,就不匹配类型了
			filterSelectArray.push(nsUI.resultTable.selected[selectI]);
		}
	}
	...
	var isComplete = nsUI.resultTable.setValues(nsUI.resultTable.copyValue, filterSelectArray);
	if(isComplete){
		nsalert('粘贴完成','success');
	}
}
//对单元格批量赋值，可以用于复制黏贴
nsUI.resultTable.setValues = function(value, idArr, isOnlyData){
		...
		for(var i = 0; i<targetIDArr.length; i++){
			...
			if(isOnlyEditData==false){
				...
				function setValue(_sourceValue){
					nsUI.resultTable.saveTdData(tdData.id,_sourceValue);
					nsUI.resultTable.refreshTd(targetIDArr[i]);
					...
					case 'multiinput':
						...
						for(valueKey in tdData.value.real){
							...
							if(isIncludeSourceValue){
								...
								//debugger
								multiinputSourceValue.visible = nsUI.resultTable.getMultiinputVisible(valueKey, sourceValue.real[valueKey], multiinputSourceValue.visible);
							}
						}
...
//使用原始尺寸显示
nsUI.resultTable.previewMode = function(){
	var config = $.extend(true, {}, nsUI.resultTable.originalConfig);
	...
	nsUI.resultTable.init(config);
}
//使用页面优化方式显示
nsUI.resultTable.inputMode = function(){
	var config = $.extend(true, {}, nsUI.resultTable.originalConfig);
...
//批量编辑 停止开发的功能
nsUI.resultTable.multiedit = function(){
	var $selectedTDs = nsUI.resultTable.config.$container.find('td.inselectzone');
	...
	var typeClassArr = {}
	for(var selectI = 0; selectI<nsUI.resultTable.selected.length; selectI++){
		var selectTDID = nsUI.resultTable.selected[selectI];
		var tdData = nsUI.resultTable.data.tdData[selectTDID];
		switch(tdData.type){
			case 'none':
				break;
			default:
...
//设置样品id，设置实验顺序
nsUI.resultTable.setSampleOrder = function(sampleId){
	...
	var simpleSort = [];
	var simpleSetSort = [];
	$.each(nsUI.resultTable.data.sampleIdSort, function(key,value){
		simpleLength++;
		simpleSort.push(key);
		if(value.type=='set'){
	...
	var setSimpleIdOrderData = nsUI.resultTable.setSimpleIdOrderData;
	if(typeof(nsUI.resultTable.data.sampleIdSort[sampleID])=='object'){
		var sortConifg = nsUI.resultTable.data.sampleIdSort[sampleID];
		//判断是添加或者修改
	...
	function returnError(){
		...
		nsUI.soundplayer.stop();
		return errorReturnData;
	}
	//成功的返回
	function returnSuccess(){
		nsUI.soundplayer.insert();
		for(var orderI = 0; orderI<setSimpleIdOrderData.order.length; orderI++){
			var orderObj = nsUI.resultTable.data.sampleIdSort[setSimpleIdOrderData.order[orderI]];
			orderObj.sort = orderI;
			orderObj.type = 'set';
			var tdData = nsUI.resultTable.data.tdData[orderObj.first];
...
//设置样品id顺序的外接接口
nsUI.resultTable.setSampleOrderInterface = function(sampleIDObj){
	if(debugerMode){
	...
	return nsUI.resultTable.setSampleOrder(sampleIDObj.sampleId);
}
//批量输入样品数据
nsUI.resultTable.setSampleData = function(data){
	...
	function getSortArray(){
		...
		for(key in nsUI.resultTable.data.sampleIdSort){
			var csdata = nsUI.resultTable.data.sampleIdSort[key];
	...
	for(var dataI = 0; dataI<data.length; dataI++){
		...
		if(typeof(sampleID)=='undefined'){
			...
			if(typeof(orderID)!='undefined'){
				...
				if(isSingleData){
					var editTdData = nsUI.resultTable.data.tdData[nsUI.resultTable.currentTdID];
					var dataNodeName = ''
					...
					if (dataNodeName.indexOf("*") == 0){
						...
						nsUI.resultTable.saveTdData(editTdData.id, value);
						editTdData.value.visible = nsUI.resultTable.getVisibleFromReal(editTdData);
						nsUI.resultTable.refreshTd(editTdData.id);

						//下一个同类型的
						var nodeNameData = nsUI.resultTable.data.tdDateByNodeName[dataNodeName];
						...
						var inputResizeData = nsUI.resultTable.getResizeDataByDom($('#'+nextSampleNameTdId));
						nsUI.resultTable.showEditPanel(inputResizeData);
				...
				if($.isArray(nsUI.resultTable.selected)){
					//有选择对象
					for(var selectI = 0; selectI<nsUI.resultTable.selected.length; selectI++){
						var selectID = nsUI.resultTable.selected[selectI];
						var selectData = nsUI.resultTable.data.tdData[selectID];
					...
					$.each(nsUI.resultTable.data.sampleIdSort, function(key,value){
						...
						sortArray.push({
							sampleID:key,
							row:nsUI.resultTable.data.tdData[firstTdID].index.row
						})
					})
					sortArray.sort(function(a,b){
						return a.row-b.row;
					...
					for(var sortI = 0; sortI<sortArray.length; sortI++){
						var sampleTdDatas = nsUI.resultTable.data.tdDataBySimpleId[sortArray[sortI].sampleID];
						var isEmpty = true;
		...
		var sampleData = nsUI.resultTable.data.tdDataBySimpleId[sampleID];
...
nsUI.resultTable.setSampleDataInterface = function(data){
	if(debugerMode){
		if($.isArray(data)==false){
	...
	return nsUI.resultTable.setSampleData(data);
}
//全屏模式
nsUI.resultTable.fullScreenMode = function(){
  var config = nsUI.resultTable.config;
  var $container = config.$container;
  if($container.hasClass('fullscreen')){
    ...
    var offset = nsUI.resultTable.config.$container.offset();
    $container.css({
      'top': '-'+offset.top+'px',
      'left': '-'+offset.left+'px',
    ...
    nsUI.resultTable.fullScreenState = true;
  }  
  config.$editPanel.css('display','none');
}
nsUI.resultTable.exitFullScreenMode = function(){
  var config = nsUI.resultTable.config;
  var $container = config.$container;
  if($container.hasClass('fullscreen')){
    //移除全屏样式
    ...
    nsUI.resultTable.fullScreenState = false;
  }
  config.$editPanel.css('display','none');
}
nsUI.resultTable.offlineMode = function(){

	var config = nsUI.resultTable.config;
	//如果已经打开缓存列表或在离线编辑状态下，按钮不起作用
	if(config.$container.hasClass('cacheListMode') || nsUI.resultTable.offlineState){
		return;
	}
	//console.log(config);
	...
	nsUI.resultTable.cacheAjaxData = {};
	nsUI.resultTable.offlineData = {
	...
	//离线原始配置存储到pad
	nsUI.resultTable.addCache(function() {
		console.log("addCache config OK");
	}, function(error) {
		console.log(error);
	}, {
		url: 'originalConfig',
		body: JSON.stringify(nsUI.resultTable.originalConfig)
	});
	...
	function initCacheListTable(initData){
		...
		nsUI.resultTable.offlineData.server = serverCacheData;
		console.log(nsUI.resultTable.offlineData.server);
		var dataConfig = {	
		...
		var columnConfig = [
			{
				field : nsUI.resultTable.config.cache.cacheListKeyName,
				title : '任务组名称',
			}, 
			{
				field:'isCached',
	...
	function cancelCacheData(row){
		if(row.rowData.isCached == false){
			...
			//离线删除pad
			nsUI.resultTable.removeCache(function(data) {
			...
			delete nsUI.resultTable.cacheAjaxData[row.rowData.cacheRowIndex];
			refreshLocalstorageTable();
		}
	}
	//选中行的动作
	...
	function getAjaxData(rowData){
		...
		$.ajax({
			...
			success:function(data){
				console.log(data);
				nsUI.resultTable.cacheAjaxData[this.rowData.cacheRowIndex] = {
					data:data,
					row:this.rowData,
				...
				nsUI.resultTable.addCache(function() {
					...
					body: JSON.stringify(nsUI.resultTable.cacheAjaxData[this.rowData.cacheRowIndex])
				});
			},
			error:function(error){
				nsVals.defaultAjaxError(error);
	...
	function inputResultTable(row){
		...
		nsUI.resultTable.loadPadData(nsUI.resultTable.cacheAjaxData[row.rowData.cacheRowIndex]);
		nsUI.resultTable.setCacheList(row.rowData.cacheRowIndex);
	}
	//开始录入
	function inputAllResultTable(){
		if(typeof(nsUI.resultTable.cacheAjaxData) == 'object'){
			var keys = Object.keys(nsUI.resultTable.cacheAjaxData);
			if(keys.length>0){
				config.$container.removeClass('cacheListMode');
...
//生成结果录入缓存数据列表
nsUI.resultTable.setCacheList = function(currentIndex){
	var lihtml = '';
	var cacheAjaxData = nsUI.resultTable.cacheAjaxData;
	for(var data in cacheAjaxData){
		...
		lihtml += 
			'<li'+currentHtml+' ns-index="'+data+'">'
				+cacheAjaxData[data].row[nsUI.resultTable.config.cache.cacheListKeyName]
			+'</li>'
	}
	var html = 
	...
	config.$cachePanelSelect.on('click', function(ev){
		if(config.$cachePanelSelect.hasClass('active')){
			...
			config.$cachePanelSelect.find('li').not('.current').one('click',function(ev){
				...
				nsUI.resultTable.loadPadData(cacheAjaxData[cacheRowIndex]);
			});
		}
	});
}
//退出离线模式
nsUI.resultTable.exitOfflineMode = function(){
	var config = nsUI.resultTable.config;
	//如果打开的是缓存列表模式退出
	if(config.$container.hasClass('cacheListMode')){
		removeCacheListMode();
		nsUI.resultTable.exitFullScreenMode();
	}else if(nsUI.resultTable.offlineState){//离线编辑模式
			...
			nsUI.resultTable.getAllStorage(function(data){
				...
				if(data && $.isArray(data.rows) && data.rows.length>0){
					nsconfirm('您将要退出离线模式，存在未上传数据，是否上传并退出？', function(isConfirm) {
						if (isConfirm) {
							if(nsUI.resultTable.isJsonp){//跨域									
								//判断是否能连接服务器
								...
								if(conn){
									//循环上传
									var successFlag = nsUI.resultTable.uploadCacheData(data.rows);
									if(successFlag){
										//循环上传，成功之后刷新页面
										nsUI.resultTable.offlineState = false;
										//移除缓存下拉框
										removeCacheSelect();
										removeCacheListMode();
										nsUI.resultTable.exitFullScreenMode();
										//重新初始化页面
										nsUI.resultTable.init(nsUI.resultTable.originalConfig);
									}
								}else{
									nsalert('未能连接服务器，上传失败');
								}
...
//循环数组上传
nsUI.resultTable.uploadCacheData = function(rows){
	...
	if(count == 0 && typeof(cordova)!='undefined'){
		successFlag = true;
		nsUI.resultTable.removeStorage(function(data){
			console.log("removeAllStorage OK");
		}, function(error){
			console.log(error);
		}, {});
		//删除缓存数据
		nsUI.resultTable.removeAllCache(function(data) {
...
nsUI.resultTable.addCache = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
...
//删除缓存数据：url必填
nsUI.resultTable.removeCache = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
...
//删除所有缓存数据
nsUI.resultTable.removeAllCache = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
...
//获取缓存数据：url必填
nsUI.resultTable.getCache = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
...
//获取缓存数据：获取全部数据
nsUI.resultTable.getAllCache = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
...
//新增数据：即使url相同，也会增加
nsUI.resultTable.addStorage = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
...
//删除数据：无参数，直接删除全部
nsUI.resultTable.removeStorage = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
...
//获取全部上传数据
nsUI.resultTable.getAllStorage = function(successCallback, errorCallback,para){
	if(typeof(cordova)!='undefined'){
