...
//sjj 20191120  多值输入组件
baseDataTable.formatHandler.dateTimeInput = function(columnValue,row,meta,formatData){
    var columnID = meta.col;
    var rowID = meta.row;
    var tableID = meta.settings.sTableId;
    var moduleID = tableID + '-' + rowID + '-' + columnID;
    var dictvalue = formatData.value;//初始化传送的默认值
    var datevalue = columnValue ? columnValue : '';
    var formatMomentStr = formatData.format;
    var isDefaultDateTime = typeof(formatData.isDefaultDateTime)=='boolean' ? formatData.isDefaultDateTime : false;
    if(typeof(datevalue)=='number'){
      datevalue = moment(datevalue).format(formatMomentStr);
    }else{
      if(isDefaultDateTime){
        datevalue = moment().format(formatMomentStr);
      }
    }
    var placeholderStr = formatData.placeholder ? formatData.placeholder : '';
    if(placeholderStr == ''){
      placeholderStr = formatMomentStr.replace(/[ymdhmsYMDHMS]/g, '_');
    }

    var dateHtml = '<input type="text" placeholder="'+placeholderStr+'" value="' + datevalue + '" ns-table="date" columnID="' + columnID + '" rowID="' + rowID + '" name="' + moduleID + '-date" id="' + moduleID + '-date" class="ns-table-input form-control" style="width:100%" />';
    return dateHtml;
}
  ...
  for (var i = 0; i < columnData.length; i++) {
    ...
    if (typeof (columnData[i].formatHandler) != 'undefined') {
      ...
      switch (customizeJson.type) {
        case 'dateTimeInput':
          handlerJson.handler = customizeJson.data.handler;
          handlerJson.format = customizeJson.data.format;
          if(handlerJson.format){
            handlerJson.formatMask = nsUI.dateTimeInput.getMaskFormatStr(handlerJson);
          }
          break;
        case 'uploadFile':
          //data就是handler
    ...
    switch (type) {
      ...
      case 'radio':
        ...
        for(var radioI=0; radioI<radioLength; radioI++){
          ...
          var $tdRadioInput = $tdRadio.find('input[type="radio"]');
          ...
          function fillHtml(data){
            ...
            $('.ajaxselect-plane ul > li').on('click',function(event){
              ...
              if(typeof(obj.data.handler)=='function'){
                obj.data.handler(funcObj,jsonData);
              }
      ...
      case 'multithumb':
       ...
       /* var $tdThumb = $container.find('td.td-thumb');
        ...
        break;
      case 'dateTimeInput':
        var $dateTimeInput = $container.find('td.td-dateTimeInput input[type="text"]');
        $dateTimeInput.inputmask(handlerObj[columnid].formatMask);
        $dateTimeInput.on('blur',{config:handlerObj[columnid]},function(blurEvent){
            var _config = blurEvent.data.config;
            var $this = $(this);
            ...
            if(inputStr != ''){
                ...
                if(inputStr.search(/_/)>-1){
                  nsalert('未完成输入','warning');
                  return false;
                }
                inputStr = moment(inputStr, _config.format).format('x');
                if(inputStr != 'Invalid date'){
                  returnTimeStamp = parseInt(inputStr);
                  //判断是否使用当前时期的结束时间 23:59:59
                  if(_config.isUseDayEnd){
                    returnTimeStamp += 86399000;
                  }
                  //判断区间
                  if(typeof(_config.validStartTimeStamp)=='number'){
                    if(returnTimeStamp < _config.validStartTimeStamp){
                      var timeStr = moment(_config.validStartTimeStamp).format('YYYY/MM/DD HH:mm:ss');
                      //提示信息类似于 不早于2018/08/08
                      return false;
                    }
                  }
                  if(typeof(_config.validEndTimeStamp)=='number'){
                    if(returnTimeStamp > _config.validEndTimeStamp){
                      var timeStr = moment(_config.validEndTimeStamp).format('YYYY/MM/DD HH:mm:ss');
                      return false;
                    }
                  }
                }else{
            ...
            funcObj.value = returnTimeStamp;
            funcObj.rowData[dataField] = returnTimeStamp;
            if(typeof(_config.handler)=='function'){
              _config.handler(funcObj);
            }
        })
        break;
    }
 ...
 /* baseDataTable.data[dataTableID].dataConfig = dataConfig;
  ...
  if ($.isArray(dataConfig.dataSource)) {
    ...
    if (isSrc) {
      ...
      if(authorization){
        if(typeof(configObj.ajax.headers) != 'object'){
          configObj.ajax.headers = {};
        }
        configObj.ajax.headers.Authorization = authorization;
      }
    } else {
  ...
  for (var columnI = 0; columnI < columnData.length; columnI++) {
    ...
    if (typeof (columnData[columnI].formatHandler) != 'undefined') {
      ...
      if(columnData[columnI].formatHandler.type == 'input'){
        if($.isArray(columnData[columnI].formatHandler.data)){
          if(columnData[columnI].formatHandler.data[0].btns){
            obj.className = 'td-' + columnData[columnI].formatHandler.type+' td-input-buttons';
          }
        }
      }
...
baseDataTable.drawHandler = function (tableID) {
  ...
  if (baseDataTable.data[tableID].uiConfig.isMulitSelect == true) {
